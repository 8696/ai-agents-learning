...
#### `coach complete` 过关检查（复盘行）

1. `{NN}-模块复盘.md` 已按 §7.2 写过（不是只有标题没实质内容），「模块验收对答」表每一项都有回答
2. 「还没搞懂的」每条都有去处（模块编号或「自己查」）
3. 「落过的 Demo」表与 `apps/{模块}/` 实际子文件夹对得上
4. **不检查任何新代码 / 端口 / yarn 脚本**，也不写 §5.2 Demo 判断表

#### 硬性规则

1. 文件标题必须复制该行「重点」原文，禁止自拟短标题。
2. **一个 MD 只对应一行**小节进度；不可把多行进度合并进一个文件。
3. `coach complete` 勾 ✅ 前：对应 MD 仍只有标题没实质内容，或知识不满足 [§7.0 结果验收](../AGENTS.md#70-写回扩写任意终端)（没漏、能转述、有通俗例子），或丢掉了已出现的追问，或外部条 Demo 还没判断要不要写，或该落可运行 Demo 却没做 → **先补再问去掉哪条**，不准勾。不要因为行数多就先压缩再勾。
4. `coach status`：报告「已 ✅ 但小节 MD 仍只有标题没实质内容」；「MD 已写但未 ✅」只提示。已 ✅ 但没有例子 → 指出；要改须学习者明确说要。已 ✅ 但没有「需求清单」（**验收准绳**，勾 ✅ 时按需求**逐条对**）→ 指出；要改须学习者明确说要。不要把「很长」报成不合格。
