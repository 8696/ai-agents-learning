# 演示写到哪里细则（§5.0 / §5.1 / §5.3）

> **不会自动注入。** Cursor / Codex / Claude Code 只自动读仓库根 [AGENTS.md](../AGENTS.md)。
> **何时必须 Read 本文件：** 判 Demo / 写出 Demo 判断表；结论是可运行时再读全文（写出或修改 HTML/koa、扩 CATALOG、拆多场景、协议 A/B）。根 [AGENTS.md §5.2](../AGENTS.md#52-小节-demo) 只留三结论摘要；Demo 判断表与打钩前检查以本文件为准。

## 5. 演示写到哪里

`apps/` 是本仓库**唯一**可以把代码写进去的地方：模块 00 mini-app（HTTP）+ 每条外部小节的最小可运行 Demo。**模块复盘不写代码**（[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)）。学完模块 23 后若要作品集，从零建独立项目。
...
| 类型 | 位置 | 干什么 | 怎么写 |
| ---- | ---- | ------ | ------ |
| **模块 00 mini-app** | `apps/00-环境准备/01-mini-app-step-1/` | 模块 00 的代码写到哪里（HTTP + SSE） | 与 §5.3 相同：根目录 `server.ts` + `public/index.html`；不拆成多个项目；**禁止** `src/index.ts` |
...
**每条外部小节在学完、在进度表打钩 之前，必须判断这一小节要不要留下一份 Demo。** `apps/{模块}/{小节}/` 是"只验证当前这一条"的教学样例，**不**为后面条目做铺垫。

#### 何时判断（start 预告 + 说写就写）

判断结论在 `coach start` 结束就必须告诉学习者。默认你没明确说要时，写入小节文档后再写代码；**学习者说写就立刻写**（不等写入小节文档，禁止用「讲课当时」拒绝）。

| 时机 | 做什么 |
| ---- | ------ |
| `coach start` 详解结束 | 按下面标准判完，写出 [这一小节接下来会写出什么](./03-progress.md#61-这一小节接下来会写出什么)。用人话说清「要 / 不要写 Demo」。默认先不写出 |
| 学习者说写 / 先写出演示 / 强制出 Demo | **立刻**按结论写满这一小节的演示 |
| 写入小节文档之后、`coach complete` 在进度表打钩 之前 | 再打 **Demo 判断表**（未打不准在进度表打钩）。与 start 预告一致则写「与 start 预告一致」；改判须一句原因。该落还没落则当场做 |

| 行类型 | Demo 判断 |
| ------ | --------- |
| 外部概念条 | **必须判断**（结论三选一：无 / 伪代码 / 可运行；**不是**每条都要可运行代码） |
| 「模块复盘」行 | **不写 Demo 判断表**。结论固定为「只写小节文档」，按 [§7.3](../AGENTS.md#73-模块复盘进度表最后一行) 写 `{NN}-模块复盘.md`。禁止用「跳过判断 / 只写笔记 / 补缺口」敷衍过去 |

已在进度表打钩的旧小节默认不改 Demo，除非学习者明确说要。

判断由助手做，对照「本条要能讲清」。一眼能定（纯概念 → 无）就自己判；**两可 / 不好处理**走 [00-mode.md §0.3](./00-mode.md#03-不好处理时先给选项)。学习者可改判：「这次不要 Demo」/「强制出 Demo」。

#### 判断标准

先问：关上文件后，是否必须看见一次**可观察的运行结果**（或写清可照着敲的步骤），才能讲清「本条要能讲清」？

| 结论 | 何时 | 落哪 |
| ---- | ---- | ---- |
| **无** | 纯概念（定义、来源、对照）；跑起来没有新信息 | 小节文档 写 `Demo：无` + 一句理由 |
| **伪代码** | 机制步骤必须写清，但不值得起进程（如 Attention 数据怎么走） | 写进该条小节文档 的机制 / 例子，**不建** `apps/` 文件夹；MD 写 `Demo：伪代码（见机制）` |
| **可运行** | 必须看见一次可观察的运行结果才能讲清「本条要能讲清」（调 API、协议字段、本地 encode / Zod parse 都算） | **一律 §5.3**（`server.ts` + 页）。哪怕逻辑只有几行、不调 LLM，也要有按钮和 `#output`。MD 写 `Demo：已写出 apps/…` |

禁止为凑而建空文件夹。可运行 Demo **走 [agents/05-demo.md §5.3.2](agents/05-demo.md#532-完整版--必做的-6-项替代-52-最低标准) 六项**，禁止用「happy path + 一个行为然后关进程」敷衍过去，也**禁止**用纯终端 `index.ts` 敷衍过去。不要把外部节奏小节的演示 做成第二份模块 00 mini-app。

#### Demo 判断表（在进度表打钩前必须写）

**判断前必须先检查变体**（[§6.3](../agents/05-demo.md#63-讲完前的自查触发器)）：这一小节核心概念的所有变体/分支是否列全？→ 每个变体决定"无 / 伪代码 / 可运行"；→ 每个变体对应需求清单一条 + 每个可观察变体对应至少 1 个 step-N。漏变体 = 打钩前检查不会过。

```text
Demo 判断
- 小节：{该行「重点」}
- 结论：无 | 伪代码 | 可运行
- Step 1 第一件事：一句话点明这一小节 Step 1 只演示什么（如「调 LLM 真跑一次 happy path」/「请求/响应 shape 静态展示」/「单 tool_call 真调 LLM 闭环」）
- 锁定时机：学习者主动决定（不是自动触发；详见 §5.3.14）
- 理由：{对照「本条要能讲清」}
- 落点：— | 该小节文档 机制节 | apps/{模块文件夹}/{小节文件夹}-step-{N}/ · yarn app:{模块两位}-{小节两位}-{英文短名}-step-{N}
...
| 打钩前检查 | 本文件对应 | 不过 |
| -- | -- | -- |
| **1** | 用词（黑话 / 缩短词）：当前小节文档 + 代码里给人看的字。对照根 [必须讲人话](../AGENTS.md) | STOP，不进后面 |
| **2** | [§7.2](../AGENTS.md#72-写入小节文档--小节进度对齐) 小节文档知识 | STOP，不进后面 |
| **3** | 本节 §5.2：Demo 行不是「还没判断要不要写」；可运行须锁定 step + yarn + `check-demo`（含行数上限 / 一路由一文件）+ `cd apps && yarn typecheck`（与 check-demo 分开，check-demo 之后跑）+ [§5.3.2](#532-完整版--必做的-6-项替代-52-最低标准) 六项齐 + 端口三处一致 + [§5.3.8](#538-http-demo-拆分多场景--多接口时强制) 交互跟笔记走 / 对照拆请求 / 主流程单独成文件 | STOP |
| **4** | [§5.4](#54-目标--代码整合打钩前检查先抽清单再逐项核对新).A+.B 独立 subagent：抽清单 + 逐项核对 | STOP；补代码或拆成两条进度 |

「锁定 + §5.3.2 + check-demo」= **打钩前检查 3**；目标↔代码 = **打钩前检查 4**。复盘行先走打钩前检查 1（只查复盘文档用词），不写打钩前检查 3/4（[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)）。
...
- **跑入口必须是 yarn 脚本，名字要能看懂是哪一条。** 新建可运行 Demo 时，同步在 `scripts` 加一条，禁止只留 `yarn tsx 长路径` 当主入口：
  ...
  - 可运行但 `package.json` 里没有对应 `app:{MM}-{SS}-{name}-step-N` → **不准在进度表打钩**（与下面打钩前检查相同）。
- 共用：`typecheck`（`tsc --noEmit`）。
- Key 只读 `apps/.env`：`load-root-env.ts` 从 `apps/load-root-env.ts` 读取 `apps/.env`。Demo 里不要再放 `.env`。
- 技术栈与 [§5.0](#50-代码约定node--ts--注释--key--选型) 相同：TS 5 + Node ≥22 + yarn；`tsconfig` `extends` `./tsconfig.base.json`；相对导入带 `.js`；`catch (error: unknown)`。写 `.html` 时 Tailwind 脚本必须用 §5.0 **HTML** 那一段（含 `integrity`），不要换。
- 起步依赖：见 [§5.0](#50-代码约定node--ts--注释--key--选型)。这一小节需要协议 B 再加 `@anthropic-ai/sdk`。不要预装智谱专属 SDK / LangChain / 向量库 / Playwright（模块未学到）。
- **无** `LEARNING.md`。概念 / 易混 / 例子只在该条小节文档。
...
写出演示 **默认情况下必须真调用大模型**。**禁止**用本地 mock / 固定返回值 / hard-coded JSON 冒充模型响应（包括「1 个函数返回假数据 + 1 个按钮 + 1 个 #output」这类最小闭环的第一步）。
...
**生效范围**：本规则对**此后新写出 / 新改**的 Demo / step 强制。已锁定旧 Demo **已在进度表打钩的旧小节默认不改**（除非你明确说要），与 §5.6 日志一致。

#### 5.3.1 适用范围

| Demo 类型 | 走哪 |
| --------- | ---- |
| **可运行外部节奏的小节**（调 API、流式、对照、以及 Token encode / Zod parse 这类本地计算） | **§5.3**（本节，前后端；必须有 HTML） |
...
| # | 项 | 含义 |
| - | -- | ---- |
| 1 | **Happy path** | 这一小节主要用例完整跑通（对照、并排、多端点等按该条需求，不要只打一次就关进程）。**笔记写了几步交互就几步**（[§5.3.8](#538-http-demo-拆分多场景--多接口时强制)），禁止压成一个按钮 |
| 2 | **错误处理**（≥2 类，能一眼分开） | **一类**：页面能看见的失败（HTTP 4xx / 5xx，或 fetch reject，或这一小节教学点里的失败：取消 / 429 / Zod 校验）。**另一类**：与第一类不同的失败通道。`catch` 后必须有面向人的红字 + `#status-pill` 变红。**不要**每页都强制「故意断网」按钮；这一小节教学点不是网络时，用空输入 400、取消、或业务失败即可。 |
...
yarn 脚本仍只指向 `server.ts`；禁止为每个场景再开一个入口或端口。子目录名按这一小节职责语义取，上表是常见切法，不是强制同名。
...
- **端口（强制：全仓库默认口不得重复）**：
  ...
  4. 跑 `node scripts/check-demo.cjs`：检查项包括「三处一致」「默认口全仓库不重复」「layout.js fallback 与这一小节一致」。
  5. 改口 / 删 demo 同理同步这四份。
      ...
      return (
        <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
          ...
          >
            ...
            <span id="env-info">
              端口 {这一小节 5MMSS} · 协议 A · provider {env.provider} · model {env.model} · Key {env.hasKey ? "✅" : "❌"}
            </span>
          </footer>
        </div>
      );
...
| **本步核心流程**（Agent 循环 / 调大模型 / 规划器等） | **必须单独成文件**，见下方「把代码写进 apps/ 之前先点名本步核心」 |
...
##### 把代码写进 apps/ 之前先点名本步核心（主流程单独成文件 · 2026-09-10 立）

本仓库是学**大模型**和 **Agent** 的。文件拆开是为了让学习者打开就能顺着主路径看懂，不是为了目录好看。

写出或修改演示 **第一件事**：Agent 自己判断并写进核心文件头 —— **本步核心：……**（一句人话）。常见就这几类，按本步教学点选 **1 个**（偶尔 2 个，例如「规划」和「按清单执行」）：
...
| 压缩、摘要、Token 预算裁剪 | `lib/flow/` 或这一小节对应职责目录里的主函数文件 |
...
写出或修改演示 **之前**对照该条写入小节文档 MD 的需求清单 + 例子 + 易混：笔记写了几步人机交互，页面就必须能逐步点完。**禁止**用「最简闭环 / 一个按钮跑完」代替笔记里的步骤。
...
「服务端其实跑过了、前端只展示最终 JSON」≠ 实现了笔记里的交互。打钩前检查 4 把「压成一次点击」判 **未实现**。
...
**生效**：此后**新写出 / 新改**的 Demo / 新 step 强制。已锁定旧 Demo **默认不改**（`check-demo` 对 2026-09-10 前已超限 / 一文件多 URL 的**具体文件**豁免；新文件 / 新 step 文件夹不准再超）。复制旧 step 做 `step-(N+1)` 时，抄过来的超限文件**不算豁免**——必须先拆再加功能。
...
**后端 `lib/` 常见切法**（子目录名按这一小节语义取，不必照抄英文单词，但**必须**能一眼看出类）：
...
**`coach complete` 打钩前检查补一句**：该写出可运行时，`server.ts` 仍须是 yarn 入口；本步核心在 `lib/flow/` 单独文件（文件头有「本步核心」）；一个业务 URL 一个 route 文件；对照可以同页但每侧分请求、页面拆组件。打钩前检查 3 另查「交互跟笔记走」+「对照拆请求」+「主流程单独成文件」；`check-demo` 拦行数上限和一文件多 URL。
...
- `hasKey: false` 时页脚显示 `Key ❌（apps/.env 未配置该家 Key）`。**这一小节会调模型**时再让主按钮 `disabled`——比等请求 503 更早告诉人。
- **这一小节不调 LLM**（本地 encode / Zod 等）：`/health` 仍回 provider / model / hasKey，并加 `callsModel: false`。页脚写「本地计算 · 不调 LLM」；**主按钮不因缺 Key 而 disabled**。
...
**这些 Demo 的读者是「几个月后回来复习的自己」**。关上文件后还能讲出来只看页面，也要能讲清这一页在演示什么。教学信息写在**页面上**，不是只写在 README 或代码注释里。
...
- 分寸：解释「这一步在协议里是什么」，**不要**把小节文档 的完整教学搬进页面。页面是「看得见的机制」，MD 是「讲透的知识」。
...
| 状态/按钮 | `中文（English · 类别）` | `跑预算（算账 → 裁剪 → 调真模型 · runBudget）` / `演示后端 5xx（5xx · 第二类错误 · /api/budget-force-error）` |
...
- **发明教学分类名当术语（2026-09-12）**：页面上禁止出现「听起来像知识点、其实只是本 Demo 自造」的标签（例：「编号问 / 口语问」——应写成「带货号的问句」「日常说法、库里未必同词的问句」）。假英文括注（`口语问（Spoken Query）`）同样禁止。代码 id 可用 `with-sku` / `paraphrase`；**给人看的字只说场景**。对照根 [AGENTS.md 必须讲人话](../AGENTS.md)。
- **页面文案不许省词、不许缩短（2026-09-12）**：按钮、标题、教学注解必须写完整说法（「把演示写进 apps/」「在进度表打钩」）。缩短词只许出现在「不准写」的例句里。对照根 [AGENTS.md 必须讲人话](../AGENTS.md) 第 3 类。
...
**Coach 写出或修改演示 当下自查清单（2026-09-10 立 · 必走 · 避免再踩「英文术语裸展示」）**：

写出或修改一个 `public/index.html` 前，先按下面清单扫一遍现有/新写的字符串；不通过不要在进度表打钩：
...
- 核心教学点只放代码注释 / README，**不**进页面（违反 §5.3.11「关上文件后还能讲出来只看页面也要能讲清」的本意）
...
- 每条 Demo 内部的 `components/` / `utils/` **只服务这一小节**。要在新 Demo 用同样的组件：**照本节规则重写一份**（可以照抄自己写过的思路，但文件归这一小节所有），不要跨目录引用。
- 重复几十行 UI 是可接受成本；跨小节耦合不是——改一条会连坐别条，教学 Demo 必须能单独删掉。

#### 5.3.13 协议 A / 协议 B 分开写出演示（强制）
...
**`coach complete` 打钩前检查补一句**：这一小节同时演示 A 与 B 时，`apps/` 下必须能看到两个文件夹 + 两条 yarn 脚本 + 两个端口；属于「对照」例外的，`lib/` 下必须有按协议分开的子目录。
...
**问题**：本节其他子条款（「锁定 = 学习者主动」「step-N = 工作区」）容易被读成"教练等学习者说再做"。但学习者从空 MD 起步时，对"这条**最完美 / 最深度 / 最广度**的 Demo 长什么样"没有先验视角——而教练有（来自「本条要能讲清」+ 配套模块写入小节文档 + 已写出同模块其它小节）。**教练不引导 = 教学责任缺位**。
...
| §5.2 Demo 判断表 | 写「子节拆分：N」 | 写「Step 1 第一件事」+「锁定时机：学习者主动决定」+「N 动态」 |
| 小节文档「Demo 子节进度」表 | ⬜/🔄/✅ 三态 | **🔄（打磨中）/ ✅（锁定）** 二态 |
...
##### 锁定 step = 学习者的主动决策（不是自动触发；**不是** `coach complete` 在进度表给这一小节打钩）

**两个流程完全不同**——Agent 看到「锁定」字样要分清：

| | 锁定 step-N | 在进度表给这一小节打钩（`coach complete`） |
| --- | --- | --- |
| **改的文件** | 该小节文档「Demo 子节进度」表 step-N 那一行（`🔄` → `✅`） | 该模块 `README.md` 小节进度表那一行（`⬜` → `✅`） |
| **跑什么** | **不跑任何打钩前检查**——以下 3 条是已完成事实，不是锁定时再去跑 | 必须跑打钩前检查 1→2→3→4 全过 |
| **触发口** | 学习者主动说「锁定 step-N」 | `coach complete` 命令 |

**学习者主动决定锁定 step-N**：「这步够好了，进下一步」→ 这一刻 freeze 当前代码，三件事**同时**满足（**已完成事实，不是锁定时再去跑**）：
...
**反例**（2026-09-16 模块 10 · 02 step-12 锁定踩坑）：学习者说「锁定 step12」，Agent 跑去跑打钩前检查 1·用词（grep「闸门」等黑话）+ 用 AskUserQuestion 让学习者选怎么处理检出项。**锁定 step 不需要跑这些**——直接改子节文档「Demo 子节进度」表 step-12 那一行即可。
...
**打钩前检查只在「锁定」时校验**。step-1 起手可以小到「1 个端点 + 1 个按钮 + 1 个 #output，调真 LLM」，不需要 health / env-info / 两类错误。等学习者说「锁定」，再补齐 §5.3.2 → 过 check-demo。（§5.3.0 硬规则：默认调真模型，**禁止**用本地 mock 凑最小闭环；本地计算 / 纯协议形状演示例外）
...
##### Step 1 入门形态（§5.2 Demo 判断表写）

§5.2 Demo 判断表**不预判 N**，只写 Step 1 入门形态。原则：

- `step-1` = **能多小就多小**；目的是让学习者**看见**概念或**跑一次**最简闭环
- **推荐形态 A**（仅当本步教学点就是「单次调用」时）：「1 个端点 + 1 个按钮 + 1 个 #output」，调真 LLM 跑一次 happy path；§5.3.0 硬规则禁止本地 mock 凑闭环
- **若本步教学点本身就是多步交互或对照**：入门形态也必须是多步 / 分侧请求。禁止用「最简闭环」当借口把笔记里的步骤压掉（[§5.3.8 交互跟笔记走](#538-http-demo-拆分多场景--多接口时强制)）
- 其他可选项 B（请求/响应 JSON 静态展示）/ C（纯伪代码）——教练按这一小节特性选
...
| **写入小节文档** | `docs/学习模块/{模块}/{小节}.md`（每个引用都要更新）、`{模块}/README.md` 进度表 / 验收表、`{模块}/{模块复盘}.md` 的 demo 表 / 跨小节说明 |
...
- `coach complete` 在进度表给这一小节打钩前：MD 笔记已写入小节文档 + **至少 1 个 step-N 锁定（✅）** + 学习者说「我懂了」
- **不要求**所有未来 `step-(N+1)` 完成，因为 N 是动态的
- 已 ✅ 的小节**可以**继续加 `step-(N+1)`（加深场景）；新加的 step 走相同打钩前检查
...
**`coach complete` 打钩前检查补一句**：这一小节可运行时，`apps/{模块文件夹}/` 下至少要有 1 个 `{小节文件夹}-step-{N}/` **已锁定（✅）+ check-demo 过**。未锁定的 step-N 不算完成。**这解决「demo 完整度」打钩前检查；目标↔代码整合还须过 §5.4**（目标→代码覆盖 / 文档→代码对齐两段）。
...
**问题**：agent 写出或修改完可运行 Demo 经常顺手起服务做 verify（`preview_start` / `Bash yarn app:…&`），verify 完忘了关，端口占着——下次学习者回来开 `yarn app:...` 直接撞口 / `EADDRINUSE`；Demo 一多互相影响。

**规则**：

| | 怎么做 |
| -- | ------ |
| **要不要起** | agent 自己判断——这一小节改动影响运行时（HTTP 行为 / 页面渲染 / 流式响应 / 端口冲突），启了看得清就启；纯类型 / 静态检查 / 文件 Read 就够的**不启** |
...
**生效范围（2026-09-07）**：这一小节闭环 / 人话规则对**此后新写出或新改的 Demo / 新 step** 强制。**已锁定的旧 Demo 默认不改**（除非学习者明确说要）。本地 freeze 副本不随顶层改动。
...
- **硬规则（锁定 / 未锁定一视同仁）**：每一个 Demo、每一个 step，**把代码写进 apps/当下就必须完整拷贝**本地 `lib/logger.ts`。**不论**该 step 是否已锁定、是否还在打磨——**一律禁止**运行时 `import apps/logger.ts` / `createLogger` 委托顶层
- **每一条 Demo / 每一个新 step 把代码写进 apps/当下**：把当时顶层 `apps/logger.ts` **完整拷**到 `apps/{demo}/lib/logger.ts`，底部再 `export const logger = createLogger(.../logs)`；业务只 `import { logger } from "./logger"`
...
**本地 freeze（写出演示 当下；与「锁定 step」无关）**
- 「锁定 step」= 教学点冻结；「拷贝 logger」= **把代码写进 apps/就做**，两者无关。未锁定工作区也必须是完整本地副本
...
- **LLM / HTTP 响应写整个对象**；`字段释义` = 只解释这一小节课用到的返回字段（如 `finish_reason` / `tool_calls` / `usage`），不要给 SDK 每个键做词典
...
- 新写出 / 新改 Demo：从顶层**完整重拷** `lib/logger.ts`（旧锁定副本默认不改，除非你明确说要）
...
| 档 | 哪些 | 五条日志之外再打什么 |
| -- | ---- | ------------------ |
| **主路径详细写** | 真发网络请求的模型/HTTP；这一小节主路径；知识点封装函数 | 函数体逐步（子调用各自五条日志）；返回值上加**教学相关** `字段释义` |
...
**业务级 schema / 命名约定由 demo 自己起**——比如 "`language` / `no_marketing` / `last_complaint`" 这种业务键、`value` 内部 JSON 结构、表名要不要分多张（多业务共存时），**都不在本节钉死**；写出演示 时按业务写。
...
- 不规定**业务层** schema：业务 key 命名（`language` / `no_marketing` / `last_complaint` …）、`value` 内部 JSON 结构、多业务共存时分几张表 → 写出演示 时按业务定
...
**问题**：旧打钩前检查（[§5.2](#52-小节-demo)「`coach complete` 打钩前检查」+ [§5.3.14](#5314-demo-子节拆分动态引导由浅入深新) 「打钩前检查」）只查「demo 跑通没 / §5.3.2 6 项齐没 / check-demo 过没 / 锁没锁」。**这些都只是「代码存在」**——但「代码存在」≠「本条要能讲清」。MD 写满了「含并行调用」+「Promise.all」，代码里却用同步 `.map(...)`，跑通 + 6 项齐 + check-demo 过 + 双 step 锁定，**打钩前检查全过、可在进度表打钩**——可「并行调用」这个目标点根本**没在代码里真实跑过**，关上文件后还能讲出来讲不清「真实跑过的并行时序长什么样」。

**解决**——在进度表打钩前再加一道打钩前检查，**先抽清单再逐项核对**，**两段都过**才能勾。任意一段不过 → 不准在进度表打钩。
...
1. 读该行「本条要能讲清」原文（[模块 README 小节进度](AGENTS.md#31-当前小节锁定对话中途绝不换成别的小节)），**逐条**拆出「目标点」清单。一行一条，每条必须含**动词 + 可观察行为**（「能画出 X」「含 Y」「支持 Z」）。复述「本条要能讲清」原文不算拆。
...
1. 通读小节文档 的「是什么 / 为什么 / 易混点 / 例子 / 踩坑 / 需求清单 / 还没搞懂的 / 我追问过的」各节，**逐条**列出 MD 提到的**可被代码实现的具体行为**（机制点 / 例子场景 / 需求场景 / 踩坑演示 / 易混对比）。
   ...
   - 「还没搞懂的 / 我追问过的」节里若学习者点了"应去 X 模块学" → 不算这一小节缺口（属跨小节依赖），但要标 "→ 模块 YY · ZZ"。
...
#### 打钩前检查逻辑（对应 [03-progress 打钩前检查 4](./03-progress.md#打钩前检查-4--54-独立代码覆盖验证抽清单--逐项核对)）

**写死的约定**：① MD 进打钩前检查 4 清单的项 → 代码必须**已实现**；② **状态栏只许「已实现 / 未实现」**（**禁止** `⚠` / `❌` / `✅` /「警告但阻塞」/「单向缺口不阻塞」/「❌ 但不阻塞」——旧表出现「❌ · 不阻塞」一律按**未实现**计）；③ 未实现 = 阻塞；缺口只许 **补代码** 或 **拆成两条进度**，**禁止**问「接受缺口写进 MD」；④ **抽清单 + 核对**都由独立 subagent **一次做完**（禁止教练自查后再派 subagent 复扫同一清单）。

```
coach complete：打钩前检查 1 → 2 → 3 → 4
打钩前检查 4 = 独立 subagent
...
| `coach start`（外部节奏的小节） | **必须写出**——即便看着齐也主动扫一遍 |
| `coach complete`（外部节奏的小节在进度表打钩前） | **必须写出**——不过不准在进度表打钩 |
| `把笔记写进小节文档`（增量更新某条 MD） | **必须写出**——增量更新后必须重跑 §5.4.B（文档改了，对齐可能破） |
| `模块复盘`（最后一行） | **不写**——[AGENTS.md §7.3](../AGENTS.md#73-模块复盘进度表最后一行) 只过 MD 打钩前检查，不查代码 |
| 已 ✅ 的旧条（不在当前小节） | **不写**——除非学习者明确说要补漏 |

#### 输出模板（写到小节文档 的「Demo 子节进度」表之后）
...
- 「代码存在」就放打钩前检查——必须逐点对证据
- 「看着齐就放过」——必须主动扫
- 把「目标点」写成「机制描述」（目标点 = 可观察行为，不是「过程定义」）
- 把「MD 讲点」当成「MD 章节标题」——必须是 MD 里**具体的一句机制 / 例子 / 踩坑**
- §5.4.A 未实现 = 「拒绝当前小节 ✅」；不是「自动建 step-(N+1) 补」——补不补、怎么补由学习者在 **补代码 / 拆成两条进度** 里选（参 [§5.3.14 交互检查点协议](05-demo.md#交互检查点协议每步之间必走) + [03-progress 打钩前检查 4](./03-progress.md#打钩前检查-4--54-独立代码覆盖验证抽清单--逐项核对)）
- 用 `⚠` / `❌` / `✅`（打钩前检查状态栏）/「警告但阻塞」/「单向缺口不阻塞」/「❌ 但不阻塞」/「注释算已实现」敷衍过去
- 问「接受缺口写进 MD / 已知缺口节」
- 已 ✅ 的旧小节默认不回头跑 §5.4 打钩前检查（除非学习者明确说要补漏）

#### 打钩前检查之间的分工

| 打钩前检查 | 解决什么 |
|---|---|
| **[打钩前检查 1 · 用词](./03-progress.md#打钩前检查-1--用词黑话缩短词2026-09-12)** | 当前小节文档 + 代码里给人看的字有没有黑话 / 缩短词 |
| **[打钩前检查 2 · §7.2](./03-progress.md#打钩前检查-2--72-小节文档知识覆盖)** | 小节文档是否写完、能否讲清「本条要能讲清」 |
| **[打钩前检查 3 · §5.2](./03-progress.md#打钩前检查-3--52-demo-完整度)** | demo 是否存在 / 跑通 / §5.3.2 / check-demo / 已锁定 |
| **[打钩前检查 4 · §5.4 独立验证](./03-progress.md#打钩前检查-4--54-独立代码覆盖验证抽清单--逐项核对)** | 从小节文档抽清单 + 对代码逐项已实现（独立 subagent 一次做完；禁教练双扫） |
| **[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)** | 模块复盘行：先走过打钩前检查 1（只查复盘文档用词），再过知识；不写打钩前检查 3 / 4 |

> **Demo 覆盖 = 打钩前检查 4 过**（§5.4.A 正向 + §5.4.B 反向，同闸内由 subagent 抽清单并核对）。A 看「**目标点有没有**」，B 看「**小节文档讲到的有没有**」。两层都不漏 = Demo 真正完整。
