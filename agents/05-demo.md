...
| 沉淀之后、`coach complete` 勾 ✅ 之前 | 再打 **Demo 判断块**（未打不准勾）。与 start 预告一致则写「与 start 预告一致」；改判须一句原因。该落还没落则当场做 |
...
#### `coach complete` 闸门（对外条目：闸门 1→2→3→4）

全文与必查清单：[agents/03-progress.md `coach complete`](./03-progress.md#coach-complete-勾前必报)。本文件只留 Demo 相关口径：

| 闸门 | 本文件对应 | 不过 |
| -- | -- | -- |
| **1** | [§7.2](../AGENTS.md#72-沉淀--小节进度对齐) MD 知识 | STOP，不进后面 |
| **2** | 本节 §5.2：Demo 行不是 `未判`；可运行须锁定 step + yarn + `check-demo` + [§5.3.2](#532-完整版--必做的-6-项替代-52-最低标准) 六项齐 + 端口三处一致 | STOP |
| **3** | [§5.4](#54-目标--代码整合闸门两段式新).A+.B 教练自查逐项 | STOP；补代码或拆条 |
| **4** | 独立 subagent 再查闸门 3 同一清单 | STOP；补代码或拆条 |

「锁定 + §5.3.2 + check-demo」= **闸门 2**；目标↔代码 = **闸门 3+4**。复盘行不打闸门 2/3/4（[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)）。
...
**`coach complete` 闸门补一句**：该落可运行且命中本条「必须拆」时，`server.ts` 仍须是 yarn 入口；业务在 `routes/` + 分层 `lib/`；`public/` 须有场景页（`pages/` 或等价），共享 UI 按 `components/` + `utils/` 分类，不能只有一个塞满的 `index.html` / 一个巨型 `ui.js`。
...
**`coach complete` 闸门补一句**：本条同时演示 A 与 B 时，`apps/` 下必须能看到两个文件夹 + 两条 yarn 脚本 + 两个端口；属于「对照」例外的，`lib/` 下必须有按协议分开的子目录。
...
- 教练一次连落多步（旧模型残留，必须改）
- `coach complete` 不过时一次打包补多个 Demo 缺口（必须走 [03-progress 不过时的收口](./03-progress.md#不过时的收口逐条列全--由浅入深只推一步--2026-09-05)：列全 + 只推由浅入深第 1 刀）
...
**`coach complete` 闸门补一句**：本条可运行时，`apps/{模块文件夹}/` 下至少要有 1 个 `{小节文件夹}-step-{N}/` **已锁定（✅）+ check-demo 过**。未锁定的 step-N 不算完成。**这解决「demo 完整度」闸门；目标↔代码整合还须过 §5.4**（目标→代码覆盖 / 文档→代码对齐两段）。
...
**约定（业务代码写法）**
- `scope`：**中文节点名**，讲清"在哪一段 / 哪个步骤"（例：`调用模型-开始`、`循环-第3轮`、`查询库存-结束`）
- `msg`：**一句话中文动作**，讲清"现在在做什么"（例：`开始调用模型`、`拿到返回值`）。禁止英文机器味（`← got response`、`enter fn`）
- `explain`：**人话**，**必填**。每一条都要能回答两句：① **为什么这么调用**（上一拍怎么把本拍推过来的）② **当前到哪一步**（整次请求里的位置）。不要只写「打这条方便排错」
- **文件里不写服务名**：logDir + 文件名自带 demo 语义；头里不带 `[serviceName]` 块
- **密钥不进日志**：API Key / Token 打码；入参里若带 Key 只打「已配置 / 未配置」或末 4 位
...
- **`data.__code` 字段**：约定为**源代码字符串**；logger 自动以 `── code ──` / `── end code ──` 输出。**每一次调用都必带**（模型 / HTTP / 业务函数 / **工具函数都要**），含当时变量值。打在「开始」那条上即可，结束条不必重复同一段源码
- **LLM / HTTP 响应打整个对象**：不要挑字段（id / choices / usage / SDK 自带字段 / status / headers / body），完整放进 `data`
- `data` 建议键名用中文，方便人读：`入参` / `返回值` / `字段释义` / `第几轮` / `本轮为什么是这些参数`
...
**业务代码打日志原则（详细优先）**
- **不怕多**：函数里每一段能单独说清的步骤都打；宁可一行业务代码旁边一行日志，不要「跑完才打一条汇总」
- 与 [§5.3.11 `// ①②③` 注解](../AGENTS.md#53-小节-Demo-完整版前后端--react--koa2026-09-02-维护模式起生效) 对齐：在该行附近插 `logger.info(...)`，不打断主流程
- `debug` 调试细节；`info` 主流程节点；`warn` 异常但可走通；`error` 失败
- **不**因为怕啰嗦省略

**闭环：每次调用只打这五件套（缺一头 = 没打完）**

一次「调用」= 业务函数 / 方法 / 工具 / 调模型 / 调外部 HTTP。读日志必须成套、顺序固定：

| 顺序 | 所有调用都要？ | 打什么 |
| ---- | -------------- | ------ |
| 1 开始 | 要 | 「开始调用 {名字}」。`explain`：**为什么调它** + **整次请求现在走到哪** |
| 2 入参 | 要 | 完整参数（不要只打个数）。可和「开始」写在同一条 `data.入参` |
| 3 源代码 | 要（**工具也要**） | `__code`：就是这一次调用的那几行（含当时变量） |
| 4 返回值 | 要 | 函数返回值，或模型/HTTP 的完整响应。`explain` 可顺带说返回值从哪一层取到（`await` / `choices[0]`），**不要另起一条「发出」「接到」** |
| 5 结束 | 要 | 「结束调用 {名字}」。失败也要结束（`error` + 错误对象当返回值） |

**禁止**再打「发出 / 已交给提供商 / 请求送出」这类中间态：它和「开始」在读者眼里是同一件事，只会把五件套打乱。

嵌套：外层开始之后、外层结束之前，完整插入内层五件套。

**两档详细度（五件套永远不省）**

| 档 | 哪些 | 五件套之外再打什么 |
| -- | ---- | ------------------ |
| **核心档** | 调模型；调外部 HTTP；本条主路径；知识点封装函数 | 函数体逐步（里面每个子调用仍是自己的五件套）；返回值上加 `字段释义` |
| **工具档** | 普通工具 / helper | 函数体**一句**说完即可；五件套（含 `__code`）仍要 |

禁止：「工具所以不打 / 不带源码」。

**核心档示例（调模型；HTTP 把名字换成 URL 即可。注意：没有「发出」）**

```ts
logger.info(
  "调用模型-开始",
  "开始调用对话补全",
  "当前：Agent 循环第 1 轮。为什么调：用户刚问库存，messages 里还没有 tool 结果，必须先问模型要不要调工具。",
...
logger.info(
  "调用模型-结束",
  "结束调用对话补全",
  "当前：第 1 轮模型已经回来。返回值就是 await 得到的整段 response。finish_reason=tool_calls，所以下一步去调 queryStock，还不是给用户的最终话。",
  { 返回值: response, 字段释义: { "choices[0].finish_reason": "tool_calls = 要调工具", "choices[0].message.tool_calls": "要执行的函数名和参数", usage: "这次花了多少 token" } },
);
```

**工具档示例（函数体一句带过，五件套含源码）**

```ts
logger.info(
  "查询库存-开始",
  "开始调用 queryStock",
  "当前：第 1 轮模型已经要求调工具。为什么调：tool_calls 里 name=queryStock、sku=SKU-88，必须先拿到件数才能进第 2 轮。",
...
logger.info(
  "查询库存-结束",
  "结束调用 queryStock",
  "当前：工具跑完。返回值 available=12，下一拍会把它当 role=tool 塞进 messages。",
  { 返回值 },
);
```
...
1. 第几轮开始（`第 3 轮 / 共 5 轮`，无限循环写已跑圈数）
2. 本轮参数是什么
3. **为什么是这些参数**（上一轮返回怎么喂进本轮）
4. 本轮里的调用仍走上面闭环
5. 本轮结果是什么
6. 第几轮结束
...
- 每个调用是否都能看到五件套：开始、入参、源码、返回值、结束？
- 工具函数是否也有 `__code`？
- 有没有多余的「发出 / 接到」把闭环打乱？
- 每条 `explain` 是否同时写了「为什么这么调」和「当前到哪一步」？
- 核心路径的返回值是否有字段人话？循环是否每一圈打满？
- 只看日志、不看代码，能否按时间顺序把这次请求讲完？

### 5.4 目标 ↔ 代码整合闸门（两段式）·新

2026-09-04 维护模式起生效。

**问题**：旧闸门（[§5.2](#52-小节-demo)「`coach complete` 闸门」+ [§5.3.14](#5314-demo-子节拆分动态引导由浅入深新) 「闸门」）只查「demo 跑通没 / §5.3.2 6 项齐没 / check-demo 过没 / 锁没锁」。**这些都只是「代码存在」**——但「代码存在」≠「本条要能讲清」。MD 写满了「含并行调用」+「Promise.all」，代码里却用同步 `.map(...)`，跑通 + 6 项齐 + check-demo 过 + 双 step 锁定，**闸门全过、可勾 ✅**——可「并行调用」这个目标点根本**没在代码里真实跑过**，合上笔记讲不清「真实跑过的并行时序长什么样」。
...
#### 闸门逻辑（2026-09-05 · 对应 [03-progress 闸门 3/4](./03-progress.md#闸门-3--54-教练自查逐项--禁止抽样)）

**硬约定**：① MD 进闸门 3/4 清单的项 → 代码必须**已实现**；② **状态栏只许「已实现 / 未实现」**（**禁止** `⚠` / `❌` / `✅` /「警告但阻塞」/「单向缺口不阻塞」/「❌ 但不阻塞」——旧表出现「❌ · 不阻塞」一律按**未实现**计）；③ 未实现 = 阻塞；缺口只许 **补代码** 或 **拆条**，**禁止**问「接受缺口写进 MD」。

```
coach complete：闸门 1 → 2 → 3 → 4
闸门 3 = §5.4.A + §5.4.B（教练自查）
  ├─ 所有项 已实现 → 闸门 3 过 → 进闸门 4
  └─ 任一 未实现 → 闸门 3 不过 → STOP
闸门 4 = 独立 subagent 再查同一清单
  ├─ 全部 已实现 → 可勾 ✅
  └─ 任一 未实现 → STOP
```
...
| 时机 | 必打？ |
|---|---|
| `coach status` | **不打**——status 只报进度 + 分流问句（[03-progress `coach status`](./03-progress.md#coach-status只报进度--一句分流)） |
| `coach start`（外部条） | **必打**——即便看着齐也主动扫一遍 |
| `coach complete`（外部条勾 ✅ 前） | **必打**——不过不准勾 |
...
- §5.4.A 未实现 = 「拒绝当前条 ✅」；不是「自动建 step-(N+1) 补」——补不补、怎么补由学习者在 **补代码 / 拆条** 里选（参 [§5.3.14 交互检查点协议](05-demo.md#交互检查点协议每步之间必走) + [03-progress 闸门 3/4](./03-progress.md#闸门-3--54-教练自查逐项--禁止抽样)）
- 用 `⚠` / `❌` / `✅`（闸门状态栏）/「警告但阻塞」/「单向缺口不阻塞」/「❌ 但不阻塞」/「注释算已实现」交差
- 问「接受缺口写进 MD / 已知缺口节」
...
| 闸门 | 解决什么 |
|---|---|
| **[闸门 1 · §7.2](./03-progress.md#闸门-1--72-md-知识覆盖)** | MD 是否沉完、能否讲清「本条要能讲清」 |
| **[闸门 2 · §5.2](./03-progress.md#闸门-2--52-demo-完整度)** | demo 是否存在 / 跑通 / §5.3.2 / check-demo / 已锁定 |
| **[闸门 3 · §5.4 教练自查](./03-progress.md#闸门-3--54-教练自查逐项--禁止抽样)** | 「本条要能讲清」+ MD 讲点 → 代码逐项已实现 |
| **[闸门 4 · 独立验证](./03-progress.md#闸门-4--独立代码覆盖验证)** | 禁止信 MD 自报；subagent 再查同一清单 |
| **[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)** | 模块复盘行专用，只过 MD 闸门 |

> **Demo 覆盖 = 闸门 3（§5.4.A 正向 + §5.4.B 反向）+ 闸门 4 都过**。A 看「**目标点有没有**」，B 看「**MD 讲到的有没有**」。两层都不漏 + 独立验证通过 = Demo 真正完整。
