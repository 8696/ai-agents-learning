...
| **2** | 本节 §5.2：Demo 行不是「还没判断要不要写」；可运行须锁定 step + yarn + `check-demo` + [§5.3.2](#532-完整版--必做的-6-项替代-52-最低标准) 六项齐 + 端口三处一致 | STOP |
...
**`coach complete` 过关检查补一句**：该落可运行且命中本条「必须拆」时，`server.ts` 仍须是 yarn 入口；业务在 `routes/` + 分层 `lib/`；`public/` 须有场景页（`pages/` 或等价），共享 UI 按 `components/` + `utils/` 分类，不能只有一个塞满的 `index.html` / 一个巨型 `ui.js`。
