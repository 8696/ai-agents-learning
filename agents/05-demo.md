...
| 状态 | 子节 | 入口 | 端口 | 本子节教学点 |
|------|------|------|------|--------------|
| 🔄 | step-1 | `yarn app:05-01-...-step-1` | `50501` | tool_call 协议结构（1 函数 mock） |
```
...
**`coach next` 闸门补一句**：本条可运行时，`apps/{模块文件夹}/` 下至少要有 1 个 `{小节文件夹}-step-{N}/` **已锁定（✅）+ check-demo 过**。未锁定的 step-N 不算完成。
