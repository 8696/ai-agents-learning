...
外部条勾 ✅ 前闸门是**三件事**（顺序硬定：先 §7.2，再 Demo / §5.4；§7.2 不过 → **STOP**，不进入 Demo / §5.4 检查，[详 agents/03-progress.md `coach complete` 段闸门一](./03-progress.md#闸门一--7-沉淀--小节进度对齐md-知识覆盖--2026-09-04-维护模式起生效)）——

1. 小节 MD 已过 [§7.2](../AGENTS.md#72-沉淀--小节进度对齐) —— **闸门一 · 先查**（教练自查：4 条底线 / §7.0 八条 / §6.3 变体覆盖；不过直接 STOP，不准勾 ✅）
...
3. **[§5.4](#54-目标--代码整合闸门两段式新) 目标 ↔ 代码整合闸门**（5.4.A / 5.4.B）—— **闸门二 · 后查**，任意一段不过，**不准勾**
...
| **[§5.2 `coach complete` 闸门](#52-小节-demo)** | demo 是否存在 / 跑通 / §5.3.2 6 项齐 / check-demo 过 / 已锁定 |
| **§5.4（新增）** | 「本条要能讲清」的目标 ↔ 代码 ↔ 文档三方对齐 |
| **[§5.4.A 正向](#54a-目标--代码覆盖正向)** | 「本条要能讲清」原文 → 拆目标点（动词 + 可观察行为）→ 代码里**逐条**找证据 |
| **[§5.4.B 反向](#54b-文档--代码对齐反向)** | MD 各节（是什么 / 为什么 / 易混点 / 例子 / 踩坑 / 需求清单）→ 代码里**逐条**找证据；例子 / 踩坑 / 易混点必须映射代码 |
| **[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)** | 模块复盘行专用，只过 MD 闸门 |

> **Demo 覆盖 = §5.4.A 正向 + §5.4.B 反向，两段都过才算 Demo 覆盖过关**。A 看「**目标点有没有**」（从「本条要能讲清」拆），B 看「**MD 讲到的有没有**」（从 MD 拆）。**只看 A 漏 B 的"MD 写满 vs 代码没实现"**，**只看 B 漏 A 的"目标点本身没在 MD 里写"**。两层都不漏 = Demo 真正完整。
