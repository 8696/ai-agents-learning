...
嵌套：外层开始之后、外层结束之前插入内层五件套；内层 `scope` 多一根 `│`。

**流式（SSE / stream）**：只打**一次**「调用模型开始」；中间 token/chunk **不要**套五件套（最多 `debug` 一句「又来了一块」）；「调用模型结束」打**拼好的完整返回值**（和最终给页面的那份一致）+ `耗时ms`。

**两档详细度（五件套永远不省）**

| 档 | 哪些 | 五件套之外再打什么 |
| -- | ---- | ------------------ |
| **核心档** | 真正出网的模型/HTTP；本条主路径；知识点封装函数 | 函数体逐步（子调用各自五件套）；返回值上加**教学相关** `字段释义` |
| **工具档** | 普通工具 / helper | 函数体**一句**；五件套（含 `__code`）仍要 |
