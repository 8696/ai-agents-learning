...
#### `coach complete` 闸门（对外条目：闸门 1→2→3）
...
| **3** | [§5.4](#54-目标--代码整合闸门两段式新).A+.B 独立 subagent：抽清单 + 逐项核对 | STOP；补代码或拆条 |

「锁定 + §5.3.2 + check-demo」= **闸门 2**；目标↔代码 = **闸门 3**。复盘行不打闸门 2/3（[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)）。
...
#### 闸门逻辑（2026-09-08 · 对应 [03-progress 闸门 3](./03-progress.md#闸门-3--54-独立代码覆盖验证抽清单--逐项核对)；合并原教练自查 + subagent 复扫）

**硬约定**：① MD 进闸门 3 清单的项 → 代码必须**已实现**；② **状态栏只许「已实现 / 未实现」**（**禁止** `⚠` / `❌` / `✅` /「警告但阻塞」/「单向缺口不阻塞」/「❌ 但不阻塞」——旧表出现「❌ · 不阻塞」一律按**未实现**计）；③ 未实现 = 阻塞；缺口只许 **补代码** 或 **拆条**，**禁止**问「接受缺口写进 MD」；④ **抽清单 + 核对**都由独立 subagent **一次做完**（禁止教练自查后再派 subagent 复扫同一清单）。

```
coach complete：闸门 1 → 2 → 3
闸门 3 = 独立 subagent
...
- §5.4.A 未实现 = 「拒绝当前条 ✅」；不是「自动建 step-(N+1) 补」——补不补、怎么补由学习者在 **补代码 / 拆条** 里选（参 [§5.3.14 交互检查点协议](05-demo.md#交互检查点协议每步之间必走) + [03-progress 闸门 3](./03-progress.md#闸门-3--54-独立代码覆盖验证抽清单--逐项核对)）
...
| **[闸门 3 · §5.4 独立验证](./03-progress.md#闸门-3--54-独立代码覆盖验证抽清单--逐项核对)** | 从 MD 抽清单 + 对代码逐项已实现（独立 subagent 一次做完；禁教练双扫） |
| **[§7.3](../AGENTS.md#73-模块复盘进度表最后一行)** | 模块复盘行专用，只过 MD 闸门 |

> **Demo 覆盖 = 闸门 3 过**（§5.4.A 正向 + §5.4.B 反向，同闸内由 subagent 抽清单并核对）。A 看「**目标点有没有**」，B 看「**MD 讲到的有没有**」。两层都不漏 = Demo 真正完整。
