...
> **不会自动注入。** 学习者说「沉淀文档 / 沉淀 / 写回当前条」、或 `coach next` 要验收小节 MD 时必须 Read。
...
| **`coach next` 触发勾 ✅** | 表行数 ≥ 1；至少 1 行 ✅；该 step-N `node scripts/check-demo.cjs` 过 |
...
`coach next` 勾本条前：表行数 ≥ 1；至少 1 行 ✅（锁定过）；该 step-N `node scripts/check-demo.cjs` 过；MD 已沉淀。**不**要求所有未来 `step-(N+1)` 完成，因为 N 动态。
...
**与 `coach next` 的分工**：`沉淀文档` = 只写/更新小节 MD；沉淀交付后立刻打判断块（外部条 [§5.2](../AGENTS.md#52-小节-demo)；复盘行不打 Demo 判断块，走 [§7.3](../AGENTS.md#73-模块复盘进度表最后一行)）。该落代码则当场做（学习者说「这次不要」除外）。`coach next` = MD 过关 + 判断已过闸门 + 勾 ✅、填「我的链接」；落了代码则该子文件夹 `README.md` 必须跟上。
...
`coach next` 勾 ✅ 前：该 MD 已有实质知识（`状态：已沉淀`），能转述「本条要能讲清」列，机制/易混/例子没漏，已出现的追问没丢；**外部条还须 Demo 行不是 `未判`**（[§5.2](../AGENTS.md#52-小节-demo)）。**§7.2 只解决「MD 够厚」；目标 ↔ 代码整合还须走 [agents/05-demo.md §5.4](../agents/05-demo.md#54-目标--代码整合闸门两段式新)（两段闸门：5.4.A 目标→代码覆盖 / 5.4.B 文档→代码对齐）——任意一段不过不准勾 ✅**。
