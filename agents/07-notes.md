...
#### 2.7 Demo 子节进度块（外部可运行条 · §5.3.14）

外部条结论是「可运行」，且 [AGENTS.md §5.2](../AGENTS.md#52-小节-demo) Demo 判断块选了「子节拆分 N>1」时，必须在该条 MD 加 `## Demo 子节进度` 块（位置：紧跟该条 MD 的「是什么 / 机制」等教材节之后，「过关自检」之前）。模板见 [agents/05-demo.md §5.3.14](05-demo.md#5314-demo-子节拆分增量构建-新)。

**写入时机**：

| 时机 | 怎么做 |
| ---- | ------ |
| **首次沉淀时** | Demo 判断块已落 → 写 MD 时一并加 `## Demo 子节进度` 块；所有 step-N 标 ⬜。 |
| **每个 step-N 落地** | 改该行状态 ⬜ → 🔄 → ✅；同步确认 `apps/.../step-N/` 存在 + `yarn app:...-step-N` 在 `apps/package.json` + `node scripts/check-demo.cjs apps/.../step-N` 过。 |
| **N 改判（拆成更多 / 合并回少）** | 回头改 [AGENTS.md §5.2](../AGENTS.md#52-小节-demo) Demo 判断块 + 改本表行数；新增 step-N 标 ⬜。 |

**禁止**：

- 漏写「Demo 子节进度」块（`N>1` 时）；教练过闸门时检查不到该块 → 不准勾
- 状态推进不写真实情况（标 ✅ 但文件夹不存在 / check-demo 不过）
- 把 step-N 顺序写错（数字顺序 = 学习顺序，不是字母序）

`coach next` 勾本条前：所有 step-N `✅` + 每个都过 `check-demo` + 本表行数 = Demo 判断块「子节拆分 N」。
