...
`coach complete` 勾 ✅ 前：该 MD 已有实质知识（`状态：已沉淀`），能转述「本条要能讲清」列，机制/易混/例子没漏，已出现的追问没丢；**外部条还须 Demo 行不是 `未判`**（[§5.2](../AGENTS.md#52-小节-demo)）。**§7.2 只解决「MD 够厚」；目标 ↔ 代码整合还须走 [agents/05-demo.md §5.4](../agents/05-demo.md#54-目标--代码整合闸门两段式新)（两段闸门：5.4.A 目标→代码覆盖 / 5.4.B 文档→代码对齐）+ **`coach complete` 独立代码覆盖验证**（[agents/03-progress.md `coach complete` 段](../agents/03-progress.md#coach-complete-勾前必报)）—— 任意一段不过不准勾 ✅**。
