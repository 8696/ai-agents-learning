...
- **症状**：学习者问「卡、x 张卡是什么意思？」——以为是库 / 论文里的对象
- **触发**：讲检索增强生成（RAG）时，把切块（Chunk）叫成「卡 / 卡片 / 张卡 / 6 张卡」
- **根因**：目录卡的生活比喻被当成计量单位反复用，听起来像术语；量词「张」是卡片的，切块该用「个 / 段」
...
### P-026  ·  coach complete 落地后漏改 §5.4 表（A/B 小结仍写「未实现」）

- **症状**：小节文档 `docs/学习模块/{模块}/{小节}.md` 的「§5.4 目标 ↔ 代码整合打钩前检查」节仍写「**A 段小结：不过。未实现 N 条**」「**B 段小结：不过。代码尚未写出**」，但本节代码已写出、§5.4 独立 subagent 报告 32/32 已实现、进度表已 ✅；MD 文字 ↔ 代码 ↔ 进度表三方对不上
- **触发**：写 step-N 落地 / `coach complete` 在进度表打钩那一刻，没回头把 §5.4.A/B 那张**首次写入预列**的表改写（表是「代码尚未写出」时留的草稿；落地后必须按 subagent 真结论逐行改成「已实现」+ 文件:行号证据 + 小结改成「过」）
- **根因**：把「subagent 报告已实现」当结论，没意识到 §5.4 那一节是 MD 的一部分、跟正文一样要按 [agents/07-notes.md §5.4.B 增量更新规则](../agents/07-notes.md#54b-增量更新时重跑打钩前检查写入小节文档差异块必含代码对齐项) 重跑；首次写入预列 ≠ 落地结论
- **修复**：`coach complete` 收口时**必走第四件事**（除 1→2→3→4 全过之外）：把 §5.4.A/B 整段按 subagent 真结论改写——每行状态 / 证据 / 小结 + 文首跑日期 + 业务换壳说明（如有）。跑日期写「YYYY-MM-DD 教练自跑 + §5.4 独立 subagent 报告 ID」，**禁止**只写「YYYY-MM-DD 预列」那种陈年日期
