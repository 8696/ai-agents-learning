...
唯一 Git 仓库，根目录放 `docs/` + `apps/`（`apps/` 是**唯一代码落点**：模块 00 mini-app + 每条外部小节的最小可运行 Demo + 每模块一行的**模块小 APP**）。学完模块 23 后若要作品集，再从零建独立项目，详见 [§5](#5-demo-落点) / [§5.4](#54-模块小-app本地产出行)。
...
2. 查 §4 得代码落点。外部节奏允许：[§5.2](#52-小节-demo) 落本条 Demo。本地产出行是 **模块小 APP**（[§5.4](#54-模块小-app本地产出行)），不是再讲一节新概念，也不是从零灌本模块全部代码（外部条 Demo 仍按条先落）。
3. 核对 `apps/` 实际子文件夹与 §4 表的「代码落点」是否一致；不一致提示。所有代码都直接落在对应小节；模块 00 mini-app 只承载模块 00 的三入口 + 其本地产出，不要把后面模块灌进去。
...
动作：新建 | 不建目录 | 外部（可 Demo） | 本地产出（模块小 APP）
```
...
- **本地节奏** → 按 [§5.4](#54-模块小-app本地产出行) 做**本模块小 APP**：对照该 README「动手产出 / 验收」写成「打开 APP 后能做什么」→ 落到 `apps/{模块}/{NN}-本地产出/`（模块 00 用已有 mini-app，不另建文件夹）→ 写入 `{NN}-本地产出.md`（串了哪些能力、数据怎么走、取舍）。禁止 import 其它小节。禁止再开一套三入口。
...
下一步选哪个？
  ...
  3) 直接勾：走 `coach next`（前提：MD 满足 §7.2；外部条 Demo 已按 §5.2 判断；本地产出已按 §5.4 闸门）
...
模块 00 的代码落点是 `apps/00-环境准备/01-mini-app/`（模块 00 mini-app，三入口：CLI-A / CLI-B / HTTP+SSE；**同时是模块 00 的本地产出小 APP**）。其余模块：外部小节按 [§5.2](#52-小节-demo) 落到 `apps/{模块文件夹}/{小节文件夹}/`；本地产出按 [§5.4](#54-模块小-app本地产出行) 落到 `apps/{模块文件夹}/{NN}-本地产出/`。代码落点统一在 `apps/`；学完模块 23 后若要作品集，再从零建独立项目（模块小 APP 留在 `apps/` 当学习产物，不是作品集）。
...
`apps/` 是本仓库**唯一**的代码落点：模块 00 mini-app（三入口，兼模块 00 本地产出）+ 每条外部小节的最小可运行 Demo + 每模块一行的模块小 APP（[§5.4](#54-模块小-app本地产出行)）。学完模块 23 后若要作品集，从零建独立项目。
...
| **小节 Demo** | `apps/{模块文件夹}/{小节文件夹}/` | 每条外部小节的最小可运行 Demo | **调 API / 必须看页** → §5.3：`server.ts` + `public/index.html` + `README.md`。**只在终端算** → `index.ts` + `README.md`。不 import 其它小节、不 import 模块小 APP |
| **模块小 APP** | `apps/{模块文件夹}/{NN}-本地产出/`（NN = 进度表最后一行两位） | **一个**完整应用；功能点覆盖本模块全部外部条 | 默认 §5.3；禁止拼盘、禁止 import 其它小节。细则 [§5.4](#54-模块小-app本地产出行) |
...
| 「本地产出」行 | **不走三选一**。结论固定为 **可运行（模块小 APP）**，按 [§5.4](#54-模块小-app本地产出行) 落 `apps/{模块}/{NN}-本地产出/`（模块 00 用已有 mini-app）。禁止用「跳过判断 / 只写笔记 / 补缺口」交差 |
...
禁止为凑而建空文件夹。调 API 的可运行 Demo **走 §5.3 四项**，禁止用「happy path + 一个行为然后关进程」交差。不要把外部条 Demo 做成第二份模块 00 三入口。只在终端算的 Demo 才用 §5.2 的 CLI `index.ts`。模块小 APP 见 [§5.4](#54-模块小-app本地产出行)。
...
**本地产出行勾 ✅ 前**另见 [§5.4 闸门](#54-模块小-app本地产出行)。
...
| **模块小 APP**（本地产出行） | 默认 **§5.3**（[§5.4](#54-模块小-app本地产出行)）；仅当本模块验收明确不要求看页时才 CLI |
...
**生效：2026-09-02 维护模式。学习者原话（以此为准）：** 本地产出要的是**一个单独完整的应用**，不是拼盘；**这个应用里面用到的功能点，要覆盖整个模块**。

按条 Demo 仍先落、互不 import。本地产出是另起的**一份产品**，本模块已学能力在产品路径**里面**干活。

| 必须 | 禁止 |
| ---- | ---- |
| **一个**应用、一套界面、一条主任务（打开像在用产品） | 把各条 Demo 的按钮/对照/故障开关摆在同一页（拼盘） |
| 本模块每条外部课的功能点，都能在这个应用的路径里指出来「在哪一步用上了」 | 只覆盖其中几条；其余用导航盘补 |
| 纯概念条（如 CoT 思想、禁止现在写 Agent Loop）→ 产品里遵守这条约束，不做成假功能按钮 | 为凑「全覆盖」去实现后面模块的 Loop / RAG / Tool |
| 默认 §5.3 四项 + `apps/{模块}/{NN}-本地产出/` + `yarn app:{MM}-{NN}-…` | import 其它小节、iframe 拼盘、再开三入口 |

**像应用 vs 像拼盘（判错就返工）：**

- 像应用：豆谷客服中台贴一封来信 → 一张工单（路由、抽取、Few-shot 分类、情感、转述都在这一次处理里）。
- 像拼盘：同一页「数 Token / 跑 Embedding / 对照 Few-shot / 角色冲突 / 故意 429」。

按条对照、A/B、故障开关属于**小节 Demo**，不要当本地产出首页。

**覆盖怎么验收：** `{NN}-本地产出.md` 用一张表：模块进度里每一条外部课 → 产品里哪一步用到了。对不上 = 没做完。概念条写「产品里故意不做什么」。

**模块 00：** 单独完整应用 = 已有 `01-mini-app`（聊天发一条、流式回来）。不另建 `04-本地产出/`。`01-API-Key-计费` 仍是按条 Demo。

**已 ✅ 回头补：** 点名才改。01 / 02 若仍是实验室拼盘，点名后按「一个应用 + 全模块功能点」返工。

**`coach start`（本地）：** 先定这个应用在办哪一件事，再列外部条如何嵌进这条路径（不是列按钮清单）。说写就写。

**`coach next` 闸门（本地产出行）：**

1. `{NN}-本地产出.md` 已过 [§7.2](#72-沉淀--小节进度对齐)，含「外部条 → 产品哪一步」覆盖表
2. 打开像一个应用，不像拼盘
3. 模块 00：mini-app 已建（不验证能跑）。其余：文件夹 + `app:{MM}-{NN}-…` + HTTP 则 §5.3 四项 + 打印地址
4. 该子文件夹 `README.md` 跟上；模块 README 验收能在这个应用里对上
...
| `coach start` | 要学当前条、不知道从哪下手 | 先打五行。外部：**默认**按 §6.2 讲完 + **本条产出预告**（要不要写 Demo；默认先不落，**说写就写**）。出门包仅当学习者说「出门包 / 我要出门学」（[§6.1](#61-外部学习出门包点名才出)）。本地：按 [§5.4](#54-模块小-app本地产出行) 讲模块小 APP 要串什么、打开能做什么 + 产出预告。**禁止跳模块** |
| `coach next` | 当前条达标，要进下一条 | **只推进当前条目**。先确认该行重点链接的小节 MD 已按 [§7.2](#72-沉淀--小节进度对齐) 写过、通过 [§7.0 结果验收](#70-写回扩写任意终端)，且 **过关自检**覆盖「本条要能讲清」列。空壳或只有提纲 → 按结果验收写入（只减不加），问去掉哪条，**不准勾**。不要因为「行数多」去压缩学习者追问进来的内容。**外部条还须** [§5.2](#52-小节-demo) Demo 判断。外部：勾小节进度 + 「我的链接」只填 URL 或 `暂无链接`（见 [§7.1](#71-我的链接列)）。本条若落过代码，该子文件夹 `README.md` 必须跟上。本地：走 [§5.4 闸门](#54-模块小-app本地产出行) → 勾「本地产出」和学习总览「本地」。 |
...
| 把笔记落进该条（不勾进度） | 「**沉淀文档**」「沉淀」「写回当前条」—— 首次写满空模板，已有内容则 [差异对照后合并](#25-差异对照仅增量更新写文件前必输出)；或「去掉沉淀里的 X」「把刚才追问的 Y 补进『我追问过的』」、贴外部 Agent 对话/笔记。沉淀交付后助手须打 [§5.2](#52-小节-demo) Demo 判断块（与 start 预告一致则写「与 start 预告一致」；本地产出行打 §5.4：结论固定为模块小 APP） |
...
```text
本条产出预告
- 本行是**一个单独完整的应用**（§5.4），不是拼盘，也不是第二份三入口 mini-app。
- Demo：可运行（模块小 APP）
  你学完要：落 apps/{模块}/{NN}-本地产出/（模块 00 用已有 mini-app），入口 yarn app:{模块两位}-{NN}-{短名}
- 这个应用在办：{一句话主任务}
- 模块每条外部课的功能点如何用上：{各一条；概念条写「产品里故意不做什么」}
- 禁止：按条按钮仪表盘 / import 其它小节 / 灌未学模块
```
...
**与 `coach next` 的分工**：`沉淀文档` = 只写/更新小节 MD；沉淀交付后立刻打判断块（外部条 [§5.2](#52-小节-demo)；本地产出 [§5.4](#54-模块小-app本地产出行)）。该落代码则当场做（学习者说「这次不要」除外）。`coach next` = MD 过关 + 判断已过闸门 + 勾 ✅、填「我的链接」；落了代码则该子文件夹 `README.md` 必须跟上。

---

## 7. 进度与沉淀

- 条目勾选只改该模块文件夹 `README.md` 的 **小节进度**；整模块外部/本地改 [docs/06-学习总览.md](docs/06-学习总览.md)。勾本地前必须写好该模块 **`{NN}-本地产出.md`**，且模块小 APP 已过 [§5.4 闸门](#54-模块小-app本地产出行)（已 ✅ 且本规则生效前勾过的不回头补，除非点名）。
...
本地产出额外要能讲清：模块小 APP **串了哪些已学能力**、打开后能做什么、和按条 Demo 差在哪；对照该模块 README **验收**。怎么跑对行看 `apps/{模块}/{NN}-本地产出/README.md`（模块 00 看 mini-app README），不要把行号写死进小节。

#### 写入顺序（助手每次落盘必走）

1. 把本对话 + 贴回材料写成**能转述的知识**，核心对象都有通俗例子。对照「本条要能讲清」扫一遍，漏了就补。
2. 学习者问过的不要丢。
3. 用学习者能说出口的几句对照「本条要能讲清」（本地产出对照 README 验收 + APP 打开后能做什么）。
