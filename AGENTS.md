...
外部条勾 ✅ 前：小节 MD 已过 [§7.2](#72-沉淀--小节进度对齐) **且** Demo 行不是 `未判`。可运行但 `apps/{模块文件夹}/` 下无任何 `{小节文件夹}-step-N` 被锁定（✅）/ 无对应 `yarn app:…-step-N` / 锁定时 `node scripts/check-demo.cjs apps/{模块文件夹}/{小节文件夹}-step-N` 不过 / §5.3.2 6 项不齐 → **不准勾**。锁定是学习者主动决定 + §5.3.2 + check-demo 三件事同时满足的那一刻。
