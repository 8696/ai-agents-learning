...
- **给人看、全中文、怕多不怕少**；文件里**每条日志前空一行**（logger 自动加）
- **`msg`**：`[请求#xxxx] 调用函数|模型|HTTP|循环开始|结束：名字`；失败加 `（失败）`；禁止只写「xxx开始」
...
- **给人看**：全中文；每条日志前空一行；`msg` 带 `[请求#]` + 调用函数/模型/HTTP/循环开始|结束；`explain` 写为什么打、当前到哪
