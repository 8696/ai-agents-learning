...
> **项目当前地图**：最简流式 UI → `apps/01-chatgpt-mini`；产品级打磨 → `apps/05-coding-agent`。禁止第 6 个 app。
...
**动手产出**：回填两个现有项目，**不要新建第 6 个 app**。① ChatGPT Mini：最简流式 Chat UI。② Coding Agent：产品级 Agent Steps / HITL / 错误 UI。
...
1. `apps/01-chatgpt-mini`：最简流式 Chat 页（逐字出现、能取消）
2. `apps/05-coding-agent`：Agent Steps / HITL / 错误 UI / 尽量不闪的 Markdown
3. 引用、上传、移动端、骨架屏只打磨进 `05`
