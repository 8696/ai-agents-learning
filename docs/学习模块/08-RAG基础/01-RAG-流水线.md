...
> 本条由 Coach 按 2026-09-11 本对话 `coach start` 详解首次沉淀。同日经追问与笔记核对、以及 step-1 跑通后的嵌入方言 / 换家重建 / 检索实现增量更新。学习者只减不加。进度表仍 ⬜，勾 ✅ 只走 `coach complete`。
...
7. **库里没有就说不知道**  
   场景：问「今晚食堂菜单」或清空库再问退货。目标：零命中 / 低分时弃权。知识点：变体 11、8。验收：回答不含编造的菜名 / 政策条款；页面能看到「无可用材料」或分数低于阈值。
...
| 状态 | 子节 | 本子节教学点 |
| ---- | ---- | ------------ |
| 🔄 | step-1 | 一份 refund.md：左边建库 / 查看库，右边提问。建库看见一书变多行；查看库把 SQLite 四件套打到页上（向量不能全是 0；智谱须 encoding_format=float）。提问看见前 K 条、分数、带来源的提示词和回答；连续提问不再跑加载/切块。MiniMax 嵌入走 HTTP（db/query），智谱/千问走 embeddings.create；换提供商须重建库。LanceDB 包已装，本步仍用 SQLite |

---

## §5.4 目标 ↔ 代码整合过关检查

跑过关检查日期：2026-09-11（step-1 已落后增量重扫；step-1 未锁定。状态只有已实现 / 未实现）
...
| A5 库里没有 / 低分时回答弃权，不编造 | 未实现 | 提示词写了不知道；无专门空库/低分验收按钮 |
...
| A11 页面说明 RAG 根因（喂模型没见过的知识），不是「长上下文优化版」 | 未实现 | `#core-takeaway` 写的是四件套，不是根因 |
...
**A 段小结**：不过（A5–A11 未实现）。step-1 已覆盖默画闭环 A1–A4、A12–A15。其余按双方确认再往后续 step 加。禁止问「接受缺口写进 MD」。
...
| RAG 存在的根因 = 喂模型它没见过的知识（不是省 token） | 页头未写根因 | 未实现 |
| 能点名出处（RAG 相对长上下文的独家优势） | 卡片带来源 | 已实现 |
| 换嵌入必须重建库 | 无自动检测；行为上必须重建 | 已实现（机制） |

**B 段小结**：不过。step-1 已对齐默画 + 四件套 + 嵌入方言 + 检索打分。缺口仍是 PDF / 失败 / 工具化 / 按文件删旧 / 弃权验收 / 根因文案。禁止问「接受缺口写进 MD」。
