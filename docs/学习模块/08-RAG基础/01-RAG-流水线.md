...
本仓库模块选型是 LanceDB。本步 Demo 先用已经装好的 SQLite 存向量库一行要存的四个字段（id / vector / text / 来源字段），因为 LanceDB 还要另下本机二进制，外网装不上会挡第一次上手用的。后来 `@lancedb/lancedb` 已经进了 `apps/package.json`，但 `vector-store.ts` **没有 import**，读写仍是 `better-sqlite3`。装上 ≠ 已经在检索。认的是「有一个专门存向量 + 原文 + 元数据的库」，不必先会某一家 API。本地库（SQLite / LanceDB）是货架；`create-embeddings.ts` 是去问嵌入模型怎么贴条码。点「查看库」只做 `SELECT`，不调模型。
...
3. **没有检索质量度量 = 答不准时修哪一块都判断不了**。下面「答准时三种修法」里 5 类症状，第一步要靠命中数 + 最高分判断。
...
**一本书建库后，在向量库里留下 N 行「向量库一行要存的四个字段」记录，不是整本书的一个大向量。**
...
**关键认知**：向量库一行要存的四个字段是**落地格式**，跟加载 / 切块 / 向量化是不同维度的概念。前两件（原文 + 向量）= **检索能力**；后两件（来源 + 位置）= **维护能力 + 可信度**。
...
**经验**：常见的是症状 2 或 3——库脏（行级）或提示词不够硬（不动库）。换嵌入模型、改切块策略，多半是早期选型或确认策略真错了才做，**不是答不准时的第一步**。
...
| 容易混成一句话 | 差在哪 | 判错会怎样 |
| -------------- | ------ | ---------- |
| RAG = 把文档丢给模型 | RAG 是「先检索再生成」。整篇塞进去叫把整篇塞进上下文窗口，会撞窗口、也没有分数 | 手册一厚就 400 / 截断，还检索不准 |
...
9. **文档更新后旧版不再被命中（按 source 整份删旧）**  
   场景：运营把 `refund.md` 从 v1.2 改成 v1.3，点了「重新入库」。目标：按文件整份删旧再整份写入。知识点：变体 12、向量库一行要存的四个字段、变体 13。验收：库里查不到任何 `source = "refund.md"` 且文本属于 v1.2 的行；没有把未改章节复制出第二份；重问「7 天无理由怎么退」，回答是 v1.3 的内容，不混 v1.2。
...
13. **查看库能看见向量库一行要存的四个字段，提问嵌入只刷这一句**  
...
| 「一本书库里到底存了什么，为什么能只改一章」 | 把「一本书」当成库里的一级实体；误以为默认按章节撕卡片 | 答在「数据怎么存」：一行是向量库一行要存的四个字段 `id` / `vector` / `text` / `metadata`。`id` 是主键，给库认行；运营按 `source` 筛。本条默认按文件整份删旧再整份写入。「能只改一章」是因为行独立且有标签，不是默认手术。 |
...
- 结论：可运行
- step-1 入口：一份售后 Markdown，点「建库」看见一书变多行（加载 / 切块 / 向量化），再点「提问」看见前 K 条 + 带来源的回答
- 锁定时机：学习者主动决定
...
| 状态 | 子节 | 本子节教学点 |
| ---- | ---- | ------------ |
| ✅ | step-1 | Markdown 按 ## 分段；PDF 按 3000 字分 chunk（凑合解，跨页段落会断）；文件名成为 source；koa-body multipart 上传；扫描版 PDF（正文为空）报错不入库。嵌入批量 10 条/批，单 chunk ≤ 3000 字。弃权阈值 + 四方式（Top-1/逐条过滤/平均分/最严）；`core-takeaway` 写根因 + 能点名出处 + 向量库一行要存的四个字段 + 提问边界。**2026-09-11 增量**：弃权四方式 + 为什么选 Top-；A5 弃权；A11 根因文案。**同日二次增量**：Markdown 上传。**同日三次增量**：PDF 上传。**同日四次增量**：嵌入批量 + 文本截断 |
...
| ✅ | step-4 | 答准时修法提示 UI（5 类症状 × 3 种修法）。底部 5 张症状卡：① 库里没这类知识（行级：补文档）/ ② 库脏（行级：按 source 先删后建）/ ③ 提示词不够硬（不动库：改 prompt）/ ④ 分数看起来低（不动库：先看原文 / 调阈值；明确写「不要换嵌入模型」）/ ⑤ 同义改写搜不到（不动库：query rewrite）。每张卡有「试试此症状」按钮：自动用预设问题跑一次 agent + 高亮对应卡。核心 takeaway = 答不准 ≠ 一律重跑整库。`diagnose-panel.js` + 症状 5 预设问题换成「寄东西大概要花多少钱」避开 embo-01 强语义捕获。2026-09-11 锁定 |
...
| 目标点 | 状态 | 证据 |
|---|---|---|
| A1 页面能按序看见 Load → Chunk → Embed → Retrieve → Generate，每步有中间产物 | 已实现 | step-1 建库三步 + 提问三步；查看库打出向量库一行要存的四个字段 |
...
| A11 页面说明 RAG 根因（喂模型没见过的知识），不是「长上下文优化版」 | 已实现 | `#core-takeaway` 列出根因（喂没见过的知识）+ 真正独有优势（能点名出处）+ 向量库一行要存的四个字段 + 提问边界 |
...
**B 段小结**：step-1 / 2 / 3 / 4 已对齐默画 + 向量库一行要存的四个字段 + 嵌入方言 + 检索打分 + 弃权 + 根因文案 + 上传入库（Markdown + PDF，按 source 行级维护）+ 检索做成工具 + 答准时修法提示（5 类症状矩阵）。唯一缺口 = PDF 命中卡片不显示页码（拆分到 step-5，按「拆走」决策）。独立 subagent 验证 108 项过 107。2026-09-11 勾 ✅ 第 1 条。禁止问「接受缺口写进 MD」。
