...
> **项目当前地图**：`apps/03-knowledge-agent/LEARNING.md`（学到再建）
...
| ⬜ | [**本地产出**](./05-本地产出.md) | 本页验收 + 学习沉淀（新建 `03-knowledge-agent`） | — | [沉淀](./05-本地产出.md) |
...
**动手产出**：Markdown/PDF → Chunk → Embedding → Vector DB → Retrieval → LLM 全流水线。检索封装成 Tool，由 **本项目（Knowledge Agent）自己的循环**决定要不要检索——不要 import `apps/02-tool-agent`。
...
> 新建 `apps/03-knowledge-agent`（先问「可以吗？」）。复制 `load-root-env.ts`。五个 app 互不 import。

1. Load → Chunk → Embed → LanceDB → Retrieve
2. 检索做成 **本项目内** 的 Tool，自带最小循环决定要不要检索
3. 「库里没有」必须说不知道，不要编
