...
1. **`README.md`** — **小节进度**（从上到下勾：先外部，最后一行**模块复盘**）+ **验收** + **动手落点**
2. **每小节一个 MD** — 对照学习要求的教学笔记，**由助手扩写**（你不手打）。验收看「有没有用、合上文件能否转述」，**没有标准范文，也不按行数**（细则 [AGENTS.md §7.0 结果验收](../../AGENTS.md#70-写回扩写任意终端)）。文件名 `{两位序号}-{短名}.md`。未学是空模板；写回细则 [AGENTS.md §7.0](../../AGENTS.md#70-写回扩写任意终端)、[§7.2](../../AGENTS.md#72-沉淀--小节进度对齐)。可多次追加。你只减不加。

「我的链接」列只填 URL 或 `暂无链接`；**来源**写进该条小节 MD，不写「留在本对话学」进链接列（[AGENTS.md §7.1](../../AGENTS.md#71-我的链接列)）。

代码落点统一在 `apps/`：模块 00 mini-app 在 `apps/00-环境准备/01-mini-app-step-1/`；其余外部小节按 [§5.2](../../AGENTS.md#52-小节-demo) 落最小可运行 Demo 到 `apps/{模块}/{小节}/`。**模块最后一行是「模块复盘」（[§7.3](../../AGENTS.md#73-模块复盘进度表最后一行)），只写 MD，不写代码、不新建目录。** 每个子文件夹只迭代 `README.md`（怎么跑 + 当前能做什么）一份，跟代码改写。**不要**改已经写好的小节 MD。各模块 README **动手落点**告诉按条 Demo 落哪个目录，整条路线过没过完看 [学习总览](../06-学习总览.md)。

**任意终端学概念 / 写回：** 标准在 [AGENTS.md §1](../../AGENTS.md#1-角色) / [§6.2](../../AGENTS.md#62-概念讲解任意终端--外部节奏) → [agents/06-teach.md](../../agents/06-teach.md) / [§7.0](../../AGENTS.md#70-写回扩写任意终端)。提问清单仅点名才给（[§6.1](../../AGENTS.md#61-出门自学用的提问清单点名才出) → [agents/06-outing.md](../../agents/06-outing.md)），产出预告见 [agents/03-progress.md](../../agents/03-progress.md#61-本条产出预告)。进度表「本条要能讲清」列管能不能勾 ✅。
...
1. **README.md**：小节进度、验收、动手落点（缺了就补，不要另起一套结构）
2. **与进度表一一对应的小节 MD**（含最后一条**模块复盘** `{NN}-模块复盘.md`；文件名带两位序号）

**时间量级**：外部一条大约 15–40 分钟；**模块复盘**按验收走，对照 README 逐条答 + 串成一条线，一次写完。
