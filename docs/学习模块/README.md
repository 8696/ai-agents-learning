...
代码落点统一在 `apps/`：模块 00 mini-app 在 `apps/00-环境准备/01-mini-app/`；其余外部小节按 [§5.2](../../AGENTS.md#52-小节-demo) 落最小可运行 Demo 到 `apps/{模块}/{小节}/`。每个子文件夹只迭代 `README.md`（怎么跑 + 当前能做什么）一份，跟代码改写。**不要**改已经写好的小节 MD。各模块 README **本地拆步**是收口对照，不是「最后一节才动手」。整条路线过没过完看 [学习总览](../06-学习总览.md)。
