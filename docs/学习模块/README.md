...
代码落点统一在 `apps/`：模块 00 mini-app 在 `apps/00-环境准备/01-mini-app-step-1/`；其余外部小节按 [§5.2](../../AGENTS.md#52-小节-demo) 落最小可运行 Demo 到 `apps/{模块}/{小节}/`。**模块最后一行是「模块复盘」（[§7.3](../../AGENTS.md#73-模块复盘进度表最后一行)），只写 MD，不落代码、不新建目录。** 每个子文件夹只迭代 `README.md`（怎么跑 + 当前能做什么）一份，跟代码改写。**不要**改已经写好的小节 MD。各模块 README **动手落点**告诉按条 Demo 落哪个目录，整条路线过没过完看 [学习总览](../06-学习总览.md)。

**任意终端学概念 / 写回：** 标准在 [AGENTS.md §1](../../AGENTS.md#1-角色) / [§6.2](../../AGENTS.md#62-概念讲解任意终端--外部节奏) → [agents/06-teach.md](../../agents/06-teach.md) / [§7.0](../../AGENTS.md#70-写回扩写任意终端)。出门包仅点名（[§6.1](../../AGENTS.md#61-外部学习出门包点名才出) → [agents/06-outing.md](../../agents/06-outing.md)），产出预告见 [agents/03-progress.md](../../agents/03-progress.md#61-本条产出预告)。进度表「本条要能讲清」列管能不能勾 ✅。
