...
**任意终端学概念 / 写回：** 出门包与详解同一套学习要求（[AGENTS.md §6.2](../../AGENTS.md#62-概念讲解任意终端--外部节奏)）。Cursor、Claude Code、Codex 打开本仓库写小节时，必须把材料**扩写**成 01～04 那一档（通俗、生活场景、每个核心对象有例子），禁止原样粘贴提纲（[AGENTS.md §7.0](../../AGENTS.md#70-写回扩写任意终端)）。进度表「本条要能讲清」列管能不能勾 ✅，不是笔记可以变短。
