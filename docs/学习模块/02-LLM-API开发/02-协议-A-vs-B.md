...
- **状态**：已沉淀
- **Demo**：已落 [demos/02-LLM-API开发/02-协议-A-vs-B/](../../../demos/02-LLM-API开发/02-协议-A-vs-B/)（5 个端点：/api/a /api/b /api/compare /api/think-compare /api/b-thinking-stream；用 MiniMax-M3 同 Key 跑双协议对照）
- **回填**：无（项目协议 B 入口 `apps/01-chatgpt-mini/src/index-anthropic.ts` 在模块 00 已超前放置；模块 02 本地拆步第 5 步「跑 yarn dev:anthropic 对照协议 A vs B（入口若已在则不必新装 SDK）」—— 这一刀**已落在模块 00**）
...
[Demos/02-LLM-API开发/03-adapter-demo](../../../demos/02-LLM-API开发/03-adapter-demo/) 把上面所有差异**收敛到一个统一接口**——业务代码从此不直接调 SDK。
...
[demos/02-LLM-API开发/03-adapter-demo](../../../demos/02-LLM-API开发/03-adapter-demo/) 是一个最小可运行的 adapter demo——业务代码完全不碰 SDK，只调 `sendMessage(opts)`。
...
```
demos/02-LLM-API开发/03-adapter-demo/
...
- **adapter demo（demos/02-LLM-API开发/03-adapter-demo/）—— 这是 adapter 的最小可运行实现**
