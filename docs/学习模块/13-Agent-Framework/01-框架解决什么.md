...
step-2 在同一份 `streamText` 出口下，把 OpenAI 协议和 Anthropic 协议两条路完整走通。**浏览器侧 0 差异**，**后端差异 = model.ts openai 分支按 provider 选 SDK（minimax 走 `createOpenAI` + 中间件；zhipu / qwen / deepseek 走 `createOpenAICompatible`）**——剩下 reasoning 怎么拆的差异，由所选 SDK 翻译层各自扛走。
...
| 思考字段位置 | `minimax` 用 ``…`` 文本标签；`zhipu` / `qwen` / `deepseek` 用 **`reasoning_content` 字段（OpenAI 官方协议里没有这个字段，是各网关自家扩展）**——所以拆法不同 | **各网关统一**（`content[type=thinking]`） |
...
| 中间件 / SDK | `minimax`：`createOpenAI` → `wrapLanguageModel` + `extractReasoningMiddleware({ tagName: "think" })`（拆 ``…`` 文本）；`zhipu` / `qwen` / `deepseek`：`createOpenAICompatible`（**换 SDK**，不是中间件——SDK 内部直接读 `delta.reasoning_content` 合成为 `reasoning-start/delta/end`） | 无中间件（SDK 内部把 `content[type=thinking]` 翻成 reasoning 段） |
| 模型 id | `modelA` | `modelB` |
...
### OpenAI 网关扩展字段 `reasoning_content`（2026-09-20 揭示）

四家网关里只有 `minimax` 走 OpenAI 官方协议的扩展——把思考写在 `content` 里、用 ``…`` 文本标签包起来；这种走法 **`@ai-sdk/openai` 配合 `extractReasoningMiddleware({ tagName: "think" })` 就能拆**。

`zhipu` / `qwen` / `deepseek` 走的是另一种：思考单独放在 `choices[0].delta.reasoning_content` 字段里——这是**各网关自家扩展的字段**，OpenAI 官方协议里没有；`@ai-sdk/openai` 的 `OpenAIChatLanguageModel.doStream` 默认**完全不读 `delta.reasoning_content`**（已经在 SDK 源码 `node_modules/@ai-sdk/openai/dist/index.js` L1458-1537 验证），所以你只换 SDK 不换写法，思考会跟正文混在一起、拆不出来。

解法是**换 SDK**：`@ai-sdk/openai-compatible` 是 Vercel AI SDK 官方给 OpenAI 兼容网关准备的 SDK，原生支持 `delta.reasoning_content`——SDK 内部在 `delta` 上读到 `reasoning_content` 字段后，直接合成为 `reasoning-start / reasoning-delta / reasoning-end` 事件扔出来，浏览器侧零改动。`createOpenAICompatible({ name, apiKey, baseURL })` 一行调用、`provider(modelId)` 拿模型，跟 `createOpenAI` 几乎一样。

判错的具体后果（点咖啡点单）：

- 用 `@ai-sdk/openai` 跑 zhipu / deepseek / qwen → 思考跟正文都落在 `content` 里，浏览器 reasoning 段是空的、text 段有"答案 + 思考"混在一起。你以为模型没思考，其实是 SDK 吃了字段。
- 自己写中间件从 raw chunks 读 `delta.reasoning_content` → 能拆，但要 `streamText({ includeRawChunks: true })` + 自己包 `TransformStream`，跟「用框架省样板」的口径相反——本课的「省了 / 藏了 / 多了」里这一档算「多了」。
- `@ai-sdk/openai-compatible` 直接拆 → 一行 SDK 替换，无中间件，符合「同一 streamText 出口」这条主线。

**`minimax` 仍走 `createOpenAI`**：它的模型真把思考写在 `` 文本里，`@ai-sdk/openai-compatible` 不读 `` 标签（只读 `reasoning_content` 字段），换 SDK 会让 minimax 也拆不出来。所以 model.ts openai 分支按 provider 选 SDK，不要统一替换。

### 为什么 OpenAI 协议各家不统一，Anthropic 协议统一（2026-09-20 揭示）

把两条协议对照看，差别在「思考这件事是不是协议原生字段」：

- **Anthropic 协议统一**：Anthropic 在 Messages API 里**原生定义了** thinking 这件事——`content[type=thinking]` 是协议字段、不是各厂商扩展。`@ai-sdk/anthropic` 是 Anthropic 官方维护的 SDK，把所有走 Anthropic 兼容协议的网关（minimax / zhipu / qwen / deepseek 都对外暴露 `/anthropic` 兼容层）当成"实现同一个协议"——只要它们按 Messages API 规约发回 `content[type=thinking]`，SDK 一律按同一条代码路径把它翻成 reasoning 段，**不需要按 provider 分流**。代价是：调用方要主动开 `providerOptions.anthropic.thinking = { type: "enabled", budgetTokens }`，因为思考这件事协议原生支持、不开就不出。
- **OpenAI 协议各家不统一**：OpenAI 官方 Chat Completions 协议里**没有独立 reasoning 字段**。各家自己决定怎么塞——minimax 把思考写在 `content` 里、用 ``…`` 文本标签包；zhipu / qwen / deepseek 没跟这套，各自把思考单独塞进 `delta.reasoning_content` 字段。这两套扩展彼此不兼容、各家自定，所以**拆法按 provider 分**：minimax 走 `extractReasoningMiddleware`，其余三家换 SDK 走 `createOpenAICompatible`。

口语版：Anthropic 协议下思考是**协议内的事**（官方字段、SDK 一条路打通）；OpenAI 协议下思考是**各家私货**（`@ai-sdk/openai` 不管、SDK 默认丢字段），所以才需要按 provider 选 SDK。

对照下来，**Anthropic 那条路径仍然是一条 SDK 走四家**——没有"按 provider 选 SDK"这一步；只有 OpenAI 这条路径需要分流。model.ts 的 anthropic 分支只有 `createAnthropic` + 手动补 `/v1` 两件事，跟 provider 无关。
...
lib/cafe/model.ts（createModel）
  ├─ OpenAI：按 provider 选 SDK
  │    ├─ minimax：createOpenAI(baseUrlA) → wrapLanguageModel + extractReasoningMiddleware({ tagName: "think" })
  │    └─ zhipu / qwen / deepseek：createOpenAICompatible(baseUrlA)（SDK 内部读 delta.reasoning_content）
...
step-1 / step-2 已锁定 ✅。表行只写已建 step；禁止预判未来。

| 状态 | 子节 | 入口 | 端口 | 本子节教学点 |
|------|------|------|------|--------------|
| ✅ | step-1 | `yarn app:13-01-framework-solves-what-step-1` | `50139` | 同一句「来一杯中杯热拿铁。」：首页手写 while 能数圈、能看见 make_latte；子页 `/pages/framework.html` 走 Vercel AI SDK 的 generateText（业务文件没有 while）。**+ framework.html 升级为 maxSteps 对照栏（变体 E+G）**：同一问句、两种 `stopWhen`，两个独立请求——左栏 `stepCountIs(1)` 只跑 1 步不调 make_latte（假循环）；右栏 `stepCountIs(5)` 至少 1 次 make_latte（真循环）。**子页 `/pages/use-chat.html`（流式 UI 教学点 · 变体 B）**：官方 `useChat` + `DefaultChatTransport` 接住 SSE 拆帧 + 消息列表维护，POST `/api/chat-stream`。**+ `/pages/no-loop.html`（新店员视角）**：把仓库文件摊开 + 标注「while 在哪」+ 模块 07 对照；GET `/api/files`。**+ `/pages/faq.html`（FAQ 不该上出杯图 · 变体 H）**：同一 FAQ 问句、两条路径——左栏直接调模型（无 tools，1 轮），右栏硬套框架 Agent（带 make_latte，agent 形式多走一步调工具）。**+ `/pages/multi-drink.html`（两杯能否并行 · 变体 C）**：注册两个工具 `make_americano` + `make_latte`，左栏一次 `generateText` 拿 2 个 tool_calls 并行执行，右栏拆 query + 分批注册工具分 2 次 `generateText` 串行执行。**+ `/pages/retry-charge.html`（扣卡被默默重试 · 变体 E · 多次 execute = 双扣）**：`charge_member_card` 工具模拟超时，execute 每次调用都计入 attempts（不管超时/成功），框架自动重试 → 客人账上可能扣多次。**+ `/pages/allergy-gate.html`（过敏拦截 · 网关 · 变体 I）**：`make_latte` 网关版，allergy=true 时 execute return `{ok:false}` → 拒绝原因回到 messages → finalText 体现（不被框架吞掉）；allergy=false 时出杯成功。 |
| ✅ | step-2 | `yarn app:13-01-framework-solves-what-step-2` | `50140` | 同一句 query 拆 reasoning 的两条路：子页 `/pages/reasoning-extract.html` 走协议下拉（provider × protocol 两个独立请求）。OpenAI 协议分支按 provider 选 SDK：`minimax` 走 `@ai-sdk/openai` + `extractReasoningMiddleware({ tagName: "think" })`（拆 `<think>…</think>` 文本），`zhipu` / `deepseek` / `qwen` 走 `@ai-sdk/openai-compatible`（SDK 内部读 `delta.reasoning_content` 拆 reasoning 段），Anthropic 协议走 `@ai-sdk/anthropic`（拆原生 thinking content blocks；需 `providerOptions.anthropic.thinking` + `maxTokens`）。浏览器侧两份代码完全一样。 |
