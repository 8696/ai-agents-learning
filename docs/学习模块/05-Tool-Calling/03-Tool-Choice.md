...
本条管另一根控制开关：**请求里显式告诉模型 —— 这一轮「可以调 / 必须调 / 禁止调 / 必须调某一个」**。
...
01 协议表里提过「强制调用 / 强制调指定」——本条把这个控制开关讲透。
...
| 5 | 谁写这个控制开关 | 服务端策略（用户无感）vs 用户显式意图（UI 开关）——最终都是后端改字段 |
...
| 5 | 设置页「只聊天 / 允许工具」 | 用户意图映射 Choice | 谁设这个开关 | UI 两档切换，请求里 Choice 字段随之变，页面能对照展示 |
...
| 强制抽取 JSON | 指定唯一 Tool（或 `required` + 单 Tool） | 当 Structured Output 用时要钉死模型吐回来的形状 |
...
跑过关检查日期：2026-09-08（step-1 已建 · 未锁定 · 过关检查 3 草稿）
