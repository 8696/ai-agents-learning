...
> **项目当前地图**：`apps/02-tool-agent/LEARNING.md`（学到再建）
...
| ⬜ | [**本地产出**](./05-本地产出.md) | 本页验收 + 学习沉淀（新建 `02-tool-agent`） | — | [沉淀](./05-本地产出.md) |
...
> 新建 `apps/02-tool-agent`（先问「可以吗？」）。从 `01-chatgpt-mini` 复制 `src/load-root-env.ts`。
