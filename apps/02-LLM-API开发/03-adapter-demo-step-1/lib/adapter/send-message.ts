 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（sendMessage / sendMessageStream 封装层，adapter 分叉）；
 *   子调用 sendViaA / sendViaB / sendViaAStream / sendViaBStream 内部已自带五件套。
 */
...
export async function sendMessage(
  ...
  logger.info(
    "│ 分叉-sendMessage",
    "调用函数开始：sendMessage",
    "为什么打：adapter 唯一分叉点；业务层只认 UnifiedResponse，不知道下面走的是 openai 还是 anthropic。当前：即将按 protocol 选 A(openai) 还是 B(anthropic)。",
    {
  ...
  logger.info(
    "│ 分叉-sendMessage",
    "调用函数结束：sendMessage",
    "为什么打：route 要把 UnifiedResponse 写进 ctx.body 交给页面；打 usage / stopReason 便于核对「两协议字段差异在 adapter 已经被抹平」。当前：分叉到的协议 sendVia* 已返回。",
    {
      返回值: {
...
export async function* sendMessageStream(
  ...
  logger.info(
    ...
    "为什么打：adapter 流式分叉点；业务层只认 UnifiedDelta 迭代器。当前：即将按 protocol 选 A(openai) 还是 B(anthropic) 的流句柄。",
    {
  ...
  // sendVia* 内部的「调用模型结束」会打完整返回值；这里只是分叉包装。
  void tFuncStart;
}