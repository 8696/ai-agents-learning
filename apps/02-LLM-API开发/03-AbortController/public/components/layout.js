 ...
 * 为什么单独成文件：这几个是「每一页都长一样」的壳，业务卡片在 abort-cards.js。
 *
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/full.html", label: "流到底", match: "full" },
    { href: "/pages/cancel.html", label: "收 N 帧停", match: "cancel" },
    { href: "/pages/no-signal.html", label: "忘传 signal", match: "no-signal" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/full.html")) return "full";
    if (p.endsWith("/cancel.html")) return "cancel";
    if (p.endsWith("/no-signal.html")) return "no-signal";
    return "";
  }
