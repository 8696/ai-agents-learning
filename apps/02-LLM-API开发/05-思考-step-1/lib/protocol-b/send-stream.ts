 ...
 * 本文件禁止 import openai。
 *
 * 日志（§5.3.16）：调用函数 五条日志（sendStreamB 封装层），调用模型 五条日志（真正发网络请求的那一层，含 __code + 字段释义）；
 *   流式规则：只在收尾写一次完整返回值；thinking event 用 debug 单独打便于核对「Anthropic-style 块是不是 content_block_delta.delta.thinking」。
 */
...
export async function sendStreamB(
  ...
  logger.info(
    "│ 协议B-sendStreamB",
    "调用函数开始：sendStreamB",
    "为什么写这条日志：route 只认这一层把 SSE 帧写到 res；里面那次才是真发网络请求（看「调用模型开始：协议B-消息流」）。当前：即将发协议 B 流式请求；plan 由 dialect 按 provider × thinkingOn 拼。",
    {
  ...
  logger.info(
    "││ 调用模型-协议B 消息流",
    "调用模型开始：协议B 消息流",
    "为什么写这条日志：本文件唯一真正发网络请求的那一层；不写就没有 thinking 流式各事件来源（content_block_delta.delta.thinking）。当前：即将发出 messages.stream 请求。",
    {
      入参: {
  ...
  try {
    ...
    logger.info(
      "││ 调用模型-协议B 消息流",
      "调用模型结束：协议B 消息流",
      "为什么写这条日志：流式场景只在收尾写一次完整返回值（thinking 累计 / sources / returnShape），便于核对页面 thinking-map。当前：finalMessage 已返回，下一步 writer.frame(thinking-map) + done()。",
      {
        返回值: {
          sourcesSeen: sources,
    ...
    logger.info(
      "│ 协议B-sendStreamB",
      "调用函数结束：sendStreamB",
      "为什么写这条日志：route 已经把 thinking-map + [DONE] 写出，连接关闭；写耗时便于和协议 A 流式对照。当前：流已消费完。",
      {
        返回值: {
          sourcesSeen: sources,
          thinkingChunks: thinkingText.chunks,
    ...
    logger.error(
      "││ 调用模型-协议B 消息流",
      "调用模型结束：协议B 消息流（失败）",
      "为什么写这条日志：拿到 SDK 错误对象（Anthropic SDK 抛的错常带 status / headers）便于排错。当前：stream 抛错；已写 error 帧。",
      {
