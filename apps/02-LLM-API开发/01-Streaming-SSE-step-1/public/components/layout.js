 ...
 * 业务卡片（答案 / 帧 / 对照）在 components/stream-cards.js。
 *
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/simulated.html", label: "模拟 SSE", match: "simulated" },
    { href: "/pages/blocking.html", label: "一次性对照", match: "blocking" },
    { href: "/pages/real.html", label: "真实模型", match: "real" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/simulated.html")) return "simulated";
    if (p.endsWith("/blocking.html")) return "blocking";
    if (p.endsWith("/real.html")) return "real";
    return "";
  }

  /**
   * §5.3.9：每页加载就请求一次 /health，把「现在用谁家的哪个模型」写进页脚。
   ...
   * 端口兜底写 50005（本条 §5.3.3 算出的默认口），真实值仍以 /health 返回的为准。
   */
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50005} · 协议 {env.protocol || "A"} · provider{" "}
