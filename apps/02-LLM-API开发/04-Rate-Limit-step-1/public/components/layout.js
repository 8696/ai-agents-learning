 ...
 * 为什么单独成文件：这几个是「每一页都长一样」的壳，业务卡片在 timeline.js。
 *
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/mock.html", label: "Mock 五场景", match: "mock" },
    { href: "/pages/real.html", label: "真 API", match: "real" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/mock.html")) return "mock";
  ...
  /**
   * §5.3.9：每页加载即拉环境信息。
   * 没拿到时保留「(待连接)」占位，不留空白；hasKey=false 时真 API 页应禁用主按钮。
   */
  function useEnvInfo() {
  ...
  function EnvFooter({ env, protocol }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50204} · 协议 {protocol || "A"} · provider{" "}
