 ...
 *
 * 日志（§5.3.16）：调用函数 五条日志（handleGetReal / handleGetRealBurst 封装层）；
 *   Key 缺失单独写 info（校验拒绝）；子调用 run-with-retry 内部已自带五条日志。
 */
...
export function mountRealRoutes(router: Router): void {
  router.get("/api/real", async (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么写这条日志：route 只认这一层返回的真请求 stats；里面 handleReal 是真正干活的那一层（retry 套真 SDK）。当前：路由层入口即将发单次真请求。",
      {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么写这条日志：route 要把 stats 写进 ctx.body 交给页面 stats 区。当前：handleReal 已返回。",
        {
      ...
      logger.error(
        ...
        "为什么写这条日志：handleReal 抛 RetryExhaustedError / NonRetryableError 都要走统一出口；记 err 便于 writeUpstreamError 决定 status。当前：handleReal 抛错。",
        {
  ...
  router.get("/api/real-burst", async (ctx: Context) => {
    ...
    if (!client) {
      logger.info(
        "api.real-burst",
        "GET /api/real-burst 被无 Key 校验挡掉",
        "为什么写这条日志：服务端兜底；没 Key 并发没必要跑。当前：apps/.env 当前 LLM_PROVIDER 无 Key。",
    ...
    logger.info(
      ...
      "为什么写这条日志：route 只认这一层返回的 BurstResult；里面 handleRealBurst 是真正干活的那一层（并发 N 个独立 retry）。当前：并发撞 429 入口即将发并发真请求。",
      {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么写这条日志：route 要把 BurstResult 写进 ctx.body 交给页面；记 aggregate 便于核对 429 / 5xx 分布。当前：handleRealBurst 已返回。",
        {
          返回值: {
      ...
      logger.error(
        ...
        "为什么写这条日志：handleRealBurst 抛 RetryExhaustedError 走统一出口；记 err 便于 writeUpstreamError 决定 status。当前：handleRealBurst 抛错。",
        {
