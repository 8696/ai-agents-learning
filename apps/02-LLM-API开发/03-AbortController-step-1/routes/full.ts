/**
 * 职责：POST /api/full —— 不取消、跑到底的 SSE 端点（薄：校验 → 开流 → 交给 flow）。
 ...
 *
 * 日志（§5.3.16）：调用函数 五条日志（handlePostFull 封装层）；
 *   校验挡下（400 / 503）单独写 info / warn（校验拒绝）；子调用 runFull 内部已自带五条日志。
 */
...
export function mountFullRoutes(router: Router): void {
  router.post("/api/full", async (ctx: Context) => {
    ...
    if (!parsed.ok) {
      logger.warn(
        "api.full",
        "POST /api/full 被入参校验挡掉",
        "为什么写这条日志：校验挡下没花模型额度也没走到 runFull；记 reason 便于复盘。当前：body 不合法。",
    ...
    logger.info(
      ...
      "为什么写这条日志：route 只认这一层把 stats 写给客户端；里面 runFull 是真正干活的那一层（看「调用函数开始：runFull」）。当前：校验全过、SSE 头已发，即将交给 runFull。",
      {
    ...
    if (stats.failed) {
      logger.error(
        ...
        "为什么写这条日志：runFull 失败已发 error 帧；记 frameIdx + failed 便于对照基线和 cancel 的 usage 差异。当前：runFull 已返回。",
        {
      ...
      logger.info(
        ...
        "为什么写这条日志：route 要把 stats 写给客户端；记帧数 + usage 摘要便于对照基线和 cancel 的差异。当前：runFull 已返回。",
        {
