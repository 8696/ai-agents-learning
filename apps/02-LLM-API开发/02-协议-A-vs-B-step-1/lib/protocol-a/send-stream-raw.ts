 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（streamOnceAClassified 封装层），调用模型 五件套（出网层）；
 *   流式规则：只在收尾打一次完整返回值，中间 chunk 打 debug 即可。
 */
...
export async function streamOnceAClassified(
  ...
  logger.info(
    ...
    "为什么打：route 只认这一层把带 kind 的帧写到 res；里面那次才是出网（看「调用模型开始：协议A-对话补全」）。当前：即将发 stream:true 请求给 /api/a-stream-raw 页面看分类。",
    {
  ...
  logger.info(
    "││ 调用模型-协议A 对话补全",
    "调用模型开始：协议A 对话补全",
    "为什么打：本文件唯一的真出网层；不打就没有 frameCount / usage / kind 分类依据。当前：即将发出 stream:true 请求。",
    {
      入参: {
  ...
  logger.info(
    "││ 调用模型-协议A 对话补全",
    "调用模型结束：协议A 对话补全",
    "为什么打：流式场景只在收尾打一次完整返回值（kind 分布便于事后核对「有没有真发出 thinking 帧」）。当前：for await 已退出，下一步 writer.done()。",
    {
