 ...
 *
 * 日志（§5.3.16）：调用函数 五条日志（streamOnceARawChunks 封装层），调用模型 五条日志（真正发网络请求的那一层）；
 *   流式规则（§5.3.16）：只在收尾写一次完整返回值，中间 chunk 写 debug 即可。
 */
...
export async function streamOnceARawChunks(
  ...
  logger.info(
    ...
    "为什么写这条日志：route 只认这一层把 openai chunk 写到 res；里面那次才是真发网络请求（看「调用模型开始：协议A-对话补全」）。当前：即将发 stream:true 请求给 curl /api/a。",
    {
  ...
  logger.info(
    "││ 调用模型-协议A 对话补全",
    "调用模型开始：协议A 对话补全",
    "为什么写这条日志：真正发网络请求的那一次；不写就没有 frameCount / usage。当前：即将发出 stream:true 请求，include_usage 强制让最后一帧带 usage。",
    {
      入参: {
  ...
  logger.info(
    "││ 调用模型-协议A 对话补全",
    "调用模型结束：协议A 对话补全",
    "为什么写这条日志：流式场景只在收尾写一次完整返回值（最后帧的 usage + finishReason）；便于核对「是否最后一帧才带 usage」。当前：for await 已退出，下一步 writer.done()。",
    {
