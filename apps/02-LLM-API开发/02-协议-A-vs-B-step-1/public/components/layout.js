 ...
 * 数据流：GET /health → 页脚同时写 modelA / modelB（对照例外）。
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/once.html", label: "一次性对照", match: "once" },
    { href: "/pages/stream-a.html", label: "流式 A", match: "stream-a" },
    { href: "/pages/stream-b.html", label: "流式 B", match: "stream-b" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/once.html")) return "once";
    if (p.endsWith("/stream-a.html")) return "stream-a";
    if (p.endsWith("/stream-b.html")) return "stream-b";
    return "";
  }
  ...
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50006} · 协议 A/B · provider{" "}
