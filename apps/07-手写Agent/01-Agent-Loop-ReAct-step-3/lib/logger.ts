/**
 * 职责：把业务事件写到文件 + 控制台（人话讲稿）。拷自顶层 apps/logger.ts 的完整 freeze 副本。
 * 本地 freeze 副本 · 拷自顶层 apps/logger.ts（2026-09-10）。
 *
 * 禁止运行时 import 顶层 apps/logger（§5.3.16）；落 Demo 当下完整拷到本夹 lib/logger.ts，
 * 业务只 import { logger } from "./logger"。
 *
 * 路径：apps/07-手写Agent/01-Agent-Loop-ReAct-step-1/lib/logger.ts → ../../../logs 之外 = "../logs"
 *   （从 lib/ 出发跳一级 = demo 根，再拼 logs/）
 *
 * 数据流：
 *   logger.info(scope, msg, explain, data?)
 *     → mkdir -p ../logs
 *     → appendFileSync(./logs/{YYYY-MM-DD}.log, …)
...
export interface Logger {
  ...
  error(scope: string, msg: string, msgExplain: string, data?: unknown): void;
}
...
// ── 本 demo 的 logger 实例：写到 demo 根目录下的 logs/（绝对路径，避免相对 cwd）──
