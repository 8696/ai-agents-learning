...
export interface RunLoopInput {
  ...
  /** step-4 新增：用户取消闸（看 abortSignal.aborted） */
  ...
  hardCap: number;
  label: string;
  /** step-4 新增：用户取消信号；和 user-cancel route 的 AbortController 共享 */
...
export async function runLoop(input: RunLoopInput): Promise<RunLoopOutput> {
  const { enableMaxStepsGate, enableTimeoutGate, enableModelStopGate, enableUserCancelGate, useRealLlm, hardCap, label, abortSignal } = input;
  ...
  logger.info(
    "调用函数-runLoop",
    "调用函数开始：runLoop",
    "为什么写这条日志：四闸门演示根函数；当前：label=" + label + " · useRealLlm=" + useRealLlm + " · max=" + (enableMaxStepsGate ? input.maxSteps : "off") + " · timeout=" + (enableTimeoutGate ? input.timeoutMs + "ms" : "off") + " · model_stop=" + (enableModelStopGate ? "on" : "off") + " · user_cancel=" + (enableUserCancelGate ? "on" : "off"),
  ...
  while (stepCount < effectiveMax) {
    // 闸门 4 · 用户取消：abortSignal.aborted → break（step-4 新增）
    if (enableUserCancelGate && abortSignal?.aborted) {
      stoppedReason = "user_cancel";
      gateTriggered = `用户取消（${abortSignal.reason || "客户端断开 / 取消按钮"}）`;
      break;
    }
    ...
    if (elapsedNow >= effectiveTimeoutMs) {
      ...
      gateTriggered = `达到超时上限（${input.timeoutMs}ms · 实际 ${elapsedNow}ms）`;
      break;
    }
    ...
    if (!enableMaxStepsGate && !enableTimeoutGate && !enableModelStopGate && !enableUserCancelGate && stepCount >= hardCap) {
