/**
 * 职责：GET /api/plan-and-execute —— 只跑右栏「先规划再执行」（含重规划）。
 * 数据流：query.task → runPlanAndExecute → ctx.body（plans[] + executeTrace.planVersion）。
...
export function mountPlanAndExecuteRoutes(router: Router): void {
  router.get("/api/plan-and-execute", async (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么写这条日志：右栏自己的入口；下一步只跑先规划再执行（含重规划）。",
    ...
    logger.info(
      ...
      "为什么写这条日志：右栏收口；前端自己拿 plans[] + 执行卡。",
      { 返回值: { planCalls: planAndExecute.summary.planCalls, executeCalls: planAndExecute.summary.executeCalls, planVersions: planAndExecute.plannerIterations }, 耗时ms: Date.now() - t0 },
    );
  });
}
