...
// ── step-6 变体 F：两阶段（planOnly 只调规划器；executeFromPlan 才执行）──
// in-memory sessions map（按 sessionId 存「plan 但未执行」的中间状态）
const sessions = new Map<string, { task: string; initialSteps: PlanStep[]; rawText: string; fallback: boolean }>();

// 阶段 1：只调真规划器 1 次，拿到 v1 plans；**不执行任何工具**
async function planOnly(task: string): Promise<{ steps: PlanStep[]; rawText: string; fallback: boolean }> {
  if (llm) {
    const r = await planWithLlm(task);
    if (r.plan.length > 0) return { steps: r.plan, rawText: r.rawText, fallback: false };
    return { steps: FALLBACK_PLAN, rawText: r.rawText, fallback: true };
  }
  return { steps: FALLBACK_PLAN, rawText: "(no LLM)", fallback: true };
}

// 阶段 2：从 v1 plans 开始执行循环（含 shouldReplan 重规划 + MAX_PLAN_VERSIONS 保险）
async function executeFromPlan(
  ...
  const plans: Array<{ version: number; steps: PlanStep[]; rawText: string; fallback: boolean; reason?: string }> = [
    { version: 1, steps: initialSteps, rawText, fallback },
  ];
  const observations: Observation[] = [];
  const executeTrace: Array<ExecuteTrace & { planVersion: number }> = [];
  let curPlan = plans[plans.length - 1];
  ...
  while (curPlanIdx < curPlan.steps.length) {
    ...
    const t0 = Date.now();
    const r = invokeTool({ tool: step.tool, args: step.args });
    const trace = { index: curPlanIdx + 1, step, result: r, costMs: Date.now() - t0, planVersion: curPlan.version };
    executeTrace.push(trace);
    observations.push({ tool: step.tool, args: step.args, result: r.result });
    ...
    if (replan.trigger && plans.length < MAX_PLAN_VERSIONS) {
      const newVersion = plans.length + 1;
      logger.info("│ 调用函数-replan-detector", "调用函数：检测到重规划触发（step-6 同 step-3）", "step-6 重规划逻辑不变", { reason: replan.reason });
      let newSteps: PlanStep[];
      let newRawText = "";
      let newFallback = false;
      if (llm) {
        const r2 = await planWithLlm(task, observations);
        if (r2.plan.length > 0) {
          newSteps = r2.plan;
          newRawText = r2.rawText;
        } else {
          newSteps = FALLBACK_PLAN;
          newRawText = r2.rawText;
          newFallback = true;
        }
      } else {
      ...
      plans.push({ version: newVersion, steps: newSteps, rawText: newRawText, fallback: newFallback, reason: replan.reason });
      curPlan = plans[plans.length - 1];
      curPlanIdx = 0;
      observations.length = 0;
    } else {
      curPlanIdx++;
  ...
  return { plans, executeTrace, plannerIterations: plans.length };
}

export function mountCompareRoutes(router: Router): void {
  // ── 阶段 1 GET：只 plan，不执行 → 返回 plans + sessionId + status='pending' ──
  router.get("/api/compare", async (ctx: Context) => {
    const task = String(ctx.query.task ?? "春季上新：拉库存、给有货 SKU 文案、通知运营");
    const sessionId = randomUUID();
    logger.info(
      "路由-compare",
      "调用函数开始：mountCompareRoutes.GET /api/compare（step-6 阶段 1：只 plan）",
    ...
    ctx.body = {
      sessionId,
      task,
      stepByStep: null, // 阶段 1 不跑 A 路径
      plans: [{ version: 1, steps: v1.steps, rawText: v1.rawText, fallback: v1.fallback }],
      plannerIterations: 1,
      executeTrace: [],
      status: "pending", // ← 关键：等用户点确认
    };
    logger.info(
      "路由-compare",
      "调用函数结束：mountCompareRoutes.GET /api/compare（阶段 1）",
      "为什么写这条日志：plan 已就位；状态 pending；前端渲染 plan 卡 + 「确认执行」按钮。",
      { 返回值: { sessionId, planStepsCount: v1.steps.length, status: "pending" }, 耗时ms: Date.now() - t0 },
    );
  });

  // ── 阶段 2 POST /api/confirm-plan：用户确认后跑执行 ──
  router.post("/api/confirm-plan", async (ctx: Context) => {
    const body = (ctx.request.body ?? {}) as { sessionId?: string };
    const sessionId = body.sessionId;
    logger.info(
      "路由-confirm",
      "调用函数开始：mountCompareRoutes.POST /api/confirm-plan",
      "为什么写这条日志：变体 F 关键 —— 人点头了；之前 pending 的 plan 现在才执行。",
    ...
    sessions.delete(sessionId); // 确认完清理（防止重复执行）

    // 变体 E：累计 executeTrace 里 result.ok === false 的次数
    const toolFailures = result.executeTrace.filter(s => s.result.ok === false).length;

    // 也跑 A 路径一次（让最终答案对照完整）
    const stepByStep = await runStepByStep(session.task);
    ...
    ctx.body = {
      ...
      comparison: {
        ...
        modelCallsB: result.plannerIterations,
        preActStepsA: 0,
        preActStepsB: 1,
        planExistsBeforeFirstActA: false,
        planExistsBeforeFirstActB: true,
        replannerTriggered: result.plannerIterations > 1,
        planVersions: result.plannerIterations,
        toolFailures,
        variantETriggered: toolFailures > 0,
      },
    };
    logger.info(
      "路由-confirm",
      "调用函数结束：mountCompareRoutes.POST /api/confirm-plan",
      "为什么写这条日志：人点头后执行；status → executed；前端渲染 executeTrace + 最终答案。",
