 ...
 * step-8 调整：工具从 queryStock 改成 queryTodo（todo 助手端到端业务）。
 */
import { logger } from "../logger.js";
import type { RunLoopInput, RunLoopOutput, StoppedReason } from "./loop-types.js";
import { mockToolQueryTodo, estimateTokensAfter } from "./loop-helpers.js";
...
export async function runOneStep(deps: StepDeps): Promise<boolean> {
  ...
  if (toolCall) {
    const toolArg = String(toolCall.args.id ?? toolCall.args.sku ?? "");
    state.lastToolArg = toolArg;
    state.toolCallCount += 1;
    const retryResult = await runWithRetry({
      tool: (attempt) => mockToolQueryTodo(
        toolArg,
    ...
    if (retryResult.ok) {
      toolResult = { ...retryResult, payload: retryResult.payload };
    } else {
      if (input.enableToolRetryGate) {
        ...
        toolResult = { ok: false, latencyMs: 0, error: retryResult.error ?? "unknown", attempt: 1, failedAttempts: retryResult.failedAttempts };
        logger.warn(
          "调用函数-runLoop",
          ...
          { toolName: toolCall.name, toolArgs: toolCall.args, toolMaxRetries: input.toolMaxRetries, finalState: "tool_unavailable" },
        );
      } else {
        toolResult = { ok: false, latencyMs: 0, error: retryResult.error ?? "unknown", attempt: 1, failedAttempts: retryResult.failedAttempts };
      }
