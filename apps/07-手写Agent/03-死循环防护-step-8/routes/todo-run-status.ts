/**
 * 职责：GET /api/agent/todo-run-status/:runId —— 轮询拿 todo 助手 run 结果。
...
export function mountTodoRunStatusRoutes(router: Router): void {
  router.get("/api/agent/todo-run-status/:runId", async (ctx: Context, _next: Next) => {
