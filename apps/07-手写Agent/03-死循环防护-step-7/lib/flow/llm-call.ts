...
export const TOOLS_SCHEMA: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  {
    ...
    function: {
      name: "queryStock",
      description: "查某个 SKU（Stock Keeping Unit，库存单品）的当前可用库存数量",
      parameters: {
        type: "object",
        properties: {
          sku: { type: "string", description: "SKU 编号，形如 SKU-001" },
        },
        required: ["sku"],
      },
    },
...
export const SYSTEM_PROMPT = `你是一个查库存助手。请按用户要求调用 queryStock 工具查每个需要的 SKU。
当所有需要的 SKU 都查完后，**不要再调工具**，输出 final_answer 字段总结（例如：「final_answer: 已查完所有 SKU」）。`;
