...
export async function runLoop(input: RunLoopInput): Promise<RunLoopOutput> {
  const { enableMaxStepsGate, enableTimeoutGate, enableModelStopGate, enableUserCancelGate, enableToolRetryGate, enableToolCallLoopGate, useRealLlm, hardCap, label, abortSignal } = input;
  ...
  logger.info(
    "调用函数-runLoop",
    "调用函数开始：runLoop",
    "为什么写这条日志：六闸门演示根函数；当前：label=" + label + " · useRealLlm=" + useRealLlm + " · max=" + (enableMaxStepsGate ? input.maxSteps : "off") + " · timeout=" + (enableTimeoutGate ? input.timeoutMs + "ms" : "off") + " · model_stop=" + (enableModelStopGate ? "on" : "off") + " · user_cancel=" + (enableUserCancelGate ? "on" : "off") + " · tool_retry=" + (enableToolRetryGate ? input.toolMaxRetries + "次" : "off") + " · tool_loop=" + (enableToolCallLoopGate ? "连续" + input.loopDetectionWindow + "次" : "off") + " · flakyRate=" + input.flakyRate + " · alwaysFail=" + input.alwaysFail,
  ...
  while (state.stepCount < effectiveMax) {
    ...
    // 全关闸的人为硬上限
    if (!enableMaxStepsGate && !enableTimeoutGate && !enableModelStopGate && !enableUserCancelGate && !enableToolRetryGate && !enableToolCallLoopGate && state.stepCount >= hardCap) {
  ...
  const out: RunLoopOutput = {
    ...
    summary: { elapsedMs, lastSku: state.lastSku, finalAnswer: state.finalAnswer, toolUnavailable: state.toolUnavailable, loopDetected: state.loopDetected, recentToolCalls: state.recentToolCalls },
  };
  logger.info(
    "调用函数-runLoop",
    "调用函数结束：runLoop",
    "为什么写这条日志：六闸门演示收口；前端要把 stoppedReason + stepCount + toolCallCount + toolRetrySuccessCount + elapsedMs 五个数字亮出来，看谁先到。当前：stepCount=" + state.stepCount + " · stoppedReason=" + state.stoppedReason + " · elapsedMs=" + elapsedMs + " · 工具调用 " + state.toolCallCount + " 次 / 失败 " + state.toolFailureCount + " 次 / 重试成功 " + state.toolRetrySuccessCount + " 次",
