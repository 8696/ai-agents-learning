...
// ── 一步步走（变体 A · ReAct · step-4 真模型 · 四件套）──
// tools（OpenAI 协议 A 格式）—— step-4 加 complete_todo（短任务用）
const TOOLS_OPENAI = [
...
const SYSTEM_PROMPT_REACT = [
  ...
  "",
  "你可以用以下工具（每个任务可能只需要其中一个）：",
  "- query_stock(sku) — 查 SKU 库存（候选 SKU-88 / SKU-89 / SKU-90）",
  "- write_copy(sku, copy) — 给 SKU 写文案",
...
export function mountCompareRoutes(router: Router): void {
  router.get("/api/compare", async (ctx: Context) => {
    const longTask = String(ctx.query.task ?? "春季上新：拉库存、给有货 SKU 文案、通知运营");
    const shortTask = "把 todo-001 标完成";
    logger.info(
      "路由-compare",
      "调用函数开始：mountCompareRoutes.GET /api/compare（step-4 四组对照）",
      "为什么写这条日志：路由是 demo 的总入口；step-4 起跑 4 组轨迹（短 A / 短 B / 长 A / 长 B）并行，对照「什么时候不值得先规划」。",
      { 入参: { shortTask, longTask }, __code: "const [shortA, shortB, longA, longB] = await Promise.all([...]);" },
    );

    const t0 = Date.now();
    // ── 四组轨迹并行跑 ──
    const [shortA, shortB, longA, longB] = await Promise.all([
      runStepByStep(shortTask),
      runPlanAndExecute(shortTask),
      runStepByStep(longTask),
      runPlanAndExecute(longTask),
    ]);

    // ── 关键对照数字：每组的模型调用次数 + 步数 + 总耗时 ──
    const sumCost = (tr: Array<{ costMs: number }>) => tr.reduce((s, r) => s + r.costMs, 0);

    const comparison = {
      short: {
        task: shortTask,
        a: {
          rounds: shortA.summary.rounds,
          modelCalls: shortA.summary.modelCalls,
          stepCount: shortA.trajectory.reduce((s, r) => s + r.act.length, 0),
          totalMs: sumCost(shortA.trajectory),
          stoppedReason: shortA.stoppedReason,
        },
        b: {
          plans: shortB.plannerIterations,
          stepCount: shortB.executeTrace.length,
          totalMs: sumCost(shortB.executeTrace),
        },
      },
      long: {
        task: longTask,
        a: {
          rounds: longA.summary.rounds,
          modelCalls: longA.summary.modelCalls,
          stepCount: longA.trajectory.reduce((s, r) => s + r.act.length, 0),
          totalMs: sumCost(longA.trajectory),
          stoppedReason: longA.stoppedReason,
        },
        b: {
          plans: longB.plannerIterations,
          stepCount: longB.executeTrace.length,
          totalMs: sumCost(longB.executeTrace),
        },
      },
      // 关键判断：A 模型调用次数 > B 规划器次数 → B 更省模型调用 → 值得先规划
      shortWorthReplan: shortB.plannerIterations < shortA.summary.modelCalls,
      longWorthReplan: longB.plannerIterations < longA.summary.modelCalls,
    };

    ctx.body = { shortTask, longTask, shortA, shortB, longA, longB, comparison };
    logger.info(
      "路由-compare",
      "调用函数结束：mountCompareRoutes.GET /api/compare",
      "为什么写这条日志：路由层收口；下一步前端按 4 栏对照渲染。",
      { 返回值: {
        shortWorthReplan: comparison.shortWorthReplan,
        longWorthReplan: comparison.longWorthReplan,
      }, 耗时ms: Date.now() - t0 },
    );
  });
}
