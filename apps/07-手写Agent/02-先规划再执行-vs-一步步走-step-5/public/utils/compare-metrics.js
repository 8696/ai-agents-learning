...
window.DemoUtils.buildComparison = function buildComparison(stepByStep, planAndExecute) {
  ...
  return {
    ...
    toolFailures: (planAndExecute.executeTrace || []).filter(function (s) { return s.result && s.result.ok === false; }).length,
    variantETriggered: (planAndExecute.executeTrace || []).some(function (s) { return s.result && s.result.ok === false; }),
  };
};
