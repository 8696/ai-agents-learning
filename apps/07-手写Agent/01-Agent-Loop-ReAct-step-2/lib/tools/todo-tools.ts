...
const completeTodoHandler: ToolHandler = async (argsJson: string) => {
  ...
  // step-2 教学点（变体 E 并行可见）：模拟「外部 API 真实耗时」50~250ms 随机延时。
  // 没有这个延时，4 个 complete_todo 各 <1ms 跑完，看不出串行 vs 并行差别。
  // 用 id 末位数字当种子让每次执行稳定可复现（同样的 id 同一延时），便于对照日志。
  const seed = parseInt(parsed.data.id.replace(/\D/g, "") || "0", 10);
  const delayMs = 50 + (seed % 5) * 50; // 50 / 100 / 150 / 200 / 250
  await new Promise<void>((resolve) => setTimeout(resolve, delayMs));
  const updated = completeTodo(parsed.data.id);
  if (!updated) {
    // 变体 G 雏形：业务失败 → 返回 not_found 而非 throw；step-2 再正式做 Observe 错误通道
    return { content: JSON.stringify({ ok: false, error: "not_found", id: parsed.data.id, simulatedDelayMs: delayMs }) };
  }
  return { content: JSON.stringify({ ok: true, todo: updated, simulatedDelayMs: delayMs }) };
};
