 ...
 *
 * 数据流：
 *   messages: [{role, content}, ...] + signal?: AbortSignal   ← 入参
 *     │
 *     │ while (没碰上停止条件 && round < MAX_ROUNDS && !signal?.aborted)
 *     │   ├─ Reason：openai.chat.completions.create({ messages, tools }, { signal })
 ...
 *   - 变体 K MAX_ROUNDS：打到上限不再继续；轨迹照写、不抛错；UI 标「达到最大轮次」
 *   - 变体 M 用户取消（step-4 主教学点）：signal.aborted === true → break；trajectory 仍完整，
 *     UI 标「用户取消」；stoppedReason = "cancelled" 与"普通失败"区分开
 *
 * step-1/2/3 不演示 M；step-4 把 signal 透传 + while 起点检测 + stoppedReason 扩 "cancelled"。
 *
 * 教学锚点（变体 H 多圈 / D 串行依赖 / J 最终答案 / M 用户取消）：
 *   - 真调 LLM（§5.3.0 强制：禁止 mock 凑闭环）
 *   - loop 写出完整轨迹（trajectory）给前端按圈展开 —— 让「循环在转」肉眼可见
 *   - 每圈日志（§5.3.16 五条日志 + 调用循环）：入参完整 messages / 返回值完整 / __code / 耗时
 *   - signal 是 AbortController 持有的；routes/cancel.ts 调 abort() → 下一次 LLM 调用抛 AbortError；
 *     已发出的 tool handler（Promise.all 启动的）不感知 signal —— 让它跑完，记「已发出未完成」
 *     （MD 例子 5 教学点）
 */
...
export type LoopResult = {
  ...
  finalAnswer: string;
  /** 解释为什么停：final_answer | max_rounds | cancelled
   *  - final_answer：本圈没 tool_calls（变体 J）
   *  - max_rounds：兜底触发（变体 K · 不指望 step-4 主出口）
   *  - cancelled：用户点了取消按钮（变体 M · step-4 主教学点）
   */
...
export type RunLoopParams = {
  ...
  maxRounds?: number;
  /** step-4 加：AbortSignal。routes/cancel.ts 触发 abort() 后，下一次 LLM 调用立刻抛 AbortError，
   *  while 起点也会检测 aborted → break + stoppedReason="cancelled"。 */
  signal?: AbortSignal;
};

/** step-4 停止条件：J 最终答案 / K MAX_ROUNDS 兜底 / M 用户取消（signal.aborted）。
 *  L 超时留给后面 step。 */
export async function runAgentLoop(params: RunLoopParams): Promise<LoopResult> {
  const { openai, model, initialMessages, signal } = params;
  ...
  // ── step-4 变体 M：signal 提前 abort 时直接不进 while ──
  if (signal?.aborted) {
    stoppedReason = "cancelled";
    logger.warn("调用循环-loop", "调用循环结束：用户取消（进入 while 前 signal 已 aborted）",
      "为什么写这条日志：routes/cancel.ts 调 abort() 时 Loop 还没启动；这里要明确记一笔，不让 cancelled 静默掉。当前：return 空 trajectory。",
      { signalAborted: true, signalReason: signal.reason });
    return {
      trajectory,
      finalAnswer: "（用户已取消 · Loop 还未启动）",
      stoppedReason: "cancelled",
      rounds: 0,
  ...
  while (round < maxRounds) {
    // ── step-4 变体 M：while 起点检测 signal.aborted ──
    // 已发出的 tool handler 不感知 signal（变体 M 的妥协：让那一圈 Act 跑完），
    // 但下一圈 Reason 不会再发起 —— 这是「跑完当前圈」与「立刻停」的边界。
    if (signal?.aborted) {
      stoppedReason = "cancelled";
      logger.warn("调用循环-loop", `调用循环结束：用户取消（第 ${round + 1} 圈前检测到 signal.aborted）`,
        "为什么写这条日志：本条变体 M 教学点 = 用户点了取消后 Loop 怎么收。当前：while 退出；不抛错、走 stoppedReason=cancelled 收尾。",
        { round, signalAborted: true, signalReason: signal.reason });
      break;
    }
    round += 1;
    ...
    logger.info("││ 调用模型-对话补全", "调用模型开始：对话补全",
      ...
      {
        入参: request,
        signal: signal ? "已透传 AbortSignal（用户取消会立刻中断）" : "未传（变体 M 不生效）",
        __code: "const completion = await openai.chat.completions.create(request, { signal });",
      });

    const t0 = Date.now();
    let completion;
    try {
      completion = await openai.chat.completions.create(request, { signal });
    } catch (err: unknown) {
      // ── step-4 变体 M：AbortError 不是失败，是用户取消的合法结果 ──
      // OpenAI SDK 在 signal abort 时抛 AbortError（DOMException）；不要当 502 回前端。
      const isAbort = err instanceof Error && (err.name === "AbortError" || err.name === "APIUserAbortError" || signal?.aborted === true);
      if (isAbort) {
        stoppedReason = "cancelled";
        logger.warn("││ 调用模型-对话补全", "调用模型结束：对话补全（用户取消）",
          "为什么写这条日志：变体 M 闭环 —— signal.aborted → OpenAI 抛 AbortError；不让它冒成 502。当前：while 退出；stoppedReason=cancelled。",
          { round, signalReason: signal?.reason, 耗时ms: Date.now() - t0 });
        break;
      }
    ...
    // step-4 把 M（用户取消）从兜底升级成主教学点 —— while 起点检测 signal.aborted，
    // LLM 调用透传 signal → 抛 AbortError → break + stoppedReason=cancelled。
  }

  if (round >= maxRounds && stoppedReason === "final_answer") {
    // 走到这里只可能是 stoppedReason 没被 J 设过 —— 说明 while 是被条件 break 而不是被 J break，
    // 即 MAX 兜底触发。**cancelled 不被覆盖**：用户取消优先级高于 MAX 兜底。
  ...
  // ── step-4 变体 M：cancelled 时 finalAnswer 用清晰文案（不是「模型未给出最终答案」）──
