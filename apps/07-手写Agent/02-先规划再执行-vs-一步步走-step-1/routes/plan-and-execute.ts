/**
 * 职责：GET /api/plan-and-execute —— 只跑右栏「先规划再执行」。
...
export function mountPlanAndExecuteRoutes(router: Router): void {
  router.get("/api/plan-and-execute", async (ctx: Context) => {
    ...
    logger.info(
      ...
      { 返回值: { planCalls: planAndExecute.summary.planCalls, executeCalls: planAndExecute.summary.executeCalls }, 耗时ms: Date.now() - t0 },
    );
  });
}
