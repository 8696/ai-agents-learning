 ...
 *
 * step-1 走 §5.3.0 例外「纯协议形状 / UI 渲染层演示」—— mock 模型 + mock 工具，不调真 LLM：
 *   - /health 加 callsModel: false，页脚写「本地计算 · 不调 LLM」
 *   - 端口基线（2026-09-10）：占用表当前最大 50052（07-01 step-4），
 *     新 demo = 50053。模块 07 · 02 第一份 HTTP Demo，从 50053 起步。
 *   - yarn script 命令 inline PORT=50053（§5.3.3 端口主源）；本 .default() 是兜底。
 *
 * getLlmOptional 保留：step-2 可能加「真规划器」，接口已经准备好；缺 Key 时 llm=null。
 */
import { z } from "zod";
