 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostStructuredOutput 封装层）；
 *   Key 缺失单独打 info 闸门拒绝；子调用 runStructuredOutput 内部已自带五件套。
 */
  ...
  router.post("/api/structured-output", async (ctx: Context) => {
    ...
    if (!client) {
      logger.info(
        ...
        "POST /api/structured-output 被无 Key 闸门挡掉",
    ...
    if (!prompt) {
      logger.info(
        "api.structured-output",
        "POST /api/structured-output 被入参闸门挡掉",
        "为什么打：闸门挡掉没花模型额度也没走到 runStructuredOutput；记 reason 便于复盘。当前：body 不合法。",
        { endpoint: "POST /api/structured-output" },
      );
      return;
    }
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 ModeCallResult；里面 runStructuredOutput 是「真活」。当前：Structured Output（语义闸）入口；记 provider / model / prompt 便于和 JSON Mode 对照同一输入下两套闸的差异。",
      {
    ...
    try {
      ...
      logger.error(
        ...
        "为什么打：协议 A 抛异常（网络 / 5xx / 4xx）；记 upstreamStatus + 错误信息便于排错，并返回前端。当前：runStructuredOutput 抛错，已交给 writeUpstreamError 写统一错误响应。",
        {
