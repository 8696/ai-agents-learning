 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostToolUse 封装层）；
 *   Key 缺失单独打 info 闸门拒绝；子调用 runToolUseForced 内部已自带五件套。
 */
...
export function mountToolUseRoutes(router: Router): void {
  router.post("/api/tool-use", async (ctx: Context) => {
    ...
    if (!prompt) {
      logger.info(
        "api.tool-use",
        "POST /api/tool-use 被入参闸门挡掉",
        "为什么打：闸门挡掉没花模型额度也没走到 runToolUseForced；记 reason 便于复盘。当前：body 不合法。",
        { endpoint: "POST /api/tool-use" },
      );
      return;
    }
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 ModeCallResult；里面 runToolUseForced 是「真活」（协议 B 强制 tool_choice）。当前：prompt + tools + tool_choice 三件套在 runToolUseForced 内拼装；这里只打 prompt 便于和响应 tool_use.input 对照。",
      {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么打：route 要把 ModeCallResult 写进 ctx.body 交给页面 stats 区；记 parseOk + toolUse 便于核对。",
        {
      ...
      logger.error(
        ...
        "为什么打：把上游 SDK 抛的错原样打出来，便于按 status / message 排错。protocolB 几乎不在 API 入口 400 坏 schema，所以这里通常是 Key / 限流 / 网关错。",
        {
