 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostToolRejected 封装层）；
 *   Key 缺失单独打 info 闸门拒绝；子调用 runToolRejected 内部已自带五件套。
 */
...
export function mountToolRejectedRoutes(router: Router): void {
  router.post("/api/tool-rejected", async (ctx: Context) => {
    ...
    if (!client) {
      logger.info(
        "api.tool-rejected",
        "POST /api/tool-rejected 被无 Key 闸门挡掉",
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 ToolRejectedResult；里面 runToolRejected 是「真活」。当前：诱导守约入口——无 body prompt，INDUCE_UNKNOWN_PROMPT 写死在 schema/intent.ts。",
      {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么打：route 要把 ToolRejectedResult 写进 ctx.body 交给页面 stats 区；记 violated + parseOk 便于核对。",
        {
      ...
      logger.error(
        ...
        "为什么打：诱导测试本身不抛错；如果抛，几乎一定是 Key / 网关 / 限流。记 err 便于排错。",
        {
