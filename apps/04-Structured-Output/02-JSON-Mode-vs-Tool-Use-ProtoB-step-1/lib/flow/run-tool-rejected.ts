 ...
 *   这一步测的是模型守约能力；和协议 A 的 strict-rejected 测的不是一回事。
 *
 * 日志（§5.3.16）：调用函数 五条日志（runToolRejected 编排层），子调用 runToolUseForced 内部已自带五条日志。
 */
...
export async function runToolRejected(llm: Llm): Promise<ToolRejectedResult> {
  ...
  // 这一步主要观察 input_schema / enum 在「prompt 故意诱导填 enum 外 action」时的守约能力。
  logger.info(
    "│ 诱导守约-runToolRejected",
    "调用函数开始：runToolRejected",
    "为什么写这条日志：route 只认这一层返回的 ToolRejectedResult；里面 runToolUseForced 是真正干活的那一层。当前：教学点——协议 B 不像协议 A 在 API 入口 400 拒坏 schema，这一步测的是模型守 input_schema / enum 的能力。",
    {
  ...
  logger.info(
    ...
    "为什么写这条日志：route 要把 ToolRejectedResult 写进 ctx.body 交给页面 stats 区；记 violated + parseOk 便于核对「prompt 故意诱导下模型守不守 enum」。",
    {
      返回值: {
        parseOk: out.parseOk,
        violated,
