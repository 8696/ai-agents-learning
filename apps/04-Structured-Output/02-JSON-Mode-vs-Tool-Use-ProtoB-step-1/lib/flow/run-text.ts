 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（runTextNoTools 封装层），调用模型 五件套（出网层，含 __code + 字段释义）。
 */
...
export async function runTextNoTools(llm: Llm, prompt: string): Promise<ModeCallResult> {
  ...
  logger.info(
    ...
    "为什么打：route 只认这一层返回的 ModeCallResult；里面那次才是出网（看「调用模型开始：协议B-消息创建」）。当前：协议 B 没有 response_format 开关；这是「JSON Mode 等价路径」，全靠 prompt 强约束。",
    {
  ...
  logger.info(
    "││ 调用模型-协议B 消息创建",
    "调用模型开始：协议B 消息创建",
    "为什么打：本文件唯一的真出网层；不打就没有 raw + usage。当前：即将发出无 tools 的 messages.create；model / max_tokens / messages 拼接（prompt + 结构指令 + 字段说明）都要打。",
    {
      入参: {
  ...
  try {
    ...
    logger.info(
      "││ 调用模型-协议B 消息创建",
      "调用模型结束：协议B 消息创建",
      "为什么打：完整打响应便于核对 SDK 自带字段：stop_reason / model / content[]（注意：text 路径模型可能夹 markdown fence / think 块，剥壳在 safeParseIntent）；同时打 usage 便于对照 token 损耗。",
      {
        返回值: {
          id: res.id,
          model: res.model,
  ...
  logger.info(
    ...
    "为什么打：route 要把 ModeCallResult 写进 ctx.body 交给页面 stats 区；打 parseOk / analysis 便于和 tool-use 对照「prompt 软约束 vs input_schema 倾向」的差异。",
    {
      返回值: {
        mode: result.mode,
