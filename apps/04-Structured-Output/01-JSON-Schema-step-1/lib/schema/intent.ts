 ...
 *
 * 日志（§5.3.16）：工具档——五件套（含 __code）仍要；本条不调 LLM，按 Zod 的 success / failure 走两条日志路径。
 */
import { z } from "zod";
...
export function repairPromptOf(error: z.ZodError): string {
  ...
  logger.info(
    ...
    "为什么打：把 Zod issues 翻成人话喂回模型；记 prompt 让排错时看清喂回去的指令。",
    {
...
export function runParse(payload: unknown) {
  ...
  logger.info(
    "│ 契约-runParse",
    "调用函数开始：runParse",
    "为什么打：route 只认这一层返回的 { parseOk, value, safeParse }；本条不调 LLM，只是 Zod 校验。当前：同份 payload 走 parse + safeParse 两条；先记原始 payload 形状便于复现 bad case。",
    {
  ...
  if (safe.success) {
    logger.info(
      "│ 契约-runParse",
      "调用函数结束：runParse",
      "为什么打：payload 符合 schema（query 非空 / action 枚举 / 数字正整数）；记 data 字段核对 default 是否生效（action 应是 search）。",
      {
        返回值: {
          parseOk: true,
          value: safe.data,
  ...
  logger.warn(
    "│ 契约-runParse",
    "调用函数结束：runParse（失败）",
    "为什么打：payload 不符 schema；走 /api/repair 路径，issues 已收集好可拼 repair prompt。warn 是「业务失败但能走通」的等级。",
    {
      返回值: {
        parseOk: false,
...
export function runRepair(payload: unknown) {
  ...
  logger.info(
    "│ 契约-runRepair",
    "调用函数开始：runRepair",
    "为什么打：route 只认这一层返回的 { success, data/issues, repairPrompt }；本条不调 LLM，只是把 issues 拼成可喂回模型的修复提示。当前：先跑一遍校验拿 issues 再拼 repair prompt。",
    {
      入参: { payloadType: typeof payload, payloadKeys: payload && typeof payload === "object" ? Object.keys(payload) : null },
      __code: "const safe = Intent.safeParse(payload);",
    },
  ...
  if (safe.success) {
    logger.info(
      "│ 契约-runRepair",
      "调用函数结束：runRepair",
      "为什么打：payload 已经合法；不拼 repair、记 note 让前端知道要换 SAMPLE_BAD 才有 issues 可拼。",
      {
  ...
  logger.warn(
    "│ 契约-runRepair",
    "调用函数结束：runRepair（失败）",
    "为什么打：详列每条 issue 的 path/code/message；这些就是 repair prompt 要喂回去的内容。",
    {
      返回值: {
        success: false,
        issues: issuesOf(safe.error),
...
export function runTransform(payload: unknown) {
  ...
  logger.info(
    "│ 契约-runTransform",
    "调用函数开始：runTransform",
    "为什么打：route 只认这一层返回的 Enriched 对象；本条不调 LLM，是 Zod.transform 的演示。当前：先 Intent 校验（补 action default），再加 repaired / when。",
    {
      入参: { payloadType: typeof payload, payloadKeys: payload && typeof payload === "object" ? Object.keys(payload) : null },
  ...
  logger.info(
    "│ 契约-runTransform",
    "调用函数结束：runTransform",
    "为什么打：transform 成功——对比原 payload 看多了哪些字段（repaired: true / when: ISO 时间戳）；记原 vs 增便于核对 schema 改结构的能力。",
    {
      返回值: { enriched },
      __code: "const enriched = Enriched.parse(payload)",
      耗时ms: Date.now() - t0,
