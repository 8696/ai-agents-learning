 ...
 * 数据流：GET /health → 页脚 #env-info（本地计算 · 不调 LLM）；pathname → PageNav。
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/parse.html", label: "parse / safeParse", match: "parse" },
    { href: "/pages/repair.html", label: "issues → repair", match: "repair" },
    { href: "/pages/transform.html", label: "transform", match: "transform" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/parse.html")) return "parse";
    if (p.endsWith("/repair.html")) return "repair";
    if (p.endsWith("/transform.html")) return "transform";
    return "";
  }
  ...
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50401} · 本地计算（不调 LLM）· provider{" "}
