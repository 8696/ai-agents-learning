 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostTransform 封装层）；
 *   闸门挡掉单独打 warn；ZodError 校验失败单独打 warn（业务失败但能走通）；子调用 runTransform 内部已自带五件套。
 */
...
export function mountTransformRoutes(router: Router): void {
  router.post("/api/transform", (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 { value: Enriched }；里面 runTransform 是「真活」（Zod.transform 演示）。当前：前端点了 transform 按钮；记入参便于核对 default 补全与新增字段。",
      {
    ...
    if (payload === null) {
      logger.info(
        ...
        "为什么打：闸门已回 400；route 不用再调 runTransform。当前：readJsonPayload 已返回 null（闸门在内部写过 warn），route 直接 return。",
        {
    ...
    try {
      ...
      logger.info(
        "api.transform",
        "调用函数结束：handlePostTransform",
        "为什么打：route 要把 { value: Enriched } 写进 ctx.body 交给页面 stats 区；记是否带 repaired / when 字段。当前：runTransform 已返回。",
        {
      ...
      if (error instanceof z.ZodError) {
        logger.warn(
          ...
          "为什么打：transform 前的 Intent 校验没过；issues 一起打出来便于核对哪条 field 不合法（与 /api/parse 应一致）。warn 是「业务失败但能走通」的等级。",
          {
      ...
      logger.error(
        "api.transform",
        "调用函数结束：handlePostTransform（失败）",
        "为什么打：transform 入口抛了非 ZodError；这通常是程序 bug 不是 schema 问题，记 err 便于排错。",
        {
          返回值: { httpStatus: 500 },
          耗时ms: Date.now() - tHandlerStart,
          错误: error,
