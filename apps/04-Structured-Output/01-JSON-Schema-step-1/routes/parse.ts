 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostParse 封装层）；
 *   闸门挡掉单独打 warn；子调用 runParse 内部已自带五件套。
 */
...
export function mountParseRoutes(router: Router): void {
  router.post("/api/parse", (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 { parseOk, value, safeParse }；里面 runParse 是「真活」（Zod 校验演示）。当前：前端点了 parse 按钮；记录入参路径便于核对。",
      {
    ...
    if (payload === null) {
      logger.info(
        ...
        "为什么打：闸门已回 400；route 不用再调 runParse。当前：readJsonPayload 已返回 null（闸门在内部写过 warn），route 直接 return。",
        {
    ...
    logger.info(
      "api.parse",
      "调用函数结束：handlePostParse",
      "为什么打：route 要把 runParse 的结果写进 ctx.body 交给页面；记 parseOk / issuesCount 便于核对「同一份 payload 的 parse vs safeParse」差异。当前：runParse 已返回。",
      {
