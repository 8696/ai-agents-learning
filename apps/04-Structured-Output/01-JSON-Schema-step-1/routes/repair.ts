 ...
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostRepair 封装层）；
 *   闸门挡掉单独打 warn；子调用 runRepair 内部已自带五件套。
 */
...
export function mountRepairRoutes(router: Router): void {
  router.post("/api/repair", (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 { success, issues, repairPrompt }；里面 runRepair 是「真活」（把 issues 拼成可喂回模型的修复提示）。当前：前端点了 repair 按钮；记录入参路径便于核对 prompt 是否真的写进了 issues。",
      {
    ...
    if (payload === null) {
      logger.info(
        ...
        "为什么打：闸门已回 400；route 不用再调 runRepair。当前：readJsonPayload 已返回 null（闸门在内部写过 warn），route 直接 return。",
        {
    ...
    logger.info(
      "api.repair",
      "调用函数结束：handlePostRepair",
      "为什么打：route 要把 runRepair 的结果写进 ctx.body 交给页面；记 success / issuesCount / hasRepairPrompt 便于核对「payload 合法时是否带 note」。当前：runRepair 已返回。",
      {
