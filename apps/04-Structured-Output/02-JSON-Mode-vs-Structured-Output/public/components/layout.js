 ...
 * 为什么单独成文件：这几个是「每一页都长一样」的壳，业务卡片在 mode-cards.js。
 *
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/json-mode.html", label: "JSON Mode", match: "json-mode" },
    { href: "/pages/structured.html", label: "Structured", match: "structured" },
    { href: "/pages/strict-rejected.html", label: "坏 schema 400", match: "strict-rejected" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/json-mode.html")) return "json-mode";
    if (p.endsWith("/structured.html")) return "structured";
    if (p.endsWith("/strict-rejected.html")) return "strict-rejected";
    return "";
  }
  ...
  function EnvFooter({ env, protocol }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50402} · 协议 {protocol || "A"} · provider{" "}
