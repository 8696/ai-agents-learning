...
| `yarn app:07-01-agent-loop-react-step-1` | `50049` | 模块 07 · 01 · Agent Loop / ReAct step-1：手写 while 跑一整轮 Loop + 副作用三栏可见（before / after / diff）；停止条件仅变体 J；MAX 兜底 |
...
| `yarn app:07-02-plan-vs-step-step-1` | `50053` | 模块 07 · 02 · 先规划再执行 vs 一步步走 step-1：双栏对照（mock 模型 + mock 工具 · 不调真 LLM）—— 一步步走 7 圈 vs 先规划 1 次 + 6 步执行；关键差异卡 3 行：模型调用次数（A=N / B=1）/ 第一次 Act 前等待（A=0 / B=1）/ 计划作为对象（A ❌ / B ✅）；step-1 sketch |
| `yarn app:07-02-plan-vs-step-step-2` | `50054` | 模块 07 · 02 · 先规划再执行 vs 一步步走 step-2：step-2 全真模型 —— A 路径每圈真调一次模型（最多 MAX_ROUNDS=8）+ B 路径真规划器一次吐清单；公平对照节奏差异；plannerFallback=true 时 B 解析失败回退 mock plan + 标 ⚠ 兜底；step-2 sketch |
| `yarn app:07-02-plan-vs-step-step-3` | `50055` | 模块 07 · 02 · 先规划再执行 vs 一步步走 step-3：step-3 加重规划 —— B 路径执行阶段每步后 shouldReplan 检测关键变化（默认：所有 SKU query_stock 库存 0）→ 触发就再调真规划器吐 v2 plan；plans 数组从 1 个变 2 个；executeTrace 每步带 planVersion；UI 能看见「计划 v1（已废灰卡）→ 计划 v2（绿卡）」；step-3 sketch |
| `yarn app:07-02-plan-vs-step-step-4` | `50056` | 模块 07 · 02 · 先规划再执行 vs 一步步走 step-4：短任务 vs 长任务对照 —— 同一接口跑 4 组轨迹（短 A「把 todo-001 标完成」/ 短 B / 长 A「春季上新」/ 长 B）；变体 D「什么时候不值得先规划」实证：短任务 B 多付 1 次规划（不值得）/ 长任务 B 更省模型调用（值得）；数字对照卡直接给「值得 vs 不值得」判断；step-4 sketch |
| `yarn app:07-02-plan-vs-step-step-5` | `50057` | 模块 07 · 02 · 先规划再执行 vs 一步步走 step-5：变体 E 演示 —— mock write_copy 第 1 次调用强制返回 `{ok:false, error:'mock 系统挂了'}`；演示 shouldReplan 不看 ok=false → 工具失败但不重规划 → 按旧清单继续做错（通知运营「文案已完成」假数据）；UI 执行卡标黄「⚠ 变体 E 触发」；对照卡新增「工具失败次数 / 已重规划」；step-5 sketch |
| `yarn app:07-02-plan-vs-step-step-6` | `50058` | 模块 07 · 02 · 先规划再执行 vs 一步步走 step-6：变体 F 演示 —— 两阶段：GET /api/compare 只 plan 不执行（status=pending）→ 用户点「✅ 确认执行」按钮 → POST /api/confirm-plan 才执行（status=executed）；UI 状态机「⏸ 待确认」→「✅ 已确认 + 已执行」；planStage 走 idle → planned → executed；确认前无副作用 —— 没点按钮 = 没执行；step-6 sketch |
