...
| `yarn app:10-02-write-policy-step-1` | `50104` | 模块 10 · 02 · 写入策略 step-1：写入策略八关里的「提取」——一段对话原文调模型抽取 0~N 条结构化候选事实（key/value/type/confidence/source/validUntil），system 里传今天日期换算相对时间；候选清单卡片区分语义/情景两类（POST /api/extract / POST /api/force-error） |
| `yarn app:10-02-write-policy-step-2` | `50105` | 模块 10 · 02 · 写入策略 step-2：在 step-1 提取之上加第 3 关「把关」的置信度阈值一道闸门；阈值滑块（0.0~1.0）控判 confidence >= threshold 通过 / BELOW_THRESHOLD 拦下；保留 /api/extract 作对照基线（POST /api/filter / POST /api/extract / POST /api/force-error） |
| `yarn app:10-02-write-policy-step-3` | `50106` | 模块 10 · 02 · 写入策略 step-3：在 step-2 置信度之上加第 3 关「把关」的内容维度 A「是不是事实」+ B「跨会话还有用吗」两道闸门；维度开关 enableA / enableB 默认都开；任一不过标 PROGRAMMATIC_RULE（维度 A）/ SESSION_ONLY（维度 B）；保留 step-1 + step-2 的两个对照接口（POST /api/filter-dimensions / POST /api/filter / POST /api/extract / POST /api/force-error） |
| `yarn app:10-02-write-policy-step-4` | `50107` | 模块 10 · 02 · 写入策略 step-4：在 step-3 维度 A/B 之上加第 3 关「把关」的最后一刀——内容维度 C「值不值得占存储」一道闸门；维度开关 enableA / enableB / enableC 默认都开；不过标 PUBLIC_KNOWLEDGE（公开常识该走 RAG 不进个人库 / 随口感慨）；把关这一关 3 把语义判定刀做齐（POST /api/filter-dimensions 接 enableC 参数 / POST /api/filter / POST /api/extract / POST /api/force-error） |
| `yarn app:10-02-write-policy-step-5` | `50108` | 模块 10 · 02 · 写入策略 step-5：在 step-4 之上加第 3 关「把关」的最后一刀——敏感信息过滤（个人身份信息 PII / 合规）一道闸门；按学习者要求用模型判（合并进现有那次调用），schema 加 isPii + piiType（id_card / bank_card / phone / health / religion / address）字段；不过标 PII_DETECTED；优先级 P > A > B > C；reasoning 必须说明属于哪一类（POST /api/filter-dimensions 接 enablePii / POST /api/filter / POST /api/extract / POST /api/force-error） |
| `yarn app:10-02-write-policy-step-6` | `50109` | 模块 10 · 02 · 写入策略 step-6：把关这一关最后一刀——人工确认（变体 3-D）。在 step-5 之上加三档分档：conf >= highThreshold（默认 0.8）自动通过 / midThreshold（默认 0.5）≤ conf < highThreshold 攒进「待确认」区等用户点 [记住] / [不用] / conf < midThreshold 已被 step-2 拦下。POST /api/filter-dimensions 接 midThreshold + highThreshold；POST /api/confirm 处理用户的 [记住] / [不用] 调用；攒起来状态用内存数组模拟（重启清空，生产再换 SQLite）。保留前几步对照接口 |
| `yarn app:10-02-write-policy-step-7` | `50110` | 模块 10 · 02 · 写入策略 step-7：把把关这一步延伸到第 4 关「去重」——变体 4-A 字面完全相同 → NOOP（不重复写）。新建 lib/db.ts（§5.3.17 通用 KV 抽象 kvGet / kvSet / kvDel / kvList + SQLite via better-sqlite3 + data/facts.db）；confirm [记住] 后调 dedupByKey 查重 + kvSet 写入；GET /api/facts 列出事实库。从「过滤」到「真正写」的质变；同原文 + 同样分档点两次 [记住] 验证 NOOP |
| `yarn app:10-02-write-policy-step-8` | `50111` | 模块 10 · 02 · 写入策略 step-8：把把关这一步延伸到第 5 关「冲突与更新」——笔记 §5 四个动作（NEW / UPDATE / MERGE / DELETE / NOOP）+ 三种旧值去向（进历史 / 软删除 / 真删接口）。继承 step-7 的 lib/db.ts + 加 deleted_at 字段 + 新增 fact_history 表；本步核心 lib/flow/resolve-conflict.ts 单独成文件；intent 让前端/用户表达不调模型判意图；4 sub-page（actions / merge / soft-delete / history）+ PageNav 共享事实库 |
...
| `yarn app:12-03-skills-vs-mcp-step-1` | `50136` | 模块 12 · 03 · Skills vs MCP step-1：同一句点单三条装配对照（只有 MCP / 只有技能 / 两边都有）；固定剧本，不调大模型 |
| `yarn app:12-03-skills-vs-mcp-step-2` | `50137` | 模块 12 · 03 · Skills vs MCP step-2：分类题（apply_latte_policy 应判进哪 + 技能里塞 HTTP+Token 应判进哪）+ 三种提示词入口（MCP 提示词模板 / 技能 / 系统提示词 三栏对照）；固定剧本，不调大模型（三装配 + system prompt 装载见 step-1 端口 50136）|
| `yarn app:12-03-skills-vs-mcp-step-3` | `50138` | 模块 12 · 03 · Skills vs MCP step-3：10 个技能短目录 + 三种装载模式对照（full 全加载 / catalog-only 仅短目录 / on-demand 短目录 + 命中加载）；固定剧本，不调大模型（分类题 + 三种提示词入口见 step-2 端口 50137）|
