...
/**
 * 跳过精排 —— 直接把粗召回 RRF 榜前 K 条当成最终 K。
 *
 * 本步教学点（变体 12 / 需求 10）：库很小 / 延迟极紧 / 评测无增益时，
 * 关掉精排是合理的。关掉时最终 Top-K 与粗召回前 K 条同序（同 RRF 名次），
 * 精排分用 RRF 分数归一化到 0~1（保持 rawScores 字段形状一致）。
 *
 * 不调网络：不发模型请求，纯本地计算。
 */
export async function skipRerank(input: RerankInput): Promise<RerankResult> {
  if (!input.query.trim()) {
    throw new HttpError(400, "query 是空的", "输入框写一句再点精排");
  }
  ...
  const t0 = Date.now();
  const maxRrf = Math.max(...input.candidates.map((c) => c.rrfScore), 0);
  // rawScores = 候选的 RRF 分数归一化到 0~1（保持 rawScores 字段有内容，
  // 让前端能展示「开 vs 关」的对照；不开时这一栏是 RRF 归一化分，不是模型给的）。
  const rawScores = input.candidates.map((c) =>
    maxRrf > 0 ? c.rrfScore / maxRrf : 0,
  );

  // rows 直接是 candidates（前 K 条按 RRF 原序），rerankScore 用归一化 RRF 分
  const rows = input.candidates.map<RerankRow>((c, i) => ({
    cardId: c.cardId,
    text: c.text,
    rerankScore: rawScores[i]!,
    rank: i + 1,
