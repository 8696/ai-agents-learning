/**
 * 职责：内置一份售后知识库（6 个切块）——本 demo 用来演示「同一库、同一问句，
 ...
 *
 * 数据流：业务读 CORPUS 拿到切块 id/text；向量侧把 text 送嵌入模型拿向量；
 *         BM25 侧按词算分。
 */
export type KnowledgeCard = {
  ...
  text: string;
  /** 类别：policy=政策 / product=产品 / sku=含货号的切块 / error=错误码 */
...
/** 给前端讲解用：哪个切块是「该命中的切块」——带货号的问句期望命中 sku-8821；日常说法、库里未必同词的问句期望命中 shipping-damage */
