...
import { mountEvalSetRoutes } from "./routes/eval-set.js";
import { mountEvaluateRoutes } from "./routes/evaluate.js";
import { mountForceErrorRoutes } from "./routes/force-error.js";
import { mountGenerateRoutes } from "./routes/generate.js";
...
mountEvalSetRoutes(router);
mountEvaluateRoutes(router);
mountGenerateRoutes(router);
...
app.listen(PORT, "127.0.0.1", () => {
  ...
  logger.info(
    "server.start",
    "调用函数开始：listen",
    "为什么写这条日志：服务起那一刻就要写日志，烟雾测试只看 logs/ 当天文件。当前：step-3 评测集 + 生成侧演示已监听。",
