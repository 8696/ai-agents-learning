/**
 * 职责：按当前提供商给文本算向量。MiniMax 不是 OpenAI 的 input 格式。
 * 建库 type=db，提问 type=query —— 同一模型、两边约定必须一致。
 * 智谱 / 千问走 embeddings.create，必须显式 encoding_format=float：
 * OpenAI SDK 默认按 base64 解码，智谱仍返回小数数组，会被解成全 0。
 *
 * 为什么放在本 Demo（不 import 模块 08 step-4）：
 *  按 AGENTS.md §2「小节不互相 import」。本 Demo 自己维护一份精简版。
 */
...
function parseVectors(raw: unknown): number[][] {
  if (!raw || typeof raw !== "object") {
    throw new HttpError(502, "嵌入接口返回不是对象");
  }
  ...
  if (baseResp && baseResp.status_code != null && baseResp.status_code !== 0) {
    throw new HttpError(
      502,
      `嵌入接口失败：${baseResp.status_msg ?? "unknown"}（status_code=${baseResp.status_code}）`,
    );
  }
  ...
  throw new HttpError(502, "嵌入接口没有 vectors / data 字段");
}
...
async function embedMiniMax(llm: Llm, texts: string[], kind: EmbedKind): Promise<number[][]> {
  ...
  try {
    ...
    throw new HttpError(response.ok ? 502 : response.status, "嵌入接口返回不是 JSON");
  }
  if (!response.ok) {
    throw new HttpError(response.status, `嵌入 HTTP 失败：${response.status}`);
  }
  return parseVectors(json);
}
...
async function embedOpenAiCompat(llm: Llm, texts: string[]): Promise<number[][]> {
  ...
  const response = await llm.openai.embeddings.create(payload);
...
export async function embedTexts(llm: Llm, texts: string[], kind: EmbedKind): Promise<number[][]> {
  if (!llm.embeddingModel) {
    throw new HttpError(
      400,
      "当前提供商没有嵌入模型（Embedding Model）。填 LLM_EMBEDDING_MODEL，或把 LLM_PROVIDER 换成 minimax / zhipu / qwen",
    );
  }
...
function validateVectors(vectors: number[][], expectedLen: number): void {
  ...
  if (vectors.some((row) => !Array.isArray(row) || row.length === 0)) {
    throw new HttpError(502, "有一条向量是空的");
  }
  if (vectors.some(isAllZero)) {
    throw new HttpError(
      502,
      "有一条向量全是 0 —— encoding_format 没生效？改显式 encoding_format=float",
    );
  }
}