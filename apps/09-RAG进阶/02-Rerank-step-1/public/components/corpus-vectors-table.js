 ...
 *   cached=true ：每张卡一个小块（正文 + 前 8 维 + 范数）
 */
...
window.DemoUI.CorpusVectorsTable = function CorpusVectorsTable({ data }) {
  ...
  if (!data.cached) {
    return React.createElement(
      ...
      "向量还没嵌入。先点上面「① 跑粗召回」建一次缓存（每张卡跑一次嵌入模型），再点「查看语料向量」就能看到浮点数。",
    );
  }
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      "每张卡一行：上方是卡片正文（实际送进嵌入模型的全文），下方是它对应的前 8 维浮点 + L2 范数。完整维度回页面会爆（动辄上千），所以只看前 8 维当样子。",
    ),
    React.createElement(
      "div",
