/**
 * 职责：把"粗召回榜"和"精排榜"并排展示 + 显示名次跳动 + 把卡片正文一起露出来。
 *
 * 挂载：window.DemoUI.RecallVsRerank
 *
 * 入参：recall = RrfSearchResult 形状（含 rows: RrfRow[]，每行 .text = 卡片正文）
 *       rerank = RerankResult 形状（含 rows: RerankRow[]，每行 .text = 卡片正文）
 * 出参：两块对照榜 + 名次跳动小条
 */
...
window.DemoUI.RecallVsRerank = function RecallVsRerank({ recall, rerank }) {
  ...
  // 一行卡片块：header 行（rank/id/分数/徽标） + body 行（正文 <pre>）
  ...
  return React.createElement(
    ...
    rerank
      ? React.createElement(
          ...
          " 名次差 = 粗召回原 rank − 精排新 rank；正值 = 升、负值 = 降；同一张卡的余弦/BM25 分数没动，但精排读了问句和正文之后名次会跳——这就是两阶段分工。",
        )
      : null,
  );
};