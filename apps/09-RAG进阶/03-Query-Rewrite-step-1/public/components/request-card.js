/**
 * 职责：请求参数卡、调用流程卡、失败红字、发给大模型的提示词卡。对应页面必须有的三件：参数 / 流程 / 结果。
 * 数据流：lastRequest / steps / error / modelCall → 中性灰 / 过程卡 / 红卡 / 右栏模型入参。
 */
(function () {
  ...
  function RequestCard(props) {
    ...
    if (!req) {
      return <div className="text-sm text-gray-500">还没有发出检索请求。点上面的按钮后，这里显示你发出去的 URL 和 body。</div>;
    }
    return (
    ...
    if (!call) {
      return (
        <div className="border border-dashed border-gray-300 rounded p-2 text-xs text-gray-500">
          还没点第二步。点了之后，这里展示发给大模型的提示词（Prompt）和参数：模型（model）、温度（temperature）、系统消息（system）、用户消息（user）。
        </div>
      );
    }
