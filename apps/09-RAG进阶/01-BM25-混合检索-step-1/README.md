...
- **同一库 6 个售后切块**：含货号的切块 2 个（SKU-8821 / SKU-8822）+ 通用政策切块 3 个（路由器保修 / 运输破损 / 发票）+ 错误码切块 1 个（ERR-4401）。
- **两个对照实验**：带货号的问句「SKU-8821 保修几年？」 + 日常说法、库里未必同词的问句「杯子裂了怎么退？」，每个实验可改问句。
- **每个实验 4 次独立请求**：向量侧一次 + BM25 侧一次（共 4 次/会话完整跑）。
- **两侧对照面板**：左侧「向量侧 · 余弦相似度」+ 右侧「BM25 侧 · 关键词打分」，每个切块显示 Top-K 排名 + 分数 + 命中词 + 「该命中的切块 ✓」徽标。
- **失败可读**：4xx（空输入 / JSON 错）+ 5xx（嵌入接口失败）—— 状态栏 + 面板状态栏标 ❌ + 服务端 logs/ 当当日文件里完整错误对象。
