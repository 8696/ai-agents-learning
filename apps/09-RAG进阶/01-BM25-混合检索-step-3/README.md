...
- **按问句偏置权重（变体 7）**：正则检测 `/[A-Z]+[-_]?\d+/` 模式 → 带货号的问句 → 建议 α 小（如 0.2）；纯日常说法 → 建议 α 大（如 0.8）；两者都有 → 中（0.5）。
- **RRF vs 加权对照**：#output 区加一张「RRF vs 加权排名对照」面板，跑两次（一次加权一次 RRF）→ 看排名差异。
- **偏置演示面板**：#output 区加一张「按问句偏置」自动检测面板，跑一次显示「检测结果 + 建议 α + 触发原因」。
