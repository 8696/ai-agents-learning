/**
 * 职责：POST /api/reasoning-anthropic。Anthropic 协议下推理提取。
 *
 * 数据流：body.{ query, provider } → pipeReasoning(..., "anthropic") → ctx.res。
...
export function mountReasoningAnthropicRoutes(router: Router): void {
  router.post("/api/reasoning-anthropic", async (ctx: Context) => {
    ...
    ctx.respond = false;
    try {
      await pipeReasoning(parsed.data.query, provider, "anthropic", ctx.res, abort.signal);
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
      logger.error("reasoning.route.anthropic", "结束：pipeReasoning（失败）", "推流失败。", {
