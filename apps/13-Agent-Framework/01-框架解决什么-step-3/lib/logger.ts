/**
 * 职责：本 Demo 的服务端日志（完整拷贝顶层模板，禁止运行时 import apps/logger.ts）。
 *
 * 数据流：createLogger(logDir) → logger.info(scope, msg, explain, data?)
 *   → 文件 appendFileSync(logDir/{YYYY-MM-DD}.log)
 *   → 每条前空一行；msg / explain / data 之间也空一行；入参与返回值全文，不截断。
 */
