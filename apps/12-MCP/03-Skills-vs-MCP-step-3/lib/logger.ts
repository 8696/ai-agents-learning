/**
 * 本地 freeze 副本 · 拷自顶层 2026-09-19。
 * 职责：本演示自己的服务端日志；禁止运行时 import apps/logger.ts。
 * 数据流：createLogger(本演示 logs/) → appendFileSync(logs/{YYYY-MM-DD}.log)。
 * 路径必须是 path.resolve(__dirname, "..", "logs")，不能用 import.meta.url 落到 lib/logs/。
 *
 * 数据流：
 *   createLogger(logDir)
 *     → mkdir -p logDir
