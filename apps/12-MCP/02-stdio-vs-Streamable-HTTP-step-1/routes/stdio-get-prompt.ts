/**
 * 职责：POST /api/stdio/get-prompt —— 触发 MCP 协议 prompts/get。
 * 数据流：校验 name + arguments → getPrompt(name, args) → ctx.body
 */
import type { Context } from "koa";
import type Router from "@koa/router";
import { z } from "zod";
import { getPrompt } from "../lib/flow/stdio-prompts.js";
...
export function mountStdioGetPrompt(router: Router): void {
  router.post("/api/stdio/get-prompt", async (ctx: Context) => {
