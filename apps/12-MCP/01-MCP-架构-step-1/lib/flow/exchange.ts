...
function duties(method: McpMethod): { agentDid: string; serverDid: string } {
  ...
  if (method === "tools/list") {
    return {
      ...
      serverDid: "吧台把工具列表发回来：上面有一杯「拿铁」。看见名字还不等于已经做出来。",
    };
  }
  ...
  if (method === "resources/list") {
    return {
      agentDid: "小程序问吧台：你还备了什么资料？菜单、过敏原说明都在吗？",
      serverDid: "吧台把资料清单发回来：今日菜单、过敏原说明各占一条。看见清单还不等于已经把资料正文读到手里。",
    };
  }
  if (method === "resources/read") {
    return {
      agentDid: "小程序替客人取一份资料（用 URI 去拿，比如 cafe://today-menu）。",
      serverDid: "吧台把这份资料的正文发回来：今日菜单的内容 / 过敏原说明的内容。",
    };
  }
