 ...
 * 数据流：PORT → routes → public/pages/compare.html → listen 127.0.0.1
 *
 * 本步只挂三原语对照用到的 route：initialize + tools/call + resources/read + prompts/get。
 * 不挂 list route —— 对照页硬编码 URI / name，不需要 list 步骤。
 */
...
app.listen(PORT, "127.0.0.1", () => {
  const t0 = Date.now();
  logger.info("server.start", "调用函数开始：listen", "服务起来就要写文件日志。step-2 教学要点：三原语同一轮对照。", {
    入参: { PORT, url },
  });
