 ...
 * 数据流：{ prompt?, runs?, temperature? } → 校验 → flow/run-sweep → SweepResponse。
 * 为什么两个端点同一个文件：它们是同一套流程的两个轴（固定一个旋钮、扫另一个），
 *   参数校验、日志、错误处理完全相同；分成两个文件只会得到两份会漂移的复制品。
 *
 * 日志（§5.3.16）：调用函数 五条日志（handlePostSweepTemperature / handlePostSweepTopP 封装层）；
...
export function mountSweepRoutes(router: Router): void {
  ...
  router.post("/api/sweep/temperature", async (ctx: Context) => {
    ...
    if (!llm) {
      logger.info(
        ...
        "POST /api/sweep/temperature 被无 Key 校验挡掉",
        "为什么写这条日志：服务端兜底；没 Key 时参数对不对根本不重要。当前：apps/.env 当前 LLM_PROVIDER 无 Key。",
    ...
    if (!body) {
      logger.info(
        "api.sweep.temperature",
        "POST /api/sweep/temperature 被入参校验挡掉",
        "为什么写这条日志：校验挡下没花模型额度也没走到 runTemperatureSweep。当前：body 不合法。",
        { endpoint: "POST /api/sweep/temperature" },
      );
      return;
    }
    ...
    logger.info(
      ...
      "为什么写这条日志：route 只认这一层返回的 SweepResponse；里面 runTemperatureSweep 是真正干活的那一层。当前：扫温度即将开始；Top-P 固定 1。",
      {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么写这条日志：route 要把 SweepResponse（axis=temperature, 三档 verdict）写进 ctx.body 交给页面。当前：runTemperatureSweep 已返回。",
        {
          返回值: {
            axis: result.axis,
            fixed: result.fixed,
      ...
      logger.error(
        ...
        "为什么写这条日志：runTemperatureSweep 抛错说明整轮都没跑起来（单次失败已在 callOnce 内转成 SingleRun.error）；记 err 便于写 writeUpstreamError。当前：已交给 writeUpstreamError 写统一错误响应。",
        {
  ...
  router.post("/api/sweep/top-p", async (ctx: Context) => {
    ...
    if (!llm) {
      logger.info(
        "api.sweep.top-p",
        "POST /api/sweep/top-p 被无 Key 校验挡掉",
        "为什么写这条日志：服务端兜底；没 Key 时参数对不对根本不重要。当前：apps/.env 当前 LLM_PROVIDER 无 Key。",
    ...
    if (!body) {
      logger.info(
        "api.sweep.top-p",
        "POST /api/sweep/top-p 被入参校验挡掉",
        "为什么写这条日志：校验挡下没花模型额度也没走到 runTopPSweep。当前：body 不合法。",
        { endpoint: "POST /api/sweep/top-p" },
      );
      return;
    }
    ...
    logger.info(
      ...
      "为什么写这条日志：route 只认这一层返回的 SweepResponse；里面 runTopPSweep 是真正干活的那一层。当前：扫 Top-P 即将开始；温度由调用方给（默认 1；选 0 验证贪心解码下 Top-P 不起作用）。",
      {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么写这条日志：route 要把 SweepResponse（axis=top_p, 三档 verdict）写进 ctx.body 交给页面。当前：runTopPSweep 已返回。",
        {
          返回值: {
            axis: result.axis,
            fixed: result.fixed,
      ...
      logger.error(
        ...
        "为什么写这条日志：runTopPSweep 抛错说明整轮都没跑起来（单次失败已在 callOnce 内转成 SingleRun.error）；记 err 便于写 writeUpstreamError。当前：已交给 writeUpstreamError 写统一错误响应。",
        {
