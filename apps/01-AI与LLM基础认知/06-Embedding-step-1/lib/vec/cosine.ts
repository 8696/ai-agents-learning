 ...
 *
 * 日志（§5.3.16）：本条是主路径（余弦公式本身）；五条日志（含 __code）仍要，零向量抛错前写 warn。
 */
...
export function cosine(a: Vec, b: Vec): number {
  ...
  logger.info(
    "││ 余弦实现-cosine",
    "调用函数开始：cosine",
    "为什么写这条日志：本条主路径——余弦公式只在 ranks/正例/零向量对照里被调用，不写就看不见「分母是 na*nb」。当前：即将算 dot / na / nb。",
    {
      入参: { a, b },
  ...
  if (na === 0 || nb === 0) {
    ...
    logger.warn(
      "││ 余弦实现-cosine",
      "调用函数结束：cosine（失败）",
      "为什么写这条日志：分母为 0 → 抛「零向量没有方向」让 route 回 400；warn 是「业务失败但能走通」的等级（§5.3.16 三档）。当前：即将 throw，rank.ts 的 catch 会接住。",
      {
        返回值: undefined,
        a,
        b,
  ...
  logger.info(
    "││ 余弦实现-cosine",
    "调用函数结束：cosine",
    "为什么写这条日志：分数就是 route 拿去排序的键，不写就丢了一个数。当前：返回 score = dot/(na*nb)。",
    {
      返回值: score,
      耗时ms: Date.now() - t0,
      字段释义: {
