...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/token-id.html", label: "Token ID 反例", match: "token-id" },
    { href: "/pages/cosine.html", label: "余弦正例", match: "cosine" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/token-id.html")) return "token-id";
    if (p.endsWith("/cosine.html")) return "cosine";
    return "";
  }
      ...
      fetch("/health")
        ...
        .then(function (h) {
          setEnv({
            ...
            words: h.words || [],
            queryDefault: h.queryDefault || "宠物",
  ...
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50003} · 本地计算（不调 LLM）· provider{" "}
