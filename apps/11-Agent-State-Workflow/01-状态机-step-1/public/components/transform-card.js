 ...
 * 数据流：step 接口返回的 transition；建工单时显示入口快照。
...
function TransformCard(props) {
  ...
  if (!lastCall && !transition && !created) {
    return <div className="text-xs text-gray-500">还没有发生转移。发送问句或点「走一步」后，这里只显示刚刚那一次。</div>;
  }
  return (
