 ...
 * 数据流：不走图，直接 500。用来对照「业务 400」和「后端挂了」。
 * 为什么单独成文件：这是独立业务 URL，不能塞进 start / step。
 */
...
export function mountForceErrorRoutes(router: Router): void {
  router.get("/api/force-error", (ctx: Context, _next: Next) => {
    logger.error(
      ...
      "为什么写这条日志：这是第二类失败通道（5xx），不是空问句那种 400。当前：故意失败。",
      { 入参: {}, __code: "ctx.status = 500" },
    );
    ctx.status = 500;
    ctx.body = { ok: false, error: "这是故意的后端 5xx，用来和第二类失败（空入参 400）对照。" };
    logger.error(
      "GET /api/force-error",
      "调用函数结束：GET /api/force-error（失败）",
      "为什么写这条日志：页面要把 #status-pill 打成红色。当前：已写 500。",
      { 返回值: ctx.body, 耗时ms: 0 },
    );
  });
}
