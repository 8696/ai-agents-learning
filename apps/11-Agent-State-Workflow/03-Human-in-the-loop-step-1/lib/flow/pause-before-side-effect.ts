...
// 让路由文件继续 `import { proposeTransfer, approveTransfer, rejectTransfer, readBalance, editTransferPending, isReadOnly } from "./pause-before-side-effect.js"`；
// 这些函数在本文件 / ./transfer-reject.ts / ./transfer-read.ts / ./transfer-edit.ts 各一份。
...
export function approveTransfer(runId: string): RunSnapshot {
  const t0 = Date.now();
  logger.info(
    "调用函数-approveTransfer",
    "调用函数开始：approveTransfer",
    "为什么写这条日志：这是本步主路径的第二下——人点头之后才允许副作用。当前：刚收到人的通过。",
    { 入参: { runId, 当前快照: cloneSnapshot() }, __code: "ledger = executeTransfer(ledger, pending.args); // 只在这时改 ledger" },
  );
  ...
  const ledgerNext: Ledger = executeTransfer(ledgerBefore, pendingNow.args);
  setLedger(ledgerNext);
  setPending({ ...pendingNow, status: "executed" });
  setCurrentNode("executed");
  setLastEvent(
    "人已通过，已调用 executeTransfer 一次。余额变成 " +
      ledgerNext.balance +
      "，调用次数变成 " +
      ledgerNext.transferCallCount +
      "。",
  );
  const result = cloneSnapshot();
  logger.info(
    ...
    "为什么写这条日志：对比提议前后的次数，才能看见「点头后才发生」。当前：pending.status=executed。",
    {
      返回值: result,
      耗时ms: Date.now() - t0,
      字段释义: {
        "pending.status": "executed = 人已同意且扣款函数已跑过",
        "ledger.transferCallCount": "应为 1；若提议阶段就已经 1，说明先做后弹窗",
      },
    },
  );
  return result;
