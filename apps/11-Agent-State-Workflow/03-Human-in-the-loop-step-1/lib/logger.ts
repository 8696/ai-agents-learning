/**
 * 本地 freeze 副本 · 拷自顶层 2026-09-18。
 * 职责：本演示自己的文件日志。业务只 import { logger } from 本文件。
 * 数据流：createLogger → apps/11-Agent-State-Workflow/03-Human-in-the-loop-step-1/logs/{日期}.log
 * 为什么单独成文件：禁止运行时委托顶层 apps/logger.ts。
 *
 * 数据流：
 *   createLogger(logDir)
 *     → mkdir -p logDir
