 ...
 *
 * 变体 F · forceFail：人通过 ≠ 执行成功。本 demo 用 forceFail=true 模拟「支付渠道挂了」（银行渠道宕机 / 余额不够等业务失败）。
 * 失败时 throw 一个特定 Error 让调用方写 failed 状态；账本不变、transferCallCount 不加。
 */
export function executeTransfer(currentLedger: Ledger, args: TransferArgs, forceFail: boolean = false): Ledger {
  const t0 = Date.now();
  logger.info(
    ...
    {
      入参: { args, 调用前余额: currentLedger.balance, 调用前次数: currentLedger.transferCallCount, forceFail },
      __code: "if (forceFail) throw new Error('模拟支付渠道失败'); if (args.amount > balance) throw; return newLedger;",
    },
  );

  // 变体 F：模拟渠道失败（demo 用 forceFail 触发；真实场景是支付渠道挂 / 银行返回失败码）。
  if (forceFail) {
    const err = new Error("支付渠道失败：模拟银行接口返回 500。");
    logger.error(
      "│ 调用函数-executeTransfer",
      "调用函数结束：executeTransfer（失败）",
      "为什么写这条日志：人通过了但渠道挂了——变体 F 的关键。当前：余额没动、次数没加。",
