 ...
 *   加载时校验 graphVersion:与进程内 currentGraphVersion 不一致 → 抛 GRAPH_VERSION_MISMATCH（变体 J）
...
import { getCurrentGraphVersion } from "./graph-version.js";
...
export function readCheckpoint(runId: string): CheckpointRecord | null {
  return logCall(
    ...
    () => {
      ...
      if (main) return stampVersion({ ...main, fellBackTo: null });
      ...
      for (const name of names) {
        ...
        if (rec) return stampVersion({ ...rec, fellBackTo: name });
      }
      return null;
    },
  );
...
function stampVersion(record: CheckpointRecord): CheckpointRecord {
  const current = getCurrentGraphVersion();
  if (record.graphVersion && record.graphVersion !== current) {
    throw Object.assign(
      new Error(
        `检查点 graphVersion = ${record.graphVersion}，与当前进程 graphVersion = ${current} 不一致。这张单按旧图跑到一半，新图无法接。`,
      ),
      {
        code: "GRAPH_VERSION_MISMATCH",
        checkpointGraphVersion: record.graphVersion,
