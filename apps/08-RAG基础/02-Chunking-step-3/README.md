# 切块（Chunking） · 第三步 · 怎么判断切得好不好 + 5 种文档类型选策略

4 维度观察（半句话开头数 / 最高分变化 / 块大小方差 / 重复度）+ FAQ / 长手册 / 法规 / 会议纪要 / 散文的不同推荐参数——纯本地文本操作。

## 跑入口

```bash
cd apps && yarn app:08-02-chunking-step-3
# 浏览器：http://127.0.0.1:50074/
```

## 端口

`50074`（已与 `apps/package.json` 的 `PORT=50074` inline + `lib/http/runtime-ctx.ts` 的 `.default(50074)` + `public/components/layout.js` 的页脚 fallback + `apps/README.md` 占用表 四处一致）。
...
- **第二类错误（5xx）**：点「演示上游失败（5xx · 第二类错误 · /api/demo-error）」→ HTTP 500 + 红字。两类红字分开可见。
...
- step-2 起手做需求 3（单位+兜底）+ 5/6（量化 overlap 代价）+ 9~15（递归完整 / 标题继承 / PDF / 不能腰斩 / 父子切块）
