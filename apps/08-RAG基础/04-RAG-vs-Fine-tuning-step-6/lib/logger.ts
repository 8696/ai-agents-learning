/**
 * 职责：本地 freeze 副本 · 拷自顶层 apps/logger.ts（拷于 2026-09-12）
 *
 * 拷完后再加：
 *   1. 顶部两行 import（path / fileURLToPath）
 *   2. 底部一行 `export const logger = createLogger(...)`，路径必须用 path.resolve(__dirname, "..", "logs")
 *
 * 细则：agents/05-demo.md §5.3.16。
 */
...
// ── 本地 freeze 副本固定路径写法（§5.3.16）──
// 禁止 new URL("./logs/", import.meta.url)（会把 logs/ 落到 lib/logs/ 而非本 demo 根目录）
