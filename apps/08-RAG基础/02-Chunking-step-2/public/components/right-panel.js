/**
 * 职责：右栏专用组件（FAQ 切）——自带「文档来源说明」卡片，演示「库长什么样是后台做的（不是用户问时临时算的）」。
...
(function () {
  ...
  // 右栏依赖 chunk-panels.js 挂出来的通用组件——IIFE 执行时 chunk-panels.js 已先挂上；
  // 这里从 DemoUI 取出来给本文件闭包用，否则 JSX <StatsBar /> 转译后是裸变量查找 → ReferenceError。
  const StatsBar = DemoUI.StatsBar;
  const ChunkCard = DemoUI.ChunkCard;
      ...
      <section className="bg-white shadow rounded p-4 space-y-3">
        ...
        <div className="border border-gray-300 bg-gray-50 rounded p-2 space-y-1">
          ...
          <div className="text-xs text-gray-600 pt-1">
            ⚠ <b>不在请求路径上</b>让模型整理——库长什么样是后台做的（不是用户问时临时算的），不是用户问问题时顺手做的。
          </div>
        </div>
        {error ? (
