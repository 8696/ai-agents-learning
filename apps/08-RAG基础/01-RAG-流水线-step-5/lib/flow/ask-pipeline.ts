...
function buildPrompt(
  ...
  const materials = hits
    .map((hit, index) => {
      const pageTag = hit.page !== undefined ? `（第 ${hit.page} 页）` : "";
      return `【材料${index + 1}】${hit.source} / ${hit.section}${pageTag}（分数 ${hit.score.toFixed(3)}）\n${hit.text}`;
    })
    .join("\n\n");
  ...
  return withCall({
    ...
    run: async () => {
      ...
      return {
        ...
        retrieval,
        hits: hits.map((hit) => {
          const out: { id: string; score: number; source: string; section: string; text: string; page?: number } = {
            id: hit.id,
            score: hit.score,
            source: hit.source,
            section: hit.section,
          ...
          if (hit.page !== undefined) out.page = hit.page;
          return out;
        }),
        prompt,
        answer,
