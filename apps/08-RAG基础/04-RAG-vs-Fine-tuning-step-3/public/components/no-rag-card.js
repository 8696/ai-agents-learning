// 职责：左栏「不接 RAG（model only · 无材料 · 无出处）」卡片（受控组件）。
// 数据流：父组件传 question / data / loading / error / forceErrorLoading → 渲染按钮 + 结果。
// 「演示后端 5xx」按钮 → onRunNoRag(true) → 父组件 fetch("/api/no-rag", {forceError:true}) → 5xx。
