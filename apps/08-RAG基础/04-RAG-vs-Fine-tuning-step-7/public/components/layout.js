...
function EnvFooter() {
  ...
  if (!info) {
    return <span id="env-info">端口 50086 · 协议 A（openai Chat Completions）· 加载中…</span>;
  }
  const providerLabel =
  ...
  return (
    <span id="env-info">
      端口 {info.port} · 本地展示（部分 sub-page 调模型） · 模型服务商 {providerLabel} · 模型 {info.model ?? "—"} · 密钥 {info.hasKey ? "✅" : "❌（apps/.env 未配置当前模型服务商的 Key · 本步部分 sub-page 不需要）"}
    </span>
  );
}
