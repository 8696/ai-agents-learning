/**
 * 本地 freeze 副本 · 拷自顶层 apps/logger.ts（2026-09-11）
 *
 * 职责：服务端文件日志 —— 业务调 logger.info(scope, msg, explain, data?) 写进 logs/{YYYY-MM-DD}.log。
 *
 * 模板说明：apps/logger.ts 是「仅作拷贝源」——本文件是 02-Chunking-step-1 的本地副本，
 * 业务代码只 import 本文件，不 import 顶层。
 *
 * 数据流：
 *   logger.info(scope, msg, explain, data?)
 *     → 文件：appendFileSync(logs/{YYYY-MM-DD}.log, …)
 *     → console：[LEVEL] [scope] msg — explain (+ data)
 *
 * 文件日志禁止截断（§5.3.16）：无 MAX_BYTES / MAX_DEPTH；入参 / 返回值原样写全。
 *
...
// ── 内置安全序列化（处理 Error / Map / Set / Date / Buffer / 循环引用）──
...
// ── 本 demo 的 logger 实例 ──
// §5.3.16：唯一合法写法 = path.resolve(__dirname, "<n 层 ..>", "logs")
