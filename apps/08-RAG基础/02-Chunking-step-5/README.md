# 切块（Chunking） · 第五步 · 递归切分完整版

## → 段落 → 句号 → 兜底再切 四级显式降级——纯本地文本操作，不调 LLM。

## 跑入口

```bash
cd apps && yarn app:08-02-chunking-step-5
# 浏览器：http://127.0.0.1:50076/
```

## 端口

`50076`（已与 `apps/package.json` 的 `PORT=50076` inline + `lib/http/runtime-ctx.ts` 的 `.default(50076)` + `public/components/layout.js` 的页脚 fallback + `apps/README.md` 占用表 四处一致）。
