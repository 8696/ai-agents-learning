      ...
      for (const p of paragraphs) {
        ...
        if (p.text.length > 800) {
          ...
          for (const s of sentences) {
            ...
            chunks.push(makeChunk(idx++, s.text, offset + s.start, offset + s.end, "句号", section));
          }
        } else {
          chunks.push(makeChunk(idx++, p.text, offset + p.start, offset + p.end, "段落", section));
        }
      }
    } else {
      chunks.push(makeChunk(idx++, sec.text, offset + sec.start, offset + sec.end, "##", section));
    }
    offset += sec.text.length;
  }
