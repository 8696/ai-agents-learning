...
function EnvFooter() {
  ...
  if (!info) {
    return <span id="env-info">端口 50083 · 本地展示（不调 LLM）· 加载中…</span>;
  }
  const providerLabel =
  ...
  return (
    <span id="env-info">
      端口 {info.port} · 本地展示（不调 LLM） · 模型服务商 {providerLabel} · 模型 {info.model ?? "—"} · 密钥 {info.hasKey ? "✅" : "❌（apps/.env 未配置当前模型服务商的 Key · 本步不调 LLM，无影响）"}
    </span>
  );
}
