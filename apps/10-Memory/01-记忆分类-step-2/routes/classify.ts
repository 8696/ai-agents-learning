/**
 * 职责：POST /api/classify，校验一句用户原话后调 classifyMemory 判类（带可选 persist 字段）。
 * 数据流：{ sentence, persist? } → Zod 校验 → classifyMemory(sentence, { persist }) → ctx.body。
 */
import { z } from "zod";
...
export function mountClassifyRoutes(router: Router): void {
  router.post("/api/classify", async (ctx: Context) => {
    ...
    const shouldPersist = parsed.data.persist === true;
    const t0 = Date.now();
    logger.info(
      "调用函数-classify路由",
      "调用函数开始：classifyMemory",
      "为什么写这条日志：这一层只认 classifyMemory 的返回结果；里面那次调对话补全才是真发网络请求（看下一条「调用模型开始：对话补全」）。当前：收到一句待判类的话，外加是否持久化的开关。",
      { 入参: { sentence, persist: shouldPersist }, __code: "const output = await classifyMemory(sentence, { persist: shouldPersist });" },
    );

    try {
      const output = await classifyMemory(sentence, { persist: shouldPersist });
      logger.info(
        "调用函数-classify路由",
        "调用函数结束：classifyMemory",
        "为什么写这条日志：要把判类结果和完整请求/响应交给页面展示；持久化开启时还要把写入结果（写没写 / 为什么）一起带上。当前：判类完成，下一步写 ctx.body。",
