 ...
 *
 * step-4 扩展：新增 rejectReason 字符串 `PUBLIC_KNOWLEDGE`（维度 C 拦下，紫色徽标）。
 */
(function () {
    ...
    if (items.length === 0) {
      return (
        <div className="border border-gray-300 bg-gray-50 rounded p-3 text-sm text-gray-600">
          当前四道闸门下<b>没有候选通过</b>——要么抽取阶段就没出候选，要么所有候选都被某一道闸门拦下了。
        </div>
      );
    }
    return (
  ...
  DemoUI.RejectedList = function RejectedList(props) {
    ...
    if (items.length === 0) {
      return (
        <div className="border border-gray-300 bg-gray-50 rounded p-3 text-sm text-gray-600">
          当前四道闸门下<b>没有候选被拦下</b>——所有抽出来的候选 confidence 都达到阈值、且都通过了维度 A / B / C 的判定。
        </div>
      );
    }
    return (
  ...
  DemoUI.FilterSummary = function FilterSummary(props) {
    ...
    lines.push("");
    lines.push(`维度开关：维度 A「是不是事实」<b>${enableA ? "开" : "关"}</b>　·　维度 B「跨会话还有用吗」<b>${enableB ? "开" : "关"}</b>　·　维度 C「值不值得占存储」<b>${enableC ? "开" : "关"}</b>`);
    return (
