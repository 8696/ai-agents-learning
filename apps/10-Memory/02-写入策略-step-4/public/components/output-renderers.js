 ...
 *
 * step-4 扩展：四道闸门的结果分五栏（绿 / 黄 / 橙 / 红 / 紫）+ 紫色栏接 PUBLIC_KNOWLEDGE 拒绝原因字符串。
 */
(function () {
  const DemoUI = window.DemoUI || (window.DemoUI = {});

  // 把 verdict 列表按 rejectReason 分桶（用于分五栏展示）
  function bucketByReason(verdicts) {
    const buckets = { PASSED: [], BELOW_THRESHOLD: [], PROGRAMMATIC_RULE: [], SESSION_ONLY: [], PUBLIC_KNOWLEDGE: [] };
  ...
  // /api/filter-dimensions 的渲染：四道闸门的结果分五栏（绿 / 黄 / 橙 / 红 / 紫）+ 两次模型调用明细
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <ModelTrace
            ...
            skipped={!r.enableA && !r.enableB && !r.enableC}
          />
        </div>
      </div>
    );
  ...
  DemoUI.FilterOutput = function FilterOutput(props) {
    ...
    return (
      <div className="border border-gray-300 bg-gray-50 rounded p-3 text-sm text-gray-700">
        这是「只把关（置信度）/ 不判维度」的对照结果——共抽出 <b>{r.originalCount}</b> 条候选，按置信度阈值 <b>{fmtThreshold(r.threshold)}</b> 切完剩 {r.passed.length} 条通过 / {r.rejected.length} 条被拦下。跟「四道闸门」对比就能看见：多加三道维度闸门切掉了什么。
      </div>
    );
  };
  ...
  DemoUI.ExtractOutput = function ExtractOutput(props) {
    ...
    return (
      <div className="border border-gray-300 bg-gray-50 rounded p-3 text-sm text-gray-700">
        这是「只提取 / 跳过所有把关」的对照结果——共抽出 <b>{r.candidates.length}</b> 条候选，不做任何判定全部进库。跟「把关」+「四道闸门」对比就能看见整条流水线切掉了什么。
      </div>
    );
  };
            ...
            return (
              <li key={idx} className="border border-gray-200 rounded p-2 text-xs text-gray-700">
                ...
                <p className="text-gray-500">
                  阈值 {fmtThreshold(item.threshold)} · A {item.enableA ? "开" : "关"} / B {item.enableB ? "开" : "关"} / C {item.enableC ? "开" : "关"} ·
