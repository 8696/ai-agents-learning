 ...
 *
 * 职责：拿已经通过置信度阈值的候选清单，让模型给每条判三道维度的通过 / 拦下。
 * 数据流：FilterVerdict[]（来自 filterByConfidence.passed）+ 维度开关 { enableA, enableB, enableC }
 *   → 拼 messages（system=三道维度判定规则 + JSON 输出格式；user=候选清单）
 *   → 调协议 A 对话补全（response_format: json_object）
 *   → Zod 校验返回数组（按候选顺序、长度等于输入条数）
 *   → 在原 FilterVerdict 上叠加 rejectReason（PROGRAMMATIC_RULE / SESSION_ONLY / PUBLIC_KNOWLEDGE）或保留 passed=true
 *   → 返回 FilterVerdict[]
 *
 * 为什么这三道都由模型判：三道都需要语义理解（"全员规则 vs 个人事实"、"本轮临时 vs 跨会话稳定"、"公开常识 vs 个人事实"），
 * 正则规则抓不住换了说法的表达——这是文档 §9「谁判：模型负责哪几步、代码负责哪几步」分工表的依据。
 *
 * 为什么单独成文件：写入策略八关里第 3 关「把关」有几道独立的判定（维度 A/B/C、置信度、敏感信息、人工确认），
 * 每一道都要单独看日志、单独换实现、单独测。维度 A/B/C 单独成文件，后面再加敏感信息过滤 / 人工确认时
...
function buildSystemPrompt(toggle: DimensionsToggle): string {
  ...
  const lines: string[] = [];
  lines.push("你是一个「候选事实把关器」。给你一组已经从原文里抽出来的候选事实（每条带 index / key / value / type），按下面三道维度给每条判「过 / 不过」并写一句理由。");
  lines.push("");
  lines.push("判定的三道维度（开 / 关由调用方决定，开的维度要判，关的维度全部默认 true）：");
  if (aOn) {
  ...
  lines.push('{"judgments": [{"index": 0, "isFact": true|false, "isCrossSession": true|false, "isStorageWorth": true|false, "reasoning": "一句话理由"}, ...]}');
...
export async function filterByContentDimensions(
  ...
  if (!toggle.enableA && !toggle.enableB && !toggle.enableC) {
    // 三道维度都关 → 直接把所有候选标 passed=true，不调模型
    return {
  ...
  logger.info(
    "│ 调用模型-内容维度判定",
    "调用模型开始：内容维度判定",
    `为什么写这条日志：这是真发网络请求的那一次，维度 A / B / C 都需要语义理解。当前：拿到 ${candidates.length} 条候选（已过置信度阈值），准备让模型判三道维度。`,
  ...
  // 优先级：维度 A > 维度 B > 维度 C（任一不过即拦下；先报的是最先发现的问题）
