 ...
 *       走三道闸门：extractFacts → filterByConfidence → filterByContentDimensions。
 * 数据流：{ text, threshold, enableA, enableB }
 ...
 *         → ③ 内容维度 A/B（语义判定，过不去标 PROGRAMMATIC_RULE / SESSION_ONLY）
 *
...
export interface FilterDimensionsResponse {
  ...
  /** filterByContentDimensions 的请求 / 响应（来自第三道闸门；前两道都关时为 null） */
...
export function mountFilterDimensionsRoutes(router: Router): void {
  router.post("/api/filter-dimensions", async (ctx: Context) => {
    ...
    if (!parsed.success) {
      sendError(ctx, 400, {
        error: "BAD_BODY",
        explain: "请求体要有 text 字符串 + threshold 数字（0~1）+ enableA / enableB 布尔。",
      });
      return;
    }
    ...
    const { threshold, enableA, enableB } = parsed.data;
    const t0 = Date.now();

    logger.info(
      "调用函数-filter-dimensions路由",
      "调用函数开始：filter-dimensions",
      `为什么写这条日志：这一层调三道闸门（提取 + 置信度 + 内容维度）；真发网络请求的有两次（提取 + 维度判定，分别看子调用）。当前：收到原文 + threshold=${threshold} + enableA=${enableA} + enableB=${enableB}。`,
      { 入参: { text, threshold, enableA, enableB }, __code: "const output = await filterDimensions(text, threshold, enableA, enableB);" },
    );
    ...
    try {
      ...
      logger.info(
        ...
        `为什么写这条日志：要把三道闸门的结果和完整请求 / 响应交给页面展示。当前：把关完成，通过 ${output.passed.length} / 置信度拦下 ${output.rejectedByThreshold.length} / 维度拦下 ${output.rejectedByDimension.length}。`,
