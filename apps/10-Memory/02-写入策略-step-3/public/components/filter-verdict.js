 ...
 *
 * step-3 扩展：每条 verdict 可选带 reasoning（维度判定时由模型给出，拦下原因的人话解释）。
 */
(function () {
  ...
  // 单条候选卡片（含把关判定）。
  // 通过：绿色描边 + 右上「PASSED」徽标。拦下：按 rejectReason 取对应颜色 + 右上「REJECTED · 拦下原因」徽标 + 拦下理由。
  // step-3 扩展：维度判定会附 reasoning（模型给的一句话人话理由）。
    ...
    if (items.length === 0) {
      return (
        <div className="border border-gray-300 bg-gray-50 rounded p-3 text-sm text-gray-600">
          当前三道闸门下<b>没有候选通过</b>——要么抽取阶段就没出候选，要么所有候选都被某一道闸门拦下了。
        </div>
      );
    }
    return (
  ...
  DemoUI.RejectedList = function RejectedList(props) {
    ...
    if (items.length === 0) {
      return (
        <div className="border border-gray-300 bg-gray-50 rounded p-3 text-sm text-gray-600">
          当前三道闸门下<b>没有候选被拦下</b>——所有抽出来的候选 confidence 都达到阈值、且都通过了维度 A / B 的判定。
        </div>
      );
    }
    return (
  ...
  DemoUI.FilterSummary = function FilterSummary(props) {
    ...
    lines.push("");
    lines.push(`维度开关：维度 A「是不是事实」<b>${enableA ? "开" : "关"}</b>　·　维度 B「跨会话还有用吗」<b>${enableB ? "开" : "关"}</b>`);
    return (
