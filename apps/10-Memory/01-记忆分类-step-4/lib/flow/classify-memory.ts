 ...
 *
 * 写入扩展（选项 C 写入侧严格度）：
 *   classifyMemory 接受可选入参 { persist?: boolean }，开启后会调 lib/storage/facts-store
 *   的 recordIfLongTerm，按「工作记忆 / 短期 → 不写；其他 → 写」规则把分类结果写进
 *   data/facts.json。写入判定 / 写入是 facts-store 的职责，本文件只负责「拿到分类结果后，
 *   顺手调一下 facts-store 并把它的返回挂到自己返回值里交给上层」。
 *
 ...
 *   → 若 opts.persist === true → 调 facts-store.recordIfLongTerm
 *   → 返回 { classification, modelRequest, modelResponse, durationMs, persistence? }，完整请求/响应原样交给页面展示。
 *
...
import { recordIfLongTerm, type PersistDecision } from "../storage/facts-store.js";
...
export interface ClassifyMemoryOutput {
  ...
  durationMs: number;
  /** 仅在 opts.persist === true 时存在：本次写入结果（写了 / 没写 + 原因） */
  persistence?: PersistDecision;
}

export interface ClassifyMemoryOptions {
  /** 是否把分类结果按规则写入到 data/facts.json。默认 false（不写入）。 */
  persist?: boolean;
}
...
export async function classifyMemory(
  sentence: string,
  opts: ClassifyMemoryOptions = {},
): Promise<ClassifyMemoryOutput> {
  const llm = getLlm();
  const messages = [
  ...
  // 写入扩展：opts.persist === true 时按 facts-store 的规则把这条分类结果写盘。
  // 不论写没写，都把 facts-store 的返回挂到 persistence 字段，方便上层路由上页展示。
  let persistence: PersistDecision | undefined;
  if (opts.persist) {
    persistence = recordIfLongTerm({
      sentence,
