...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/hybrid.html", label: "混合编排（路由层 hard-code）", match: "hybrid" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/hybrid.html")) return "hybrid";
    return "";
  }
  ...
  /**
   * step-7 是 mock demo：页脚标"本地 mock · 不调 LLM"。
   * 端口兜底 50023（§5.3.3 算的默认口），真实值仍以 /health 返回的为准。
   */
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50023} · 本地 mock · 不调 LLM · provider{" "}
