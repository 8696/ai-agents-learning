...
// ── A 组 · 描述无反例（baseline）──
//   description 太短/太泛、只说「查物流」+「订单」关键词，user 问订单地址时被「订单」字眼带偏
//   order_id.description 写明「订单号=快递单号=tracking number」别名映射（避免字面不一致导致模型犹豫）
...
// ── B 组 · 描述含反例（improved）──
//   description 加一句反例：「不要用于查订单状态/金额/地址」—— 拦下语义相近的 query
//   order_id.description 同 A 组别名映射
