...
- **4 处关键差异**（与 step-6 协议 A 对照看）：
  ...
  - **塞回 messages结果**：协议 A `{role:"tool", tool_call_id, content}` vs 协议 B `{role:"user", content:[{type:"tool_result", tool_use_id, content}]}`
...
| 塞回 messages结果 role | `"tool"` | `"user"` + `tool_result` blocks |
