 ...
 *   getToolsForLLM / getToolsMeta / getToolNames 是普通函数——五条日志（含 __code）仍要。
 */
...
export function executeTool(
  ...
  logger.info(
    "│ 工具执行-executeTool",
    "调用函数结束：executeTool",
    "为什么写这条日志：完整写 tool_result：模型第二轮拿到后拼人话回复（callProtocolA round-2）。",
    {
...
/** 给 LLM 看：name + description + parameters（JSON Schema），不含 dangerous 标记 */
export function getToolsForLLM() {
  ...
  const tools = Object.values(TOOLS).map((t) => ({
    ...
    description: t.description,
    dangerous: t.dangerous,
    parameters: (t as unknown as { parameters?: unknown }).parameters ?? null,
  }));
  logger.debug(
    "│ Registry-getToolsForLLM",
    "调用函数结束：getToolsForLLM",
    "为什么写这条日志：debug 是「细节」等级；tools 在 chat.ts 启动时一次性派生，缓存用。当前：派生 LLM 用 tools schema（OpenAI Chat Completions tools 数组里单项的形状）。",
    {
