 ...
 *     · `input` 是**对象**（vs vs `tool_calls[i].function.arguments` = JSON 字符串）
...
//   每一项 = 一份「工具契约」告诉模型：你可以调这个，参数长值样（JSON Schema）。
...
export type AnthropicContentBlock =
  ...
  | { type: "tool_result"; tool_use_id: string; content: string | AnthropicContentBlock[]; iserror?: boolean };
