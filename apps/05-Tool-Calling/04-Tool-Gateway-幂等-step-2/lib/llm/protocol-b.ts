...
export async function callProtocolB(request: ProtocolBRequest): Promise<ProtocolBResponse> {
  ...
  logger.info(
    ...
    "为什么写这条日志：route 只认这一层返回的 ProtocolBResponse；里面那次才是真发网络请求（看「调用模型开始：协议B-消息创建」）。当前：调 Anthropic 协议 B 发起请求；完整写 request 便于核对 model / messages / tools / max_tokens / system。",
    {
      入参: { model: request.model, max_tokens: request.max_tokens, messagesCount: request.messages.length, toolsCount: request.tools?.length ?? 0, hasSystem: Boolean(request.system) },
