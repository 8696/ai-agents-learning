 ...
 *   闸门挡掉（400）单独打 warn；子调用 callProtocolB / executeTool 内部已自带五件套。
 */
...
async function callLlmOnce(messages: AnthropicChatMsg[], system?: string): Promise<CallResult> {
  ...
  try {
    ...
    logger.error(
      "│ chat-callLlmOnce",
      "调用函数结束：callLlmOnce（失败）",
      "为什么打：协议 B 抛异常（网络 / 5xx / 4xx）；记 upstreamStatus + 错误信息便于排错。",
      {
...
export function mountChatRoutes(router: Router): void {
  router.post("/api/chat", async (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的响应包；里面两轮 callLlmOnce + executeTool 是「真活」。当前：step-1 演示「模型发 tool_use → Gateway 三钩子 → 拒绝/通过/二次确认」。",
      {
    ...
    const toolResults: ExecResult[] = toolUsesFromLLM.map((tu) => {
      ...
      logger.info(
        "││ chat-handlePostChat",
        `tool_result（${tu.name}）`,
        "为什么打：Tool 走完 Registry + Gateway；打 ok + code + retryAfterMs 摘要（#14 端到端透传）。",
        {
    ...
    if (toolUsesFromLLM.length === 0) {
      ...
      logger.info(
        ...
        "为什么打：模型没调 delete_user；可能是 query 不够触发。",
        {
    ...
    logger.info(
      ...
      "为什么打：route 要把响应包（4 张数据卡 + 钩子判定链）写进 ctx.body 交给页面 stats 区；记 finalLen 便于核对。",
      {
