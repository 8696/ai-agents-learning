...
- **演示协议 B 物理形态**：content blocks 数组、tool_use.input 是对象、必填 max_tokens、tool_result 用 `role:"user"` + blocks 回灌
...
| **协议 B 字段层形态** | Round 1/2 数据卡 | content 是 blocks 数组、tool_use.input 是对象、必填 max_tokens、回灌用 role:"user" + tool_result blocks |
| **三钩子顺序** | 钩子判定链 | 鉴权（必须最先）→ 配额（每月上限）→ 危险（confirm_token）—— 任一不过立即停，不进入下一钩 |
