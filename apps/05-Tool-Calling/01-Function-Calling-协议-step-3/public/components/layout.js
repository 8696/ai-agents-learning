...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/single.html", label: "单跑（parallel | serial）", match: "single" },
    { href: "/pages/compare.html", label: "串/并行对比", match: "compare" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/single.html")) return "single";
  ...
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50019} · 本地 mock · 不调 LLM · provider{" "}
