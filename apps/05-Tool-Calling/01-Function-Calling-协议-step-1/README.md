...
- **「试危险工具 calc（gateway 必拦）」** → Gateway 拦下，把 `{ok:false, error}` 塞回 messages；trace 显示 **kind="reject" 红边卡片**（pill 仍 ✅，因为 API 本身成功，是工具层失败）
...
| 错误回传 | 真实 handler 失败（DB 超时 / 第三方 5xx）包成 `{ok:false, error}` 塞回 messages模型（这是 step-1 已经覆盖的，加进 § 错误处理例子里） |
