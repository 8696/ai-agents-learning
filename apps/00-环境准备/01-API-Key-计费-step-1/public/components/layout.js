/**
 * 职责：页面壳 —— 顶栏状态 pill、页间导航、说明区、页脚环境条，以及拉 /health 的 hook。
 *
 * 数据流：GET /health → useEnvInfo → EnvFooter（provider / model / port / Key）
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/usage.html", label: "单次计费", match: "usage" },
    { href: "/pages/compare.html", label: "输入/输出对照", match: "compare" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/usage.html")) return "usage";
  ...
  function EnvFooter({ env }) {
    return (
      <footer id="page-footer" className="border-t p-2 text-xs text-gray-500 text-center">
        <span id="env-info">
          端口 {env.port || 50001} · 协议 A · provider{" "}
