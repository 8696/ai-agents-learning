/**
 * 模块 00 · 最小 HTTP + SSE 服务端（协议 A · OpenAI 兼容）
 *
 ...
 *     → Node http.createServer 收 body → Zod 校验请求体
 ...
 *
 * 进阶能力（取消、限流、对照演示）在 apps/02-LLM-API开发/ 对应小节，
 * **不**塞进本入口（演示动作会污染产品入口）。
 *
 * 对应路径：apps/00-环境准备/01-mini-app/src/server.ts
 *
...
server.listen(env.PORT, "127.0.0.1", () => {
  console.log(`──── 00 环境准备 · HTTP + SSE Server ────`);
