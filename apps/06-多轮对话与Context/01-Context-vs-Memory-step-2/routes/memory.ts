...
export function mountMemoryRoutes(router: Router): void {
  router.post("/api/memory", async (ctx: Context) => {
    ...
    const t0 = Date.now();
    logger.info("memory.set", "调用函数开始：memory.set", "为什么打：演示 O1 写入（用户偏好 → SQLite 持久化）；写入后下次会话发送就能从 db 读出注入 system。当前：用户主动点「记住偏好」按钮。", {
      入参: { userId: USER_ID, key, value },
      __code: "await kvSet(USER_ID, key, value);",
    });
    await kvSet(USER_ID, key, value);
    logger.info("memory.set", "调用函数结束：memory.set", "为什么打：写入成功；下次发送会通过 kvList 读到这条偏好注入 model。当前：db 已落盘 preferences.db。", {
  ...
  router.get("/api/memory", async (ctx: Context) => {
    const t0 = Date.now();
    logger.info("memory.list", "调用函数开始：memory.list", "为什么打：演示 O2 列出（前端显示当前已记住的所有偏好）；页面加载时拿一次填 UI。当前：前端 GET 触发。", {
      入参: { userId: USER_ID },
      __code: "const prefs = await kvList(USER_ID);",
    });
    const prefs = await kvList(USER_ID);
    logger.info("memory.list", "调用函数结束：memory.list", "为什么打：返回给前端 + 给学习者看（页面 #memory-list 区域）。当前：rows → Record。", {
  ...
  router.delete("/api/memory", async (ctx: Context) => {
    ...
    const t0 = Date.now();
    logger.info("memory.del", "调用函数开始：memory.del", "为什么打：演示 O4 删除（用户要求清某条偏好）；step-2 只保留端点，UI 上 step-3 才显眼（验证链路）。当前：DELETE 触发。", {
      入参: { userId: USER_ID, key },
      __code: "await kvDel(USER_ID, key);",
    });
    await kvDel(USER_ID, key);
    logger.info("memory.del", "调用函数结束：memory.del", "为什么打：证明 db 操作成功；下次发送那条偏好就不会再被注入。当前：rows - 1（如果之前存在）。", {
      返回值: { ok: true, key },
      耗时ms: Date.now() - t0,
    });
