...
export function mountErrorDemoRoutes(router: Router): void {
  router.post("/api/chat-force-error", async (ctx: Context) => {
    logger.warn("chat.error_demo", "调用函数结束：chat-force-error", "为什么打：教学演示 5xx 通道；让学习者看见上游失败时页面长什么样", {
