/**
 * 职责：POST /api/summarize-force-error —— 教学演示用的「演示后端 5xx」端点，立刻回 502。
 *
...
export function mountErrorDemoRoutes(router: Router): void {
  router.post("/api/summarize-force-error", async (ctx: Context) => {
    logger.warn("summarize.error_demo", "调用函数结束：summarize-force-error", "为什么写这条日志：教学演示 5xx 通道；让学习者看见后端 5xx 时页面长什么样", {
