/**
 * 职责：POST /api/compare-force-error —— 教学演示用的「上游失败」端点，立刻回 502。
 *
 * 数据流：无入参 → ctx.status = 502 + 结构化错误 → 前端红字 + #status-pill ❌。
 *
 * 教学锚点（§5.3.2 #2 · 「错误处理 ≥2 类，能一眼分开」）：
 *   - 类 A（4xx）走 /api/compare 的 Zod 校验 → 400（缺 messages / 缺 keyFact）
...
export function mountErrorDemoRoutes(router: Router): void {
  router.post("/api/compare-force-error", async (ctx: Context) => {
    logger.warn("compare.error_demo", "调用函数结束：compare-force-error", "为什么写这条日志：教学演示 5xx 通道；让学习者看见上游失败时页面长什么样", {
