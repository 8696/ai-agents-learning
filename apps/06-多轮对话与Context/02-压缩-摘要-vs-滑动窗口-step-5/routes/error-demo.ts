/**
 * 职责：POST /api/token-budget-force-error —— 教学演示用的「演示后端 5xx」端点，立刻回 502。
 */
...
export function mountErrorDemoRoutes(router: Router): void {
  router.post("/api/token-budget-force-error", async (ctx: Context) => {
    logger.warn("token-budget.error_demo", "调用函数结束：token-budget-force-error", "为什么写这条日志：教学演示 5xx 通道", {
