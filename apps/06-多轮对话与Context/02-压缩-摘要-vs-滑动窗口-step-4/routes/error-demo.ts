 ...
 *   - 类 A（4xx）走 /api/three-way-with-fallback 的 Zod 闸门 → 400
...
export function mountErrorDemoRoutes(router: Router): void {
  router.post("/api/three-way-force-error", async (ctx: Context) => {
    logger.warn("three-way.error_demo", "调用函数结束：three-way-force-error", "为什么打：教学演示 5xx 通道（与 step-4 的「摘要失败 → 自动降级 200」形成对照）", {
