 ...
 * 分叉只在本文件。
 *
 * 日志（§5.3.16）：调用函数 五件套（handlePostCase2 封装层）；
  ...
  router.post("/api/case2-with-history", async (ctx: Context) => {
    ...
    if (!client) {
      logger.info(
        ...
        "POST /api/case2-with-history 被无 Key 闸门挡掉",
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的 CaseResponse；里面 A/B handler 是「真活」。当前：Case 2（3 轮 user / assistant / user 含历史）即将并发跑 A / B。",
      {
        入参: {
    ...
    try {
      ...
      logger.info(
        ...
        "为什么打：route 要把 CaseResponse 写进 ctx.body；记两边结果状态便于核对「A 提到北京 vs B 提到北京」。当前：allSettled 已返回。",
        {
          返回值: {
            caseName: CASE_WITH_HISTORY.caseName,
            aStatus: aRes.status,
      ...
      logger.error(
        ...
        "为什么打：Case 2 整条 handler 抛异常（不是 A/B 单边失败 —— 那是 Promise.allSettled 兜住的）；写 500 给前端，记 error 排错。当前：allSettled 之外的代码抛错。",
        {
