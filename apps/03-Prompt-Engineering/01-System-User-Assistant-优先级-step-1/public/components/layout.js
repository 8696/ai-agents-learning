 ...
 * 数据流：GET /health → 页脚同时写 modelA / modelB（§5.3.13 对照例外）。
...
(function () {
  ...
  var NAV_ITEMS = [
    ...
    { href: "/pages/priority.html", label: "Case 1 优先级", match: "priority" },
    { href: "/pages/with-history.html", label: "Case 2 有历史", match: "with-history" },
    { href: "/pages/no-history.html", label: "Case 3 无历史", match: "no-history" },
  ];

  function currentKey() {
    var p = window.location.pathname;
    if (p === "/" || p.endsWith("/index.html")) return "index";
    if (p.endsWith("/priority.html")) return "priority";
    if (p.endsWith("/with-history.html")) return "with-history";
    if (p.endsWith("/no-history.html")) return "no-history";
    return "";
  }
