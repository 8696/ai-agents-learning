 ...
 *   闸门挡掉单独打 warn / info；子调用 compareVersions 内部已自带五件套。
 */
...
export function mountCompareRoutes(router: Router): void {
  router.post("/api/compare", async (ctx: Context) => {
    ...
    logger.info(
      ...
      "为什么打：route 只认这一层返回的对照结构；里面 compareVersions 是「真活」。当前：前端发来对照请求；记 query 摘要便于复盘。",
      {
    ...
    if (!currentLlm) {
      logger.info(
        ...
        "为什么打：闸门已回 503；route 不用再算 compareVersions。当前：requireLlm 已返回 null（闸门在内部写过 error）。",
        {
    ...
    if (!body) {
      logger.info(
        "api.compare",
        "调用函数结束：handlePostCompare",
        "为什么打：闸门已回 400；route 不用再算 compareVersions。当前：readCompareBody 已返回 null（闸门在内部写过 warn）。",
        {
    ...
    if (packed.allFailed) {
      logger.error(
        ...
        "为什么打：两版都失败时把 HTTP 状态抬成上游码；记 status 便于前端显示「上游挂了」。当前：allFailed 已给出上游码 + error。",
        {
    ...
    logger.info(
      "api.compare",
      "调用函数结束：handlePostCompare",
      "为什么打：route 要把对照结构（input / versions / results）写进 ctx.body 交给页面 stats 区；记 results 数便于核对。当前：compareVersions 已返回，至少一版成功。",
      {
