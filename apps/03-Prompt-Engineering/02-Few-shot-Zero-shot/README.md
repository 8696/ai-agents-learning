...
对应小节：[docs/学习模块/03-Prompt-Engineering/02-Few-shot-Zero-shot.md](../../../docs/学习模块/03-Prompt-Engineering/02-Few-shot-Zero-shot.md)
...
cd apps
yarn install
yarn app:03-02-few-shot-zero-shot
```

启动后打开 `http://127.0.0.1:50302/`。需要 `apps/.env` 里的 `MINIMAX_API_KEY`（协议 A）。并排对比一次 = 两次短请求。
...
- **错误处理**：空输入 → HTTP 400；Key 缺失 / 上游 API 失败 → 5xx 文案进红字；浏览器 25s abort → `#status-pill` 红色（网络/超时）。
...
`docs/学习模块/03-Prompt-Engineering/02-Few-shot-Zero-shot.md`（讲完后沉淀；本 README 只描述代码怎么跑）。
